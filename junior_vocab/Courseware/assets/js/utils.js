...
const STORAGE_PREFIX = (typeof TEXTBOOK_DATA !== 'undefined' && TEXTBOOK_DATA.book?.id)
  ? String(TEXTBOOK_DATA.book.id).replace(/[^a-zA-Z0-9]+/g, '_') + '_'
  : 'pep_junior_';
...
/**
 * 为缺少 COS 配图的单词生成插画风 SVG 词卡（data URI）
 * 保证每个词都有可用主图，游戏/听写/学习页均可直接使用
 */
function makeWordCardDataUri(w, variant = 1) {
  const word = (w && w.word) || '';
...
/** 解析单词主图：优先 COS，失败/缺失时用 SVG 词卡 */
function resolveWordImage(w, preferSecond = false) {
  if (!w) return makeWordCardDataUri({ word: '?', chinese: '' }, 1);
  const primary = preferSecond ? (w.image2 || w.image) : w.image;
  const fallback = preferSecond ? (w.image || '') : (w.image2 || '');
  return primary || fallback || makeWordCardDataUri(w, preferSecond ? 2 : 1);
}

function bindWordImage(imgEl, w, preferSecond = false) {
  if (!imgEl || !w) return;
  const src = resolveWordImage(w, preferSecond);
  imgEl.alt = w.word || '';
  imgEl.onerror = () => {
    imgEl.onerror = null;
    imgEl.src = makeWordCardDataUri(w, preferSecond ? 2 : 1);
  };
  imgEl.src = src;
}

/** 渲染音素盒子；初中无音素拆分时展示 IPA */
...
/** 启动时补齐缺失配图（COS 缺失 → SVG 词卡） */
function ensureWordImages() {
  if (typeof TEXTBOOK_DATA === 'undefined' || !TEXTBOOK_DATA.units) return;
  TEXTBOOK_DATA.units.forEach((u) => {
    (u.words || []).forEach((w) => {
      if (!w.image) w.image = makeWordCardDataUri(w, 1);
      if (!w.image2) w.image2 = makeWordCardDataUri(w, 2);
      if (!Array.isArray(w.phonemes)) w.phonemes = [];
