...
def call_deepseek(prompt: str) -> str:
    body = json.dumps({
        "model": "deepseek-chat",
        "messages": [
            {"role": "system", "content": "你是小学英语教师，为五年级学生编写例句。只输出合法 JSON。"},
...
def main():
    ...
    all_words = [w for ws in UNITS_5SHANG.values() for w in ws if w not in existing]
    ...
    for i in range(0, len(all_words), 12):
        ...
        prompt = f"为以下五年级单词各写1条例句（8-12词，生活化，含目标词，附中文）：\n{word_list}\n输出 JSON，key=单词，value={{'en':'','zh':''}}"
