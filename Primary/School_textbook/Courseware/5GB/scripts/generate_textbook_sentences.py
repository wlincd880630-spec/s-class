...
TEXTBOOK = os.path.join(PROJECT, "教材文字", "5下-外研版三起点", "完整教材.md")
...
NOISE = re.compile(
    r"^(Look,|Listen|Read |Work in|Point|Say|Write|Choose|Act |Module \d|Unit \d|OK!|Activity)",
    re.I,
)
...
def score_sentence(s: str, word: str) -> int:
    ...
    if NOISE.match(s):
        return -100
    if normalize(word).lower() not in normalize(s).lower() and not word_pattern(word).search(s):
        return -100
    wc = len(s.split())
    if wc < 3 or wc > 24:
        return -50
...
def main():
    ...
    text = load_text()
    result, missing = {}, []
    for words in UNITS_5XIA.values():
        for word in words:
    ...
    _all = [w for ws in UNITS_5XIA.values() for w in ws]
