# -*- coding: utf-8 -*-
"""从教材数据生成 Courseware/6GB/assets/data/data.js（六年级下册）"""
import json
import os
import re
import sys
...
from word_units_config import UNITS_6XIA

UNIT_META = {
    "Module 1": {"id": "module1", "title": "快餐点餐", "en": "Fast Food Order"},
    "Module 2": {"id": "module2", "title": "公园天气", "en": "Park Weather"},
    "Module 3": {"id": "module3", "title": "风筝飞走", "en": "Kite Flying Away"},
    "Module 4": {"id": "module4", "title": "气球楼梯", "en": "Balloons & Stairs"},
    "Module 5": {"id": "module5", "title": "演奏狗狗", "en": "Playing Music"},
    "Module 6": {"id": "module6", "title": "太空故事", "en": "Space Story"},
    "Module 7": {"id": "module7", "title": "杨利伟榜样", "en": "Role Model"},
    "Module 8": {"id": "module8", "title": "棒球计划", "en": "Baseball Plans"},
    "Module 9": {"id": "module9", "title": "毕业祝福", "en": "Graduation Wishes"},
    "Module 10": {"id": "module10", "title": "中学告别", "en": "Middle School"},
}
...
def lookup_keys(word: str) -> list:
    keys = [word, normalize_word(word)]
    if " = " in word:
        keys.append(word.split(" = ", 1)[0].strip())
    return keys


def lookup_dict(d: dict, word: str) -> dict:
...
def slug(word: str) -> str:
    return re.sub(r"[^a-z0-9]+", "_", normalize_word(word).lower()).strip("_")


def load_phonemes(phoneme_db: dict, word: str) -> list:
    for key in lookup_keys(word):
        items = phoneme_db.get(key)
        if items:
...
def make_sentences(word: str, textbook: dict, context: dict) -> list:
    tb = lookup_dict(textbook, word)
    ctx = lookup_dict(context, word)
    sents = []
    if tb.get("en"):
...
def main():
    vocab_raw = json.load(open(os.path.join(PROJECT, "3-6年级英语单词表.json"), encoding="utf-8"))["grades"][GRADE]["words"]
    vocab = {x["word"]: clean_meaning(x["meaning"]) for x in vocab_raw}
    for x in vocab_raw:
        vocab.setdefault(normalize_word(x["word"]), clean_meaning(x["meaning"]))
    ...
    units, wid = [], 0
    for unit_name, words in UNITS_6XIA.items():
        ...
        for word in words:
            ...
            unit_words.append({
                "id": f"w{wid}",
                "word": word,
                "chinese": vocab.get(word, vocab.get(normalize_word(word), word)),
    ...
    data = {"book": {"id": "fltrp-g6x", "name": "外研版六年级下册", "grade": 6, "semester": "下"}, "units": units}
    js = "/**\n * 外研版小学英语六年级下册单词数据\n * 自动生成，请勿手动编辑\n */\nconst TEXTBOOK_DATA = "
