# -*- coding: utf-8 -*-
"""从教材数据生成 Courseware/4GA/assets/data/data.js（四年级上册）"""
import json
import os
import re
import sys
...
UNIT_META = {
    "Unit 1": {"id": "unit1", "title": "我爱运动", "en": "I Love Sports"},
    "Unit 2": {"id": "unit2", "title": "在家帮忙", "en": "Helping at Home"},
    "Unit 3": {"id": "unit3", "title": "天气怎么样", "en": "What's the Weather"},
    "Unit 4": {"id": "unit4", "title": "美妙的季节", "en": "Wonderful Seasons"},
    "Unit 5": {"id": "unit5", "title": "出发吧", "en": "Let's Go"},
    "Unit 6": {"id": "unit6", "title": "找到你的路", "en": "Find Your Way"},
}
...
def main():
    ...
    with open(os.path.join(data_dir, "phonemes.json"), encoding="utf-8") as f:
        phoneme_db = json.load(f)

    units = []
    wid = 0
    for unit_name, words in UNITS_4SHANG.items():
        meta = UNIT_META[unit_name]
        unit_words = []
        for word in words:
            wid += 1
            chinese = vocab.get(word, word)
            img = image_path(word, progress)
    ...
    data = {
        "book": {"id": "fltrp-g4s", "name": "外研版四年级上册", "grade": 4, "semester": "上"},
        "units": units,
    }
