...
TEXTBOOK = os.path.join(PROJECT, "教材文字", "4上-外研版三起点-新版", "完整教材.md")
...
NOISE = re.compile(
    ...
    r"Take photos|Pick a|Go back|START|OK!|Yes\.|Sure!|Unit\s*\d|Helping at home|"
    r"[A-Z][a-z]+ [A-Z][a-z]+$|\d+$|▢)",
    re.I,
)
...
def main():
    with open(VOCAB, encoding="utf-8") as f:
        meanings = {x["word"]: x["meaning"] for x in json.load(f)["grades"]["4上"]["words"]}
    text = load_text()
    result = {}
    missing = []
    ...
    _all = [w for ws in UNITS_4SHANG.values() for w in ws]
