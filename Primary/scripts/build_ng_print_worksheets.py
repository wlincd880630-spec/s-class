...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .maze-target img {{ width: 100%; max-height: 52px; object-fit: contain; }}
    .maze-target .zh {{ font-size: 0.72rem; color: #546e7a; margin-top: 2px; }}
    .maze-target .num {{ font-family: "Fredoka", sans-serif; font-size: 0.75rem; color: var(--accent); }}
    .maze-grid {{
      display: grid;
      gap: 2px;
      max-width: 220px;
...
{INLINE_JS}
  ...
  (function() {{
    ...
    function renderConnectSheet(group, idx, total, showAns) {{
      var words = shuffle(group.slice(0, 3));
    ...
    function generate() {{
      ...
      if (optConnect) {{
        var connGroups = shuffle(NgReview.chunkGroups(shuffle(pool), 3));
