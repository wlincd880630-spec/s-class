...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0, viewport-fit=cover">
  <title>纸质练习 / PDF · {label}</title>
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css">
  <link rel="stylesheet" href="assets/css/common.css">
  <link rel="stylesheet" href="../assets/css/print-worksheets.css">
</head>
<body class="print-worksheet-page">
  <div class="container">
    ...
    <div class="pw-toolbar no-print">
      <h1>📄 纸质练习 · 打印 / 导出 PDF</h1>
      <p class="pw-desc">根据所选单元与单词生成可在纸上完成的练习。导出 PDF：点「打印」→ 目标选「另存为 PDF」→ 缩放 <strong>100%</strong>、边距 <strong>无/默认</strong>。</p>
      <div class="control-row"><label>单元：</label><select id="unitSelect"></select></div>
      ...
      <div class="pw-row">
        ...
        <label><input type="checkbox" id="optPickImg" checked /> 看单词选图</label>
        <label><input type="checkbox" id="optMemory" checked /> 翻翻配对卡片</label>
        <label><input type="checkbox" id="optSentence" checked /> 句子排序</label>
        <label><input type="checkbox" id="optAnswers" /> 含答案页（教师用）</label>
      </div>
      <div class="pw-row" style="margin-top:0.75rem;">
    ...
    <div id="printArea">
      <p class="pw-empty no-print" id="placeholder">请选择单元与单词，练习纸将自动生成</p>
    </div>
  </div>
  <script>
    window.PRINT_WORKSHEETS = {{ bookName: {json.dumps(book_name, ensure_ascii=False)} }};
  </script>
...
def patch_index(code, label):
    ...
    if "review-print.html" not in html:
        insert = """        <a class="nav-card game" href="review-print.html" target="_blank">
          ...
          <h3>纸质练习 / PDF</h3>
          <p>迷宫、拼词、连线、选择题、选图、配对卡片、句子排序——彩色练习纸打印或导出 PDF</p>
        </a>
"""
        html = html.replace(
