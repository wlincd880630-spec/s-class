...
(function () {
  ...
  var ctxA = bindTrace("K", "cap");
  var ctxa = bindTrace("k", "small");
  ...
  function renderCircle() {
    ...
    $("btn-check-b").addEventListener("click", function () {
      ...
      $("fb-b").textContent = ok ? "全对！kangaroo、key、king、kite 都是 /k/。" : "再看看：只圈 /k/ 开头的图。";
  ...
  function renderKiteColor() {
    ...
    var picked = {};
    board.innerHTML = (L.kiteColorLetters || []).map(function (ch, i) {
    ...
    board.querySelectorAll(".letter-btn").forEach(function (btn) {
      btn.addEventListener("click", function () {
        ...
        var isK = ch === "K" || ch === "k";
        picked[i] = !picked[i];
        btn.classList.toggle("good", picked[i] && isK);
        btn.classList.toggle("bad", picked[i] && !isK);
        if (!picked[i]) {
          btn.classList.remove("good", "bad");
        }
        if (window.AAAudio && isK) AAAudio.speakLetter();
      });
    });
    $("btn-check-c").addEventListener("click", function () {
      ...
      board.querySelectorAll(".letter-btn").forEach(function (btn) {
        ...
        if (isK && !btn.classList.contains("good")) ok = false;
        if (!isK && on) ok = false;
      });
      $("fb-c").textContent = ok ? "涂对了！只给 K 和 k 涂色。" : "只涂 K 和 k，其它字母不要涂。";
