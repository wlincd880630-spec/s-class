...
(function (global) {
  ...
  function onsetHTML(item) {
    if (!item) return "";
    if (item.id === "Kk" || item.kind === "letter") {
      return '<span class="letter-pair"><span class="letter-cap onset">K</span><span class="letter-small rest">k</span></span>';
    }
    if (item.id === "kicking-kangaroo") {
      return '<span class="onset">k</span>icking <span class="onset">k</span>angaroo';
    }
    if (!item.k) {
  ...
  function staveHTML(item) {
    ...
    if (item.id === "kicking-kangaroo") {
      return (
        ...
        '<span class="letter-small onset">k</span><span class="letter-small rest">icking</span> ' +
        '<span class="letter-small onset">k</span><span class="letter-small rest">angaroo</span>' +
        "</span>"
      );
    }
    return (
  ...
  function bookFoot(page, total) {
    return (
      '<div class="bk-foot">' +
      '<span class="bk-foot-meta">S-Class · The Alphabet · <b>Kk</b></span>' +
  ...
  function foot(page, total, pack) {
    return '<div class="foot"><span>S-Class · Kk · ' + pack + "</span><span>" + page + " / " + total + "</span></div>";
  }
  ...
  function kiteSkyHTML() {
    return (
      '<div class="bk-connect">' +
        '<div class="bk-connect-write">' +
          '<p class="bk-connect-label">Write Kk.</p>' +
          stave(staveHTML({ kind: "letter", id: "Kk" }), "write", "fs-lg", "") +
        "</div>" +
        '<figure class="bk-connect-map"><img src="' + L.kiteSky + '" alt="Listen and write Kk on kites"></figure>' +
      "</div>"
    );
  }

  function kiteColorHTML() {
    return (
      '<div class="bk-j-maze">' +
        '<figure class="bk-j-maze-art"><img src="' + L.kiteColor + '" alt="Color K and k on the kite"></figure>' +
        '<div class="bk-j-maze-write">' +
          stave(staveHTML({ kind: "letter", id: "Kk" }), "write", "fs-lg", "Write Kk") +
        "</div>" +
      "</div>"
    );
  }
  ...
  function buildBook() {
    ...
    pages.push(bookSheet("book-cover",
      ...
      '<p class="bk-display">Kk</p>' +
      '<p class="bk-display-hand">Kk</p>' +
      '<p class="bk-phrase">kicking kangaroo</p>' +
      '<p class="bk-ipa">/k/</p>' +
      '<figure class="bk-hero"><img src="' + L.hero + '" alt="kicking kangaroo"></figure>' +
      '<p class="bk-words">kangaroo  ·  key  ·  king  ·  kite</p>' +
      nameRow() +
      bookFoot(1, total)
    ));
    pages.push(bookSheet("",
      ...
      '<div class="bk-split">' +
        '<figure class="bk-plate">' +
          '<img src="' + L.mascot.img + '" alt="kicking kangaroo">' +
          '<figcaption class="bk-caption">kicking kangaroo</figcaption>' +
        "</figure>" +
        '<div class="bk-letter">' +
          '<div class="bk-faces">' +
            '<div class="bk-face is-print">' +
              '<span class="face-lab">Print</span>' +
              '<span class="pair">Kk</span>' +
            "</div>" +
        ...
        "</div>" +
      "</div>" +
      stave(staveHTML({ kind: "letter", id: "Kk" }), "trace", "fs-lg", "Trace") +
    ...
    pages.push(bookSheet("",
      ...
      '<div class="letter-models">' +
        ...
        modelCard("k", "small") +
      "</div>" +
      stave('<span class="letter-pair"><span class="letter-cap onset">K</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-cap onset">K</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      stave('<span class="letter-pair"><span class="letter-small onset">k</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-small onset">k</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      bookFoot(4, total)
    ));
    pages.push(bookSheet("",
      bookHead("D", "Listen. Then write Kk or cross it out.") +
      bookSection("听音后在风筝上写 Kk 或打叉，然后写 Kk。") +
      kiteSkyHTML() +
      bookFoot(5, total)
    ));
    pages.push(bookSheet("",
      ...
      "</div>" +
      '<figure class="bk-chant-girl"><img src="' + (L.chantCharacter || L.mascot.img) + '" alt=""><p class="bk-chant-bubble">This is a <img src="' + w("key").img + '" alt="key" class="bk-inline-egg"></p></figure>' +
      bookFoot(6, total)
    ));
    pages.push(bookSheet("",
      bookHead("F", "Match") +
    ...
    pages.push(bookSheet("",
      ...
      '<div class="bk-copy-page">' +
        wordCopyBlock(w("king")) +
        wordCopyBlock(w("kite")) +
      "</div>" +
      bookFoot(12, total)
    ));
    pages.push(bookSheet("",
      bookHead("L", "Circle the pictures beginning with the /k/ sound.") +
      '<p class="bk-wb-label">Workbook · Unit 4 Kk</p>' +
      workbookCircleHTML() +
      bookFoot(13, total)
    ));
    pages.push(bookSheet("",
      bookHead("M", "Color K and k. Write Kk.") +
      bookSection("给风筝上的 <b>K</b> 和 <b>k</b> 涂色，说一说，然后写 <b>Kk</b>。") +
      kiteColorHTML() +
      bookFoot(14, total)
    ));
    return pages.join("");
  ...
  function buildGames() {
    ...
    pages.push(sheet("theme-candy",
      header("Paper games · Kk", "六个复习游戏 · 纸上也能玩", "课堂 / 回家") +
      nameRow() +
      '<ul class="howto">' +
        ...
        "<li><b>⑤ 写 Kk 还是打叉</b> 老师读（或放 Track 09），学生写。</li>" +
    ...
    pages.push(sheet("theme-sky",
      header("Game 5 · Kk or X", "听音写 Kk 或打叉", "Track 09") +
      task("5", "老师放 Track 09 或读图名。是 /k/ 就写 Kk，不是就打 X。", "sky") +
      '<div class="six-grid">' +
    ...
    pages.push(sheet("theme-leaf",
      ...
      task("6", "剪下四张图。边唱 chant 边排队：key, kite, king, kangaroo。", "leaf") +
      '<div class="cut-grid">' +
        L.chantOrder.map(function (id, i) {
          var item = w(id);
          return (
  ...
  function buildCards() {
    ...
    pages.push(sheet("theme-poster-a",
      '<div class="letter-poster"><div class="giant">K</div><div class="sub">the letter K · /k/</div>' +
      '<img src="' + L.mascot.img + '" alt="kicking kangaroo"></div>' +
      foot(1, total, "字母卡")
    ));
    pages.push(sheet("theme-poster-s",
      '<div class="letter-poster"><div class="giant">k</div><div class="sub">little k · /k/</div>' +
      '<img src="' + w("kite").img + '" alt="kite"></div>' +
      foot(2, total, "字母卡")
    ));
    pages.push(sheet("theme-poster",
      '<div class="letter-poster"><div class="giant">Kk</div><div class="sub">kicking kangaroo · beginning sound /k/</div>' +
      '<img src="' + L.hero + '" alt="Kk"></div>' +
      foot(3, total, "字母卡")
    ));
    pages.push(sheet("",
      header("Fold cards 1–2", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("kangaroo")) + foldCard(w("key")) + "</div>" +
      foot(4, total, "对折卡")
    ));
    pages.push(sheet("",
      header("Fold cards 3–4", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("king")) + foldCard(w("kite")) + "</div>" +
      foot(5, total, "对折卡")
    ));
    pages.push(sheet("",
    ...
    pages.push(sheet("theme-leaf",
      ...
      '<div class="mini-grid">' +
        ["kangaroo", "key", "king", "kite", "gorilla", "horse", "egg", "dog"].map(function (id) {
          var item = w(id);
    ...
    pages.push(sheet("theme-candy",
      header("Mascot + letter", "kicking kangaroo 与 Kk 小卡", "剪开") +
      '<div class="cut-grid">' +
        '<div class="cut-card"><img src="' + L.mascot.img + '" alt=""><div class="lab">' + onsetHTML(w("kicking-kangaroo")) + "</div></div>" +
        '<div class="cut-card word-only"><div class="lab" style="font-size:64pt;">Kk</div><div class="ipa" style="font-size:28pt;color:#2e7d32;">/k/</div></div>' +
