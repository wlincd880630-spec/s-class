...
(function (global) {
  ...
  function onsetHTML(item) {
    if (!item) return "";
    if (item.id === "Rr" || item.kind === "letter") {
      return '<span class="letter-pair"><span class="letter-cap onset">R</span><span class="letter-small rest">r</span></span>';
    }
    if (item.id === "racing-rabbit") {
      return '<span class="onset">n</span>oisy <span class="onset">n</span>ut';
    }
    if (!item.r) {
  ...
  function staveHTML(item) {
    ...
    if (item.id === "racing-rabbit") {
      return (
        ...
        '<span class="letter-small onset">r</span><span class="letter-small rest">oisy</span> ' +
        '<span class="letter-small onset">r</span><span class="letter-small rest">ut</span>' +
        "</span>"
      );
    }
    return (
  ...
  function bookFoot(page, total) {
    return (
      '<div class="bk-foot">' +
      '<span class="bk-foot-meta">S-Class · The Alphabet · <b>Rr</b></span>' +
  ...
  function foot(page, total, pack) {
    return '<div class="foot"><span>S-Class · Rr · ' + pack + "</span><span>" + page + " / " + total + "</span></div>";
  }
  ...
  function matchWriteHTML() {
    return '<figure class="bk-connect-map"><img src="' + L.matchWrite + '" alt="Match and write"></figure>';
  }

  function wbMatchHTML() {
    return '<div class="bk-wb-match">' +
      (L.wbMatchRows || []).map(function (row) {
        return '<div class="bk-wb-match-row">' +
          '<span class="bk-wb-match-label">Rr</span>' +
  ...
  function robotShapesHTML() {
    return (
      '<div class="bk-j-maze">' +
        '<figure class="bk-j-maze-art"><img src="' + L.robotShapes + '" alt="Robot shapes write"></figure>' +
        '<div class="bk-j-maze-write">' +
          stave(staveHTML({ kind: "letter", id: "Rr" }), "write", "fs-lg", "Write Rr") +
        "</div>" +
      "</div>"
    );
  }

  function wbMatchSixHTML() {
    return '<div class="bk-wb-six">' +
      '<div class="bk-wb-six-center">' + stave(staveHTML({ kind: "letter", id: "Rr" }), "write", "fs-lg", "Write Rr") + "</div>" +
      (L.wbMatchItems || []).map(function (row) {
        var item = w(row.id);
        return '<div class="bk-wb-six-item"><b>' + row.num + '</b><img src="' + item.img + '" alt="' + item.en + '"></div>';
      }).join("") +
      "</div>";
  }

  function rabbitConnectHTML() {
    return (
      '<div class="bk-j-maze">' +
        '<figure class="bk-j-maze-art"><img src="' + L.rabbitConnect + '" alt="Rabbit connect connect"></figure>' +
        '<div class="bk-j-maze-write">' +
          stave(staveHTML({ kind: "letter", id: "Rr" }), "write", "fs-lg", "Write Rr") +
        "</div>" +
      "</div>"
  ...
  function buildBook() {
    ...
    pages.push(bookSheet("book-cover",
      ...
      '<p class="bk-unit">Unit 6</p>' +
      '<p class="bk-display">Rr</p>' +
      '<p class="bk-display-hand">Rr</p>' +
      '<p class="bk-phrase">racing rabbit</p>' +
      '<p class="bk-ipa">/r/</p>' +
      '<figure class="bk-hero"><img src="' + L.hero + '" alt="racing rabbit"></figure>' +
      '<p class="bk-words">rice  ·  robot  ·  rabbit  ·  rose</p>' +
      nameRow() +
      bookFoot(1, total)
    ));
    pages.push(bookSheet("",
      ...
      '<div class="bk-split">' +
        '<figure class="bk-plate">' +
          '<img src="' + L.mascot.img + '" alt="racing rabbit">' +
          '<figcaption class="bk-caption">racing rabbit</figcaption>' +
        "</figure>" +
        '<div class="bk-letter">' +
          '<div class="bk-faces">' +
            '<div class="bk-face is-print">' +
              '<span class="face-lab">Print</span>' +
              '<span class="pair">Rr</span>' +
            "</div>" +
        ...
        "</div>" +
      "</div>" +
      stave(staveHTML({ kind: "letter", id: "Rr" }), "trace", "fs-lg", "Trace") +
    ...
    pages.push(bookSheet("",
      ...
      '<div class="letter-models">' +
        ...
        modelCard("n", "small") +
      "</div>" +
      stave('<span class="letter-pair"><span class="letter-cap onset">R</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-cap onset">R</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      stave('<span class="letter-pair"><span class="letter-small onset">r</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-small onset">r</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      bookFoot(4, total)
    ));
    pages.push(bookSheet("",
      bookHead("D", "Connect. Then write Rr.") +
      bookSection("沿着迷宫连线：rabbit → rice → robot → rose → rug。说一说，然后写 <b>Rr</b>。") +
      rabbitConnectHTML() +
      bookFoot(5, total)
    ));
    pages.push(bookSheet("",
      ...
      "</div>" +
      '<figure class="bk-chant-girl"><img src="' + (L.chantCharacter || L.mascot.img) + '" alt=""><p class="bk-chant-bubble">' + (L.chantBubblePrefix || "That is your") + ' <img src="' + w(L.chantBubbleWord || "rabbit").img + '" alt="rabbit" class="bk-inline-egg"></p></figure>' +
      bookFoot(6, total)
    ));
    pages.push(bookSheet("",
      bookHead("F", "Match") +
    ...
    pages.push(bookSheet("",
      ...
      '<div class="bk-copy-page">' +
        wordCopyBlock(w("rice")) +
        wordCopyBlock(w("robot")) +
      "</div>" +
      bookFoot(11, total)
    ));
    pages.push(bookSheet("",
      ...
      '<div class="bk-copy-page">' +
        wordCopyBlock(w("rabbit")) +
        wordCopyBlock(w("rose")) +
      "</div>" +
      bookFoot(12, total)
    ));
    pages.push(bookSheet("",
      bookHead("L", "Write Rr. Then match.") +
      '<p class="bk-wb-label">Workbook · Unit 6 Rr</p>' +
      wbMatchSixHTML() +
      bookFoot(13, total)
    ));
    pages.push(bookSheet("",
      bookHead("M", "Draw the shapes and write.") +
      bookSection("按形状图找出 R 和 r，说一说，然后写 <b>Rr</b>。") +
      robotShapesHTML() +
      bookFoot(14, total)
    ));
    return pages.join("");
  ...
  function buildGames() {
    ...
    pages.push(sheet("theme-candy",
      header("Paper games · Rr", "六个复习游戏 · 纸上也能玩", "课堂 / 回家") +
      nameRow() +
      '<ul class="howto">' +
        "<li><b>① 开头音小侦探</b> 蜡笔圈出 /r/ 开头的图。</li>" +
        "<li><b>② 听一听点一点</b> 老师读词，学生在图旁写 1–4。</li>" +
        "<li><b>③ 谁是局外人</b> 每一行打叉不是 /r/ 的那张。</li>" +
        "<li><b>④ 图词连线 + 剪卡配对</b> 先连线，再剪开做实物配对。</li>" +
        "<li><b>⑤ 写 Rr 还是打叉</b> 老师读（或放 Track 09），学生写。</li>" +
    ...
    pages.push(sheet("theme-leaf",
      ...
      task("1", "用红蜡笔圈出开头音是 /r/ 的图。不是 /r/ 的请不要圈。") +
    ...
    pages.push(sheet("",
      ...
      task("3", "每一行有三张图。把不是 /r/ 开头的那一张打 X。", "sun") +
    ...
    pages.push(sheet("theme-sky",
      header("Game 5 · Rr or X", "听音写 Rr 或打叉", "Track 09") +
      task("5", "老师放 Track 09 或读图名。是 /r/ 就写 Rr，不是就打 X。", "sky") +
      '<div class="six-grid">' +
    ...
    pages.push(sheet("theme-leaf",
      ...
      task("6", "剪下四张图。边唱 chant 边排队：rabbit, rose, rice, robot。", "leaf") +
      '<div class="cut-grid">' +
        L.chantOrder.map(function (id, i) {
          var item = w(id);
          return (
  ...
  function buildCards() {
    ...
    pages.push(sheet("theme-poster-a",
      '<div class="letter-poster"><div class="giant">N</div><div class="sub">the letter N · /r/</div>' +
      '<img src="' + L.mascot.img + '" alt="racing rabbit"></div>' +
      foot(1, total, "字母卡")
    ));
    pages.push(sheet("theme-poster-s",
      '<div class="letter-poster"><div class="giant">n</div><div class="sub">little n · /r/</div>' +
      '<img src="' + w("rabbit").img + '" alt="rabbit"></div>' +
      foot(2, total, "字母卡")
    ));
    pages.push(sheet("theme-poster",
      '<div class="letter-poster"><div class="giant">Rr</div><div class="sub">racing rabbit · beginning sound /r/</div>' +
      '<img src="' + L.hero + '" alt="Rr"></div>' +
      foot(3, total, "字母卡")
    ));
    pages.push(sheet("",
      header("Fold cards 1–2", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("rice")) + foldCard(w("robot")) + "</div>" +
      foot(4, total, "对折卡")
    ));
    pages.push(sheet("",
      header("Fold cards 3–4", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("rabbit")) + foldCard(w("rose")) + "</div>" +
      foot(5, total, "对折卡")
    ));
    pages.push(sheet("",
    ...
    pages.push(sheet("theme-leaf",
      ...
      '<div class="mini-grid">' +
        ["rice", "robot", "rabbit", "rose", "milk", "net", "egg", "dog"].map(function (id) {
          var item = w(id);
    ...
    pages.push(sheet("",
      ...
      '<div class="cut-grid">' +
        ["lamp", "checklist", "dog", "fan"].map(function (id) {
          var item = w(id);
          return '<div class="cut-card"><img src="' + item.img + '" alt=""><div class="lab">' + item.en + "</div></div>";
        }).join("") +
      "</div>" +
    ...
    pages.push(sheet("theme-candy",
      header("Mascot + letter", "racing rabbit 与 Rr 小卡", "剪开") +
      '<div class="cut-grid">' +
        '<div class="cut-card"><img src="' + L.mascot.img + '" alt=""><div class="lab">' + onsetHTML(w("racing-rabbit")) + "</div></div>" +
        '<div class="cut-card word-only"><div class="lab" style="font-size:64pt;">Rr</div><div class="ipa" style="font-size:28pt;color:#2e7d32;">/r/</div></div>' +
