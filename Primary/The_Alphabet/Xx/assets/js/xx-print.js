...
(function (global) {
  ...
  function onsetHTML(item) {
    if (!item) return "";
    if (item.id === "Xx" || item.kind === "letter") {
      return '<span class="letter-pair"><span class="letter-cap onset">X</span><span class="letter-small rest">x</span></span>';
    }
    if (item.id === "fox-in-box") {
      return '<span class="rest">fox in a bo</span><span class="onset">x</span>';
    }
    if (!item.x) {
      return '<span class="rest">' + item.en + "</span>";
    }
    return '<span class="rest">' + item.body + '</span><span class="onset">' + item.coda + "</span>";
  }
  ...
  function staveHTML(item) {
    ...
    if (item.id === "fox-in-box") {
      return (
        ...
        '<span class="letter-small rest">fox in a bo</span>' +
        '<span class="letter-small onset">x</span>' +
        "</span>"
      );
    }
    if (!item.x) {
    ...
    return (
      '<span class="letter-pair">' +
      '<span class="letter-small rest">' + item.body + "</span>" +
      '<span class="letter-small onset">' + item.coda + "</span>" +
      "</span>"
    );
  }
  ...
  function bookFoot(page, total) {
    return (
      '<div class="bk-foot">' +
      '<span class="bk-foot-meta">S-Class · The Alphabet · <b>Xx</b></span>' +
  ...
  function foot(page, total, pack) {
    return '<div class="foot"><span>S-Class · Xx · ' + pack + "</span><span>" + page + " / " + total + "</span></div>";
  }
  ...
  function matchSilhouetteHTML() {
    var tops = L.matchSilhouette || [];
    var bg = ["#ffe0b2", "#fff9c4", "#c8e6c9", "#f8bbd0"];
    var bottomOrder = ["fox", "wax", "six", "box"];
    return (
      '<div class="bk-xx-match">' +
        '<div class="bk-xx-match-top">' +
          tops.map(function (row, i) {
            var item = w(row.id);
            return '<div class="bk-xx-match-pic"><b>' + (i + 1) + '</b><img src="' + item.img + '" alt="' + item.en + '"></div>';
          }).join("") +
        "</div>" +
        '<div class="bk-xx-match-bottom">' +
          bottomOrder.map(function (id, i) {
            var item = w(id);
            return (
              '<div class="bk-xx-match-sil" style="background:' + bg[i] + '">' +
                '<img src="' + item.img + '" class="is-silhouette" alt="">' +
                stave("", "write", "fs-sm", "") +
              "</div>"
            );
          }).join("") +
  ...
  function workbookCircleHTML() {
    return '<div class="bk-wb-circle">' +
      (L.workbookCircle || []).map(function (id, i) {
        var item = w(id);
        var sample = (L.workbookCircleAnswers || []).indexOf(id) !== -1;
        return '<div class="bk-wb-circle-cell' + (sample ? " is-sample" : "") + '">' +
          '<b>' + (i + 1) + '</b><img src="' + item.img + '" alt=""><span class="bk-ring"></span></div>';
      }).join("") + "</div>";
  }

  function connectMazeHTML() {
    var path = L.connectPath || [];
    return (
      '<div class="bk-xx-maze">' +
        '<div class="bk-xx-maze-start"><img src="' + w(path[0]).img + '" alt=""><span>Start</span></div>' +
        '<div class="bk-xx-maze-path">' +
          path.slice(1, -1).map(function (id) {
            var item = w(id);
            return '<div class="bk-xx-maze-node"><img src="' + item.img + '" alt=""></div>';
          }).join("") +
        "</div>" +
        '<div class="bk-xx-maze-end">' +
          '<img src="' + w(path[path.length - 1]).img + '" alt="">' +
          stave(staveHTML({ kind: "letter", id: "Xx" }), "write", "fs-lg", "Write Xx") +
        "</div>" +
      "</div>"
    );
  }
  ...
  function matchEightHTML() {
    return '<div class="bk-wb-six"><div class="bk-wb-six-center">' + stave(staveHTML({ kind: "letter", id: "Xx" }), "write", "fs-lg", "Write Xx") + '</div>' +
        ...
        (L.wbMatchRows || []).map(function (row, i) {
          ...
          return (
            '<div class="bk-ee-match-row">' +
              '<span class="bk-ee-label"><b>' + (i + 1) + '</b> Xx</span>' +
        ...
        (L.track05Items || []).map(function (row, i) {
          ...
          return (
            '<div class="bk-ee-write-item">' +
              ...
              '<img src="' + item.img + '" alt="' + item.en + '">' +
              '<div class="bk-egg-write"><span>Xx / ✗</span></div>' +
            "</div>"
          );
        }).join("") +
      "</div>"
  ...
  function buildBook() {
    ...
    pages.push(bookSheet("book-cover",
      ...
      '<p class="bk-unit">Unit 8</p>' +
      '<p class="bk-display">Xx</p>' +
      '<p class="bk-display-hand">Xx</p>' +
      '<p class="bk-phrase">fox in a box</p>' +
      '<p class="bk-ipa">/ks/</p>' +
      '<figure class="bk-hero"><img src="' + L.hero + '" alt="fox in a box"></figure>' +
      '<p class="bk-words">fox  ·  box  ·  six  ·  wax</p>' +
      nameRow() +
      bookFoot(1, total)
    ));
    pages.push(bookSheet("",
      ...
      '<div class="bk-split">' +
        '<figure class="bk-plate">' +
          '<img src="' + L.mascot.img + '" alt="fox in a box">' +
          '<figcaption class="bk-caption">fox in a box</figcaption>' +
        "</figure>" +
        '<div class="bk-letter">' +
          '<div class="bk-faces">' +
            '<div class="bk-face is-print">' +
              '<span class="face-lab">Print</span>' +
              '<span class="pair">Xx</span>' +
            "</div>" +
            ...
            "</div>" +
          "</div>" +
          '<span class="ipa">/ks/</span>' +
        "</div>" +
      "</div>" +
      stave(staveHTML({ kind: "letter", id: "Xx" }), "trace", "fs-lg", "Trace") +
    ...
    pages.push(bookSheet("",
      ...
      '<div class="letter-models">' +
        ...
        modelCard("x", "small") +
      "</div>" +
      stave('<span class="letter-pair"><span class="letter-cap onset">X</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-cap onset">X</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      stave('<span class="letter-pair"><span class="letter-small onset">x</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-small onset">x</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      bookFoot(4, total)
    ));
    pages.push(bookSheet("",
      bookHead("D", "Match. Then write Xx.") +
      bookSection("把上图与描影配对，然后在四线格写 <b>Xx</b>。") +
      matchSilhouetteHTML() +
      bookFoot(5, total)
    ));
    pages.push(bookSheet("",
      ...
      "</div>" +
      '<figure class="bk-chant-girl"><img src="' + (L.chantCharacter || L.mascot.img) + '" alt=""><p class="bk-chant-bubble">' + (L.chantBubblePrefix || "Do you see a") + ' <img src="' + w(L.chantBubbleWord || "web").img + '" alt="web" class="bk-inline-egg">?</p></figure>' +
      bookFoot(6, total)
    ));
    pages.push(bookSheet("",
      bookHead("F", "Match") +
    ...
    pages.push(bookSheet("",
      ...
      '<div class="bk-copy-page">' +
        wordCopyBlock(w("fox")) +
        wordCopyBlock(w(L.chantBubbleWord || "box")) +
      "</div>" +
      bookFoot(11, total)
    ));
    pages.push(bookSheet("",
    ...
    pages.push(bookSheet("",
      ...
      '<p class="bk-wb-label">Workbook · Unit 8 Xx</p>' +
      '<figure class="bk-story"><img src="' + w("fox-in-box").img + '" alt="fox in a box"></figure>' +
      stave(staveHTML({ kind: "letter", id: "Xx" }), "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      bookFoot(13, total)
    ));
    pages.push(bookSheet("",
      bookHead("M", "Circle the pictures ending with the x sound.") +
      bookSection('Circle the pictures <b>ending</b> with the <span class="bk-ipa-inline">/ks/</span> sound.') +
      workbookCircleHTML() +
      bookFoot(14, total)
    ));
    pages.push(bookSheet("",
      bookHead("N", "Connect, say, and write Xx.") +
      bookSection("从狐狸出发，沿路径连到盒子，最后写 <b>Xx</b>。") +
      connectMazeHTML() +
      bookFoot(15, total)
    ));
    return pages.join("");
  ...
  function buildGames() {
    ...
    pages.push(sheet("theme-candy",
      header("Paper games · Xx", "六个复习游戏 · 纸上也能玩", "课堂 / 回家") +
      nameRow() +
      '<ul class="howto">' +
        "<li><b>① 词尾音小侦探</b> 蜡笔圈出 /ks/ 开头的图。</li>" +
        "<li><b>② 听一听点一点</b> 老师读词，学生在图旁写 1–4。</li>" +
        "<li><b>③ 谁是局外人</b> 每一行打叉不是 /ks/ 的那张。</li>" +
        "<li><b>④ 图词连线 + 剪卡配对</b> 先连线，再剪开做实物配对。</li>" +
        "<li><b>⑤ 写 Xx 还是打叉</b> 老师读（或放 Track 09），学生写。</li>" +
        "<li><b>⑥ Chant 排队</b> 剪下四张图，按歌曲顺序排好。</li>" +
      "</ul>" +
      foot(1, total, "游戏")
    ));
    pages.push(sheet("theme-leaf",
      header("Game 1 · Beginning sound", "词尾音小侦探", "圈一圈") +
      task("1", "用红蜡笔圈出词尾音是 /ks/ 的图。不是 /ks/ 的请不要圈。") +
    ...
    pages.push(sheet("",
      ...
      task("3", "每一行有三张图。把词尾没有 /ks/的那一张打 X。", "sun") +
    ...
    pages.push(sheet("theme-sky",
      header("Game 5 · Xx or X", "听音写 Xx 或打叉", "Track 09") +
      task("5", "老师放 Track 09 或读图名。是 /ks/ 就写 Xx，不是就打 X。", "sky") +
      '<div class="six-grid">' +
        L.track05Items.map(function (row, i) {
          var item = w(row.id);
          return (
    ...
    pages.push(sheet("theme-leaf",
      ...
      task("6", "剪下四张图。边唱 chant 边排队：box, wax, fox, six。", "leaf") +
      '<div class="cut-grid">' +
        L.chantOrder.map(function (id, i) {
          var item = w(id);
          return (
  ...
  function buildCards() {
    ...
    pages.push(sheet("theme-poster-a",
      '<div class="letter-poster"><div class="giant">X</div><div class="sub">the letter X · /ks/</div>' +
      '<img src="' + L.mascot.img + '" alt="fox in a box"></div>' +
      foot(1, total, "字母卡")
    ));
    pages.push(sheet("theme-poster-s",
      '<div class="letter-poster"><div class="giant">w</div><div class="sub">little w · /ks/</div>' +
      '<img src="' + w("fox").img + '" alt="fox"></div>' +
      foot(2, total, "字母卡")
    ));
    pages.push(sheet("theme-poster",
      '<div class="letter-poster"><div class="giant">Xx</div><div class="sub">fox in a box · ending sound /ks/</div>' +
      '<img src="' + L.hero + '" alt="Xx"></div>' +
      foot(3, total, "字母卡")
    ));
    pages.push(sheet("",
      header("Fold cards 1–2", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("fox")) + foldCard(w("fox")) + "</div>" +
      foot(4, total, "对折卡")
    ));
    pages.push(sheet("",
      header("Fold cards 3–4", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("box")) + foldCard(w("six")) + "</div>" +
      foot(5, total, "对折卡")
    ));
    pages.push(sheet("",
    ...
    pages.push(sheet("theme-leaf",
      ...
      '<div class="mini-grid">' +
        ["fox", "box", "six", "wax", "watch", "kite", "web", "wolf"].map(function (id) {
          var item = w(id);
    ...
    pages.push(sheet("",
      ...
      '<div class="cut-grid">' +
        ["tent", "bear", "cat", "dog"].map(function (id) {
          var item = w(id);
          return '<div class="cut-card"><img src="' + item.img + '" alt=""><div class="lab">' + item.en + "</div></div>";
        }).join("") +
      "</div>" +
    ...
    pages.push(sheet("theme-candy",
      header("Mascot + letter", "fox in a box 与 Xx 小卡", "剪开") +
      '<div class="cut-grid">' +
        '<div class="cut-card"><img src="' + L.mascot.img + '" alt=""><div class="lab">' + onsetHTML(w("fox-in-box")) + "</div></div>" +
        '<div class="cut-card word-only"><div class="lab" style="font-size:64pt;">Xx</div><div class="ipa" style="font-size:28pt;color:#2e7d32;">/ks/</div></div>' +
