...
(function (global) {
  ...
  function onsetHTML(item) {
    if (!item) return "";
    if (item.id === "Mm" || item.kind === "letter") {
      return '<span class="letter-pair"><span class="letter-cap onset">M</span><span class="letter-small rest">m</span></span>';
    }
    if (item.id === "merry-monkey") {
      return '<span class="onset">m</span>erry <span class="onset">m</span>onkey';
    }
    if (!item.m) {
  ...
  function staveHTML(item) {
    ...
    if (item.id === "merry-monkey") {
      return (
        ...
        '<span class="letter-small onset">m</span><span class="letter-small rest">erry</span> ' +
        '<span class="letter-small onset">m</span><span class="letter-small rest">onkey</span>' +
        "</span>"
      );
    }
    return (
  ...
  function bookFoot(page, total) {
    return (
      '<div class="bk-foot">' +
      '<span class="bk-foot-meta">S-Class · The Alphabet · <b>Mm</b></span>' +
  ...
  function foot(page, total, pack) {
    return '<div class="foot"><span>S-Class · Mm · ' + pack + "</span><span>" + page + " / " + total + "</span></div>";
  }
  ...
  function monkeyMatchHTML() {
    return (
      '<div class="bk-connect">' +
        '<div class="bk-connect-write">' +
          '<p class="bk-connect-label">Write Mm.</p>' +
          stave(staveHTML({ kind: "letter", id: "Mm" }), "write", "fs-lg", "") +
        "</div>" +
        '<figure class="bk-connect-map"><img src="' + L.monkeyMatch + '" alt="Listen and write Mm then match"></figure>' +
      "</div>"
    );
  }

  function moneyColorHTML() {
    return (
      '<div class="bk-j-maze">' +
        '<figure class="bk-j-maze-art"><img src="' + L.moneyColor + '" alt="Color M and m on the money"></figure>' +
        '<div class="bk-j-maze-write">' +
          stave(staveHTML({ kind: "letter", id: "Mm" }), "write", "fs-lg", "Write Mm") +
        "</div>" +
      "</div>"
    );
  }
  ...
  function buildBook() {
    ...
    pages.push(bookSheet("book-cover",
      ...
      '<p class="bk-unit">Unit 5</p>' +
      '<p class="bk-display">Mm</p>' +
      '<p class="bk-display-hand">Mm</p>' +
      '<p class="bk-phrase">merry monkey</p>' +
      '<p class="bk-ipa">/m/</p>' +
      '<figure class="bk-hero"><img src="' + L.hero + '" alt="merry monkey"></figure>' +
      '<p class="bk-words">monkey  ·  milk  ·  money  ·  mouse</p>' +
      nameRow() +
      bookFoot(1, total)
    ));
    pages.push(bookSheet("",
      ...
      '<div class="bk-split">' +
        '<figure class="bk-plate">' +
          '<img src="' + L.mascot.img + '" alt="merry monkey">' +
          '<figcaption class="bk-caption">merry monkey</figcaption>' +
        "</figure>" +
        '<div class="bk-letter">' +
          '<div class="bk-faces">' +
            '<div class="bk-face is-print">' +
              '<span class="face-lab">Print</span>' +
              '<span class="pair">Mm</span>' +
            "</div>" +
        ...
        "</div>" +
      "</div>" +
      stave(staveHTML({ kind: "letter", id: "Mm" }), "trace", "fs-lg", "Trace") +
    ...
    pages.push(bookSheet("",
      ...
      '<div class="letter-models">' +
        ...
        modelCard("m", "small") +
      "</div>" +
      stave('<span class="letter-pair"><span class="letter-cap onset">M</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-cap onset">M</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      stave('<span class="letter-pair"><span class="letter-small onset">m</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-small onset">m</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      bookFoot(4, total)
    ));
    ...
    pages.push(bookSheet("",
      ...
      "</div>" +
      '<figure class="bk-chant-girl"><img src="' + (L.chantCharacter || L.mascot.img) + '" alt=""><p class="bk-chant-bubble">I like the <img src="' + w(L.chantBubbleWord || "mouse").img + '" alt="mouse" class="bk-inline-egg"></p></figure>' +
      bookFoot(6, total)
    ));
    pages.push(bookSheet("",
      bookHead("F", "Match") +
    ...
    pages.push(bookSheet("",
      ...
      '<div class="bk-copy-page">' +
        wordCopyBlock(w("monkey")) +
        wordCopyBlock(w(L.chantBubbleWord || "mouse")) +
      "</div>" +
      bookFoot(11, total)
    ));
    pages.push(bookSheet("",
      ...
      '<div class="bk-copy-page">' +
        wordCopyBlock(w("milk")) +
        wordCopyBlock(w("money")) +
      "</div>" +
      bookFoot(12, total)
    ));
    pages.push(bookSheet("",
      bookHead("L", "Circle the pictures beginning with the /m/ sound.") +
      '<p class="bk-wb-label">Workbook · Unit 5 Mm</p>' +
      workbookCircleHTML() +
      bookFoot(13, total)
    ));
    pages.push(bookSheet("",
      bookHead("M", "Color M and m. Write Mm.") +
      bookSection("给钞票上的 <b>M</b> 和 <b>m</b> 涂色，说一说，然后写 <b>Mm</b>。") +
      moneyColorHTML() +
      bookFoot(14, total)
    ));
    return pages.join("");
  ...
  function buildGames() {
    ...
    pages.push(sheet("theme-candy",
      header("Paper games · Mm", "六个复习游戏 · 纸上也能玩", "课堂 / 回家") +
      nameRow() +
      '<ul class="howto">' +
        "<li><b>① 开头音小侦探</b> 蜡笔圈出 /m/ 开头的图。</li>" +
        "<li><b>② 听一听点一点</b> 老师读词，学生在图旁写 1–4。</li>" +
        "<li><b>③ 谁是局外人</b> 每一行打叉不是 /m/ 的那张。</li>" +
        "<li><b>④ 图词连线 + 剪卡配对</b> 先连线，再剪开做实物配对。</li>" +
        "<li><b>⑤ 写 Mm 还是打叉</b> 老师读（或放 Track 09），学生写。</li>" +
    ...
    pages.push(sheet("theme-leaf",
      ...
      task("1", "用红蜡笔圈出开头音是 /m/ 的图。不是 /m/ 的请不要圈。") +
    ...
    pages.push(sheet("",
      ...
      task("3", "每一行有三张图。把不是 /m/ 开头的那一张打 X。", "sun") +
    ...
    pages.push(sheet("theme-sky",
      header("Game 5 · Mm or X", "听音写 Mm 或打叉", "Track 09") +
      task("5", "老师放 Track 09 或读图名。是 /m/ 就写 Mm，不是就打 X。", "sky") +
      '<div class="six-grid">' +
    ...
    pages.push(sheet("theme-leaf",
      ...
      task("6", "剪下四张图。边唱 chant 边排队：mouse, money, milk, monkey。", "leaf") +
      '<div class="cut-grid">' +
        L.chantOrder.map(function (id, i) {
          var item = w(id);
          return (
  ...
  function buildCards() {
    ...
    pages.push(sheet("theme-poster-a",
      '<div class="letter-poster"><div class="giant">M</div><div class="sub">the letter M · /m/</div>' +
      '<img src="' + L.mascot.img + '" alt="merry monkey"></div>' +
      foot(1, total, "字母卡")
    ));
    pages.push(sheet("theme-poster-s",
      '<div class="letter-poster"><div class="giant">m</div><div class="sub">little m · /m/</div>' +
      '<img src="' + w("mouse").img + '" alt="mouse"></div>' +
      foot(2, total, "字母卡")
    ));
    pages.push(sheet("theme-poster",
      '<div class="letter-poster"><div class="giant">Mm</div><div class="sub">merry monkey · beginning sound /m/</div>' +
      '<img src="' + L.hero + '" alt="Mm"></div>' +
      foot(3, total, "字母卡")
    ));
    pages.push(sheet("",
      header("Fold cards 1–2", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("monkey")) + foldCard(w("mouse")) + "</div>" +
      foot(4, total, "对折卡")
    ));
    pages.push(sheet("",
      header("Fold cards 3–4", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("milk")) + foldCard(w("money")) + "</div>" +
      foot(5, total, "对折卡")
    ));
    pages.push(sheet("",
    ...
    pages.push(sheet("theme-leaf",
      ...
      '<div class="mini-grid">' +
        ["monkey", "mouse", "milk", "money", "lamp", "lion", "egg", "dog"].map(function (id) {
          var item = w(id);
    ...
    pages.push(sheet("",
      ...
      '<div class="cut-grid">' +
        ["insect", "kangaroo", "dog", "fan"].map(function (id) {
          var item = w(id);
          return '<div class="cut-card"><img src="' + item.img + '" alt=""><div class="lab">' + item.en + "</div></div>";
        }).join("") +
      "</div>" +
    ...
    pages.push(sheet("theme-candy",
      header("Mascot + letter", "merry monkey 与 Mm 小卡", "剪开") +
      '<div class="cut-grid">' +
        '<div class="cut-card"><img src="' + L.mascot.img + '" alt=""><div class="lab">' + onsetHTML(w("merry-monkey")) + "</div></div>" +
        '<div class="cut-card word-only"><div class="lab" style="font-size:64pt;">Mm</div><div class="ipa" style="font-size:28pt;color:#2e7d32;">/m/</div></div>' +
