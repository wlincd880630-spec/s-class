...
(function (global) {
  ...
  function onsetHTML(item) {
    if (!item) return "";
    if (item.id === "Bb" || item.kind === "letter") {
      return '<span class="letter-pair"><span class="letter-cap onset">B</span><span class="letter-small rest">b</span></span>';
    }
    if (item.id === "big-bear") {
      return '<span class="onset">b</span>ig <span class="onset">b</span>ear';
    }
    if (!item.b) {
  ...
  function staveHTML(item) {
    ...
    if (item.id === "big-bear") {
      return (
        ...
        '<span class="letter-small onset">b</span><span class="letter-small rest">ig</span> ' +
        '<span class="letter-small onset">b</span><span class="letter-small rest">ear</span>' +
        "</span>"
      );
    }
    return (
  ...
  function bookFoot(page, total) {
    return (
      '<div class="bk-foot">' +
      '<span class="bk-foot-meta">S-Class · The Alphabet · <b>Bb</b></span>' +
  ...
  function foot(page, total, pack) {
    return '<div class="foot"><span>S-Class · Bb · ' + pack + "</span><span>" + page + " / " + total + "</span></div>";
  }
  ...
  function buildBook() {
    ...
    pages.push(bookSheet("book-cover",
      ...
      '<p class="bk-display">Bb</p>' +
      '<p class="bk-display-hand">Bb</p>' +
      '<p class="bk-phrase">big bear</p>' +
      '<p class="bk-ipa">/b/</p>' +
      '<figure class="bk-hero"><img src="' + L.mascot.img + '" alt="big bear"></figure>' +
      '<p class="bk-words">bear  ·  bird  ·  bed  ·  banana</p>' +
      nameRow() +
      bookFoot(1, total)
    ));
    pages.push(bookSheet("",
      ...
      '<div class="bk-split">' +
        '<figure class="bk-plate">' +
          '<img src="' + L.mascot.img + '" alt="big bear">' +
          '<figcaption class="bk-caption">big bear</figcaption>' +
        "</figure>" +
        '<div class="bk-letter">' +
          '<div class="bk-faces">' +
            '<div class="bk-face is-print">' +
              '<span class="face-lab">Print</span>' +
              '<span class="pair">Bb</span>' +
            "</div>" +
        ...
        "</div>" +
      "</div>" +
      stave(staveHTML({ kind: "letter", id: "Bb" }), "trace", "fs-lg", "Trace") +
    ...
    pages.push(bookSheet("",
      ...
      '<div class="letter-models">' +
        ...
        modelCard("b", "small") +
      "</div>" +
      stave('<span class="letter-pair"><span class="letter-cap onset">B</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-cap onset">B</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      stave('<span class="letter-pair"><span class="letter-small onset">b</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-small onset">b</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      bookFoot(4, total)
    ));
    ...
    pages.push(bookSheet("",
      ...
      '<div class="bk-copy-page">' +
        wordCopyBlock(w("bear")) +
        wordCopyBlock(w("bird")) +
      "</div>" +
      bookFoot(11, total)
    ));
    pages.push(bookSheet("",
      ...
      '<div class="bk-copy-page">' +
        wordCopyBlock(w("bed")) +
        wordCopyBlock(w("banana")) +
      "</div>" +
      bookFoot(12, total)
    ));
    pages.push(bookSheet("",
      bookHead("L", "Circle and colour") +
      bookSection('Circle the <span class="bk-ipa-inline">/b/</span> pictures') +
      '<div class="bk-picnic">' +
      ...
      "</div>" +
      bookSection('Colour A and <span class="bk-keep-case">b</span>') +
  ...
  function buildGames() {
    ...
    pages.push(sheet("theme-candy",
      header("Paper games · Bb", "六个复习游戏 · 纸上也能玩", "课堂 / 回家") +
      nameRow() +
      '<ul class="howto">' +
        "<li><b>① 开头音小侦探</b> 蜡笔圈出 /b/ 开头的图。</li>" +
    ...
    pages.push(sheet("theme-leaf",
      ...
      task("1", "用红蜡笔圈出开头音是 /b/ 的图。不是 /b/ 的请不要圈。") +
    ...
    pages.push(sheet("",
      ...
      task("3", "每一行有三张图。把不是 /b/ 开头的那一张打 X。", "sun") +
  ...
  function buildCards() {
    ...
    pages.push(sheet("theme-poster-a",
      '<div class="letter-poster"><div class="giant">B</div><div class="sub">the letter B · /b/</div>' +
      '<img src="' + L.mascot.img + '" alt="big bear"></div>' +
      foot(1, total, "字母卡")
    ));
    pages.push(sheet("theme-poster-s",
      '<div class="letter-poster"><div class="giant">b</div><div class="sub">little b · /b/</div>' +
      '<img src="' + w("bird").img + '" alt="bird"></div>' +
      foot(2, total, "字母卡")
    ));
    pages.push(sheet("theme-poster",
      '<div class="letter-poster"><div class="giant">Bb</div><div class="sub">big bear · beginning sound /b/</div>' +
      '<img src="' + L.hero + '" alt="Bb"></div>' +
      foot(3, total, "字母卡")
    ));
    pages.push(sheet("",
      header("Fold cards 1–2", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("bear")) + foldCard(w("bird")) + "</div>" +
      foot(4, total, "对折卡")
    ));
    pages.push(sheet("",
      header("Fold cards 3–4", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("bed")) + foldCard(w("banana")) + "</div>" +
      foot(5, total, "对折卡")
    ));
    pages.push(sheet("",
    ...
    pages.push(sheet("theme-leaf",
      ...
      '<div class="mini-grid">' +
        ["bear", "bird", "bed", "banana", "apple", "ant", "ball", "cup"].map(function (id) {
          var item = w(id);
    ...
    pages.push(sheet("theme-candy",
      header("Mascot + letter", "big bear 与 Bb 小卡", "剪开") +
      '<div class="cut-grid">' +
        '<div class="cut-card"><img src="' + L.mascot.img + '" alt=""><div class="lab">' + onsetHTML(w("big-bear")) + "</div></div>" +
        '<div class="cut-card word-only"><div class="lab" style="font-size:64pt;">Bb</div><div class="ipa" style="font-size:28pt;color:#2e7d32;">/b/</div></div>' +
        '<div class="cut-card word-only"><div class="lab" style="font-size:80pt;color:#c62828;">B</div></div>' +
        '<div class="cut-card word-only"><div class="lab" style="font-size:80pt;color:#2e7d32;">b</div></div>' +
      "</div>" +
      foot(11, total, "字母小卡")
    ));
    return pages.join("");
