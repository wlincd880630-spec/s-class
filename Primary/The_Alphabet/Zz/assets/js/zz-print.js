...
(function (global) {
  ...
  function onsetHTML(item) {
    if (!item) return "";
    if (item.id === "Zz" || item.kind === "letter") {
      return '<span class="letter-pair"><span class="letter-cap onset">Z</span><span class="letter-small rest">z</span></span>';
    }
    if (item.id === "zany-zebra") {
      return '<span class="onset">z</span>any <span class="onset">z</span>ebra';
    }
    if (!item.z) {
  ...
  function staveHTML(item) {
    ...
    if (item.id === "zany-zebra") {
      return (
        ...
        '<span class="letter-small onset">z</span><span class="letter-small rest">any</span> ' +
        '<span class="letter-small onset">z</span><span class="letter-small rest">ebra</span>' +
        "</span>"
      );
    }
    if (!item.z) {
  ...
  function bookFoot(page, total) {
    return (
      '<div class="bk-foot">' +
      '<span class="bk-foot-meta">S-Class · The Alphabet · <b>Zz</b></span>' +
  ...
  function foot(page, total, pack) {
    return '<div class="foot"><span>S-Class · Zz · ' + pack + "</span><span>" + page + " / " + total + "</span></div>";
  }
  ...
  function listenWriteMatchHTML() {
    ...
    return (
      '<div class="bk-zz-listen">' +
        '<div class="bk-zz-write-row">' +
          items.map(function (row, i) {
            return '<div class="bk-zz-write-cell"><b>' + (i + 1) + "</b>" + stave("", "write", "fs-sm", "") + "</div>";
          }).join("") +
        "</div>" +
        '<div class="bk-zz-pic-row">' +
          items.map(function (row) {
            var item = w(row.id);
            return '<div class="bk-zz-pic-cell"><img src="' + item.img + '" alt="' + item.en + '"><span class="bk-ring"></span></div>';
          }).join("") +
        "</div>" +
      "</div>"
    );
  ...
  function matchEightHTML() {
    return '<div class="bk-wb-six"><div class="bk-wb-six-center">' + stave(staveHTML({ kind: "letter", id: "Zz" }), "write", "fs-lg", "Write Zz") + '</div>' +
        ...
        (L.wbMatchRows || []).map(function (row, i) {
          ...
          return (
            '<div class="bk-ee-match-row">' +
              '<span class="bk-ee-label"><b>' + (i + 1) + '</b> Zz</span>' +
        ...
        (L.track05Items || []).map(function (row, i) {
          ...
          return (
            '<div class="bk-ee-write-item">' +
              ...
              '<img src="' + item.img + '" alt="' + item.en + '">' +
              '<div class="bk-egg-write"><span>Zz / ✗</span></div>' +
            "</div>"
          );
        }).join("") +
      "</div>"
  ...
  function buildBook() {
    ...
    pages.push(bookSheet("book-cover",
      ...
      '<p class="bk-display">Zz</p>' +
      '<p class="bk-display-hand">Zz</p>' +
      '<p class="bk-phrase">zany zebra</p>' +
      '<p class="bk-ipa">/z/</p>' +
      '<figure class="bk-hero"><img src="' + L.hero + '" alt="zany zebra"></figure>' +
      '<p class="bk-words">zebra  ·  zero  ·  zip  ·  zoo</p>' +
      nameRow() +
      bookFoot(1, total)
    ));
    pages.push(bookSheet("",
      ...
      '<div class="bk-split">' +
        '<figure class="bk-plate">' +
          '<img src="' + L.mascot.img + '" alt="zany zebra">' +
          '<figcaption class="bk-caption">zany zebra</figcaption>' +
        "</figure>" +
        '<div class="bk-letter">' +
          '<div class="bk-faces">' +
            '<div class="bk-face is-print">' +
              '<span class="face-lab">Print</span>' +
              '<span class="pair">Zz</span>' +
            "</div>" +
        ...
        "</div>" +
      "</div>" +
      stave(staveHTML({ kind: "letter", id: "Zz" }), "trace", "fs-lg", "Trace") +
    ...
    pages.push(bookSheet("",
      ...
      '<div class="letter-models">' +
        ...
        modelCard("z", "small") +
      "</div>" +
      stave('<span class="letter-pair"><span class="letter-cap onset">Z</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-cap onset">Z</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      stave('<span class="letter-pair"><span class="letter-small onset">z</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-small onset">z</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      bookFoot(4, total)
    ));
    pages.push(bookSheet("",
      ...
      bookSection("听音写在格子里，再连线配对图片。（Disc 2 · Track 59）") +
    ...
    pages.push(bookSheet("",
      ...
      "</div>" +
      '<figure class="bk-chant-girl"><img src="' + (L.chantCharacter || L.mascot.img) + '" alt=""><p class="bk-chant-bubble">' + (L.chantBubblePrefix || "I don't have a") + ' <img src="' + w(L.chantBubbleWord || "zip").img + '" alt="zebra" class="bk-inline-egg">.</p></figure>' +
      bookFoot(6, total)
    ));
    pages.push(bookSheet("",
      bookHead("F", "Match") +
    ...
    pages.push(bookSheet("",
      ...
      '<div class="bk-copy-page">' +
        wordCopyBlock(w("zebra")) +
        wordCopyBlock(w(L.chantBubbleWord || "zip")) +
      "</div>" +
      bookFoot(11, total)
    ));
    pages.push(bookSheet("",
    ...
    pages.push(bookSheet("",
      ...
      '<p class="bk-wb-label">Workbook · Unit 8 Zz</p>' +
      '<figure class="bk-story"><img src="' + w("zebra").img + '" alt="zebra"></figure>' +
      stave(staveHTML({ kind: "letter", id: "Zz" }), "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      bookFoot(13, total)
    ));
    pages.push(bookSheet("",
      bookHead("M", "Which ones begin with z? Match and write.") +
      bookSection('Which ones begin with the <span class="bk-ipa-inline">/z/</span> sound? Match and write <b>Zz</b>.') +
      workbookMatchHTML() +
      bookFoot(14, total)
    ));
    pages.push(bookSheet("",
      bookHead("N", "Say. Then write Zz or cross it out.") +
      bookSection("说一说，然后在四线格写 <b>Zz</b> 或打叉。") +
      workbookWriteHTML() +
      bookFoot(15, total)
    ));
    return pages.join("");
  ...
  function buildGames() {
    ...
    pages.push(sheet("theme-candy",
      header("Paper games · Zz", "六个复习游戏 · 纸上也能玩", "课堂 / 回家") +
      nameRow() +
      '<ul class="howto">' +
        "<li><b>① 开头音小侦探</b> 蜡笔圈出 /z/ 开头的图。</li>" +
        "<li><b>② 听一听点一点</b> 老师读词，学生在图旁写 1–4。</li>" +
        "<li><b>③ 谁是局外人</b> 每一行打叉不是 /z/ 的那张。</li>" +
        "<li><b>④ 图词连线 + 剪卡配对</b> 先连线，再剪开做实物配对。</li>" +
        "<li><b>⑤ 写 Zz 还是打叉</b> 老师读（或放 Track 09），学生写。</li>" +
    ...
    pages.push(sheet("theme-leaf",
      ...
      task("1", "用红蜡笔圈出开头音是 /z/ 的图。不是 /z/ 的请不要圈。") +
    ...
    pages.push(sheet("",
      ...
      task("3", "每一行有三张图。把不是 /z/ 开头的那一张打 X。", "sun") +
    ...
    pages.push(sheet("theme-sky",
      header("Game 5 · Zz or X", "听音写 Zz 或打叉", "Track 09") +
      task("5", "老师放 Track 09 或读图名。是 /z/ 就写 Zz，不是就打 X。", "sky") +
      '<div class="six-grid">' +
        L.track05Items.map(function (row, i) {
          var item = w(row.id);
          return (
    ...
    pages.push(sheet("theme-leaf",
      ...
      task("6", "剪下四张图。边唱 chant 边排队：zip, zebra, zero, zoo。", "leaf") +
      '<div class="cut-grid">' +
        L.chantOrder.map(function (id, i) {
          var item = w(id);
          return (
  ...
  function buildCards() {
    ...
    pages.push(sheet("theme-poster-a",
      '<div class="letter-poster"><div class="giant">Z</div><div class="sub">the letter Z · /z/</div>' +
      '<img src="' + L.mascot.img + '" alt="zany zebra"></div>' +
      foot(1, total, "字母卡")
    ));
    pages.push(sheet("theme-poster-s",
      '<div class="letter-poster"><div class="giant">z</div><div class="sub">little z · /z/</div>' +
      '<img src="' + w("zebra").img + '" alt="zebra"></div>' +
      foot(2, total, "字母卡")
    ));
    pages.push(sheet("theme-poster",
      '<div class="letter-poster"><div class="giant">Zz</div><div class="sub">zany zebra · beginning sound /z/</div>' +
      '<img src="' + L.hero + '" alt="Zz"></div>' +
      foot(3, total, "字母卡")
    ));
    pages.push(sheet("",
      header("Fold cards 1–2", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("zebra")) + foldCard(w("zero")) + "</div>" +
      foot(4, total, "对折卡")
    ));
    pages.push(sheet("",
      header("Fold cards 3–4", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("zero")) + foldCard(w("zip")) + "</div>" +
      foot(5, total, "对折卡")
    ));
    pages.push(sheet("",
    ...
    pages.push(sheet("theme-leaf",
      ...
      '<div class="mini-grid">' +
        ["sick", "six", "pizza", "book", "sixteen", "wet", "sun", "van"].map(function (id) {
          var item = w(id);
    ...
    pages.push(sheet("",
      ...
      '<div class="cut-grid">' +
        ["sun", "bear", "cat", "apple"].map(function (id) {
          var item = w(id);
          return '<div class="cut-card"><img src="' + item.img + '" alt=""><div class="lab">' + item.en + "</div></div>";
        }).join("") +
      "</div>" +
    ...
    pages.push(sheet("theme-candy",
      header("Mascot + letter", "zany zebra 与 Zz 小卡", "剪开") +
      '<div class="cut-grid">' +
        '<div class="cut-card"><img src="' + L.mascot.img + '" alt=""><div class="lab">' + onsetHTML(w("zany-zebra")) + "</div></div>" +
        '<div class="cut-card word-only"><div class="lab" style="font-size:64pt;">Zz</div><div class="ipa" style="font-size:28pt;color:#2e7d32;">/z/</div></div>' +
