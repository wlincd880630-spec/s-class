/**
 * Vv 互动练习册：描红、配对、写 Vv 或打叉。
 */
(function () {
  "use strict";
  var L = window.VV_LESSON;
  var step = 1;
  var titles = ["", "练习册 A · 描红", "练习册 B · 配对", "练习册 C · Vv / ✗"];
  ...
  var ctxV = bindTrace("V", "cap");
  var ctxv = bindTrace("v", "small");
  ...
  function renderMatch() {
    ...
    $("btn-check-b").addEventListener("click", function () {
      ...
      $("fb-b").textContent = ok ? "全对！vet、vest、van、violin 都是 /v/ 开头。" : "再看看：每行只选 v 开头的图。";
  ...
  function renderCross() {
    ...
    grid.innerHTML = (L.track05Items || []).map(function (row, i) {
      ...
      return (
        '<div class="wb-cross-item" data-i="' + i + '">' +
          ...
          '<div class="en">' + item.en + "</div>" +
          '<div class="btn-row">' +
            '<button type="button" class="btn btn-leaf" data-pick="vv" data-i="' + i + '">Vv</button>' +
    ...
    $("btn-check-c").addEventListener("click", function () {
      ...
      $("fb-c").textContent = ok ? "全对！van、violin、vest 写 Vv；jet 打叉。" : "再听一遍：/v/ 开头写 Vv，否则打叉。";
  ...
  $("btn-clear-trace").addEventListener("click", function () {
    if (ctxV) ctxV.clearRect(0, 0, ctxV.canvas.width, ctxV.canvas.height);
    if (ctxv) ctxv.clearRect(0, 0, ctxv.canvas.width, ctxv.canvas.height);
  });
