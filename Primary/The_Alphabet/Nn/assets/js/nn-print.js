...
(function (global) {
  ...
  function onsetHTML(item) {
    if (!item) return "";
    if (item.id === "Nn" || item.kind === "letter") {
      return '<span class="letter-pair"><span class="letter-cap onset">N</span><span class="letter-small rest">n</span></span>';
    }
    if (item.id === "noisy-nut") {
  ...
  function staveHTML(item) {
    ...
    if (item.id === "noisy-nut") {
      return (
        ...
        '<span class="letter-small onset">n</span><span class="letter-small rest">oisy</span> ' +
        '<span class="letter-small onset">n</span><span class="letter-small rest">ut</span>' +
        "</span>"
      );
    }
    return (
  ...
  function bookFoot(page, total) {
    return (
      '<div class="bk-foot">' +
      '<span class="bk-foot-meta">S-Class · The Alphabet · <b>Nn</b></span>' +
  ...
  function foot(page, total, pack) {
    return '<div class="foot"><span>S-Class · Nn · ' + pack + "</span><span>" + page + " / " + total + "</span></div>";
  }
    ...
    return '<div class="bk-wb-match">' +
      (L.wbMatchRows || []).map(function (row) {
        return '<div class="bk-wb-match-row">' +
          '<span class="bk-wb-match-label">Nn</span>' +
  ...
  function nestMazeHTML() {
    return (
      '<div class="bk-j-maze">' +
        '<figure class="bk-j-maze-art"><img src="' + L.nestMaze + '" alt="Nest maze connect"></figure>' +
        '<div class="bk-j-maze-write">' +
          stave(staveHTML({ kind: "letter", id: "Nn" }), "write", "fs-lg", "Write Nn") +
        "</div>" +
      "</div>"
    );
  }
  ...
  function buildBook() {
    ...
    pages.push(bookSheet("book-cover",
      ...
      '<p class="bk-display">Nn</p>' +
      '<p class="bk-display-hand">Nn</p>' +
      '<p class="bk-phrase">noisy nut</p>' +
      '<p class="bk-ipa">/n/</p>' +
      '<figure class="bk-hero"><img src="' + L.hero + '" alt="noisy nut"></figure>' +
      '<p class="bk-words">nut  ·  net  ·  nest  ·  nose</p>' +
      nameRow() +
      bookFoot(1, total)
    ));
    pages.push(bookSheet("",
      ...
      '<div class="bk-split">' +
        '<figure class="bk-plate">' +
          '<img src="' + L.mascot.img + '" alt="noisy nut">' +
          '<figcaption class="bk-caption">noisy nut</figcaption>' +
        "</figure>" +
        '<div class="bk-letter">' +
          '<div class="bk-faces">' +
            '<div class="bk-face is-print">' +
              '<span class="face-lab">Print</span>' +
              '<span class="pair">Nn</span>' +
            "</div>" +
        ...
        "</div>" +
      "</div>" +
      stave(staveHTML({ kind: "letter", id: "Nn" }), "trace", "fs-lg", "Trace") +
    ...
    pages.push(bookSheet("",
      ...
      '<div class="letter-models">' +
        ...
        modelCard("n", "small") +
      "</div>" +
      stave('<span class="letter-pair"><span class="letter-cap onset">N</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-cap onset">N</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      stave('<span class="letter-pair"><span class="letter-small onset">n</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-small onset">n</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      bookFoot(4, total)
    ));
    ...
    pages.push(bookSheet("",
      ...
      "</div>" +
      '<figure class="bk-chant-girl"><img src="' + (L.chantCharacter || L.mascot.img) + '" alt=""><p class="bk-chant-bubble">' + (L.chantBubblePrefix || "That is a") + ' <img src="' + w(L.chantBubbleWord || "nest").img + '" alt="nest" class="bk-inline-egg"></p></figure>' +
      bookFoot(6, total)
    ));
    pages.push(bookSheet("",
      bookHead("F", "Match") +
    ...
    pages.push(bookSheet("",
      ...
      '<div class="bk-copy-page">' +
        wordCopyBlock(w("nest")) +
        wordCopyBlock(w("nose")) +
      "</div>" +
      bookFoot(12, total)
    ));
    pages.push(bookSheet("",
      bookHead("L", "Which ones begin with the n sound? Match and write.") +
      '<p class="bk-wb-label">Workbook · Unit 5 Nn</p>' +
      wbMatchHTML() +
      bookFoot(13, total)
    ));
    pages.push(bookSheet("",
      bookHead("M", "Connect, say, and write Nn.") +
      bookSection("沿着迷宫连线：nut → nose → net → nest。说一说，然后写 <b>Nn</b>。") +
      nestMazeHTML() +
      bookFoot(14, total)
    ));
    return pages.join("");
  ...
  function buildGames() {
    ...
    pages.push(sheet("theme-candy",
      header("Paper games · Nn", "六个复习游戏 · 纸上也能玩", "课堂 / 回家") +
      nameRow() +
      '<ul class="howto">' +
        "<li><b>① 开头音小侦探</b> 蜡笔圈出 /n/ 开头的图。</li>" +
        "<li><b>② 听一听点一点</b> 老师读词，学生在图旁写 1–4。</li>" +
        "<li><b>③ 谁是局外人</b> 每一行打叉不是 /n/ 的那张。</li>" +
        "<li><b>④ 图词连线 + 剪卡配对</b> 先连线，再剪开做实物配对。</li>" +
        "<li><b>⑤ 写 Nn 还是打叉</b> 老师读（或放 Track 09），学生写。</li>" +
    ...
    pages.push(sheet("theme-leaf",
      ...
      task("1", "用红蜡笔圈出开头音是 /n/ 的图。不是 /n/ 的请不要圈。") +
    ...
    pages.push(sheet("",
      ...
      task("3", "每一行有三张图。把不是 /n/ 开头的那一张打 X。", "sun") +
    ...
    pages.push(sheet("theme-sky",
      header("Game 5 · Nn or X", "听音写 Nn 或打叉", "Track 09") +
      task("5", "老师放 Track 09 或读图名。是 /n/ 就写 Nn，不是就打 X。", "sky") +
      '<div class="six-grid">' +
    ...
    pages.push(sheet("theme-leaf",
      ...
      task("6", "剪下四张图。边唱 chant 边排队：nest, nose, nut, net。", "leaf") +
      '<div class="cut-grid">' +
        L.chantOrder.map(function (id, i) {
          var item = w(id);
          return (
  ...
  function buildCards() {
    ...
    pages.push(sheet("theme-poster-a",
      '<div class="letter-poster"><div class="giant">N</div><div class="sub">the letter N · /n/</div>' +
      '<img src="' + L.mascot.img + '" alt="noisy nut"></div>' +
      foot(1, total, "字母卡")
    ));
    pages.push(sheet("theme-poster-s",
      '<div class="letter-poster"><div class="giant">n</div><div class="sub">little n · /n/</div>' +
      '<img src="' + w("nest").img + '" alt="nest"></div>' +
      foot(2, total, "字母卡")
    ));
    pages.push(sheet("theme-poster",
      '<div class="letter-poster"><div class="giant">Nn</div><div class="sub">noisy nut · beginning sound /n/</div>' +
      '<img src="' + L.hero + '" alt="Nn"></div>' +
      foot(3, total, "字母卡")
    ));
    pages.push(sheet("",
      header("Fold cards 1–2", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("nut")) + foldCard(w("net")) + "</div>" +
      foot(4, total, "对折卡")
    ));
    pages.push(sheet("",
      header("Fold cards 3–4", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("nest")) + foldCard(w("nose")) + "</div>" +
      foot(5, total, "对折卡")
    ));
    pages.push(sheet("",
    ...
    pages.push(sheet("theme-leaf",
      ...
      '<div class="mini-grid">' +
        ["nut", "net", "nest", "nose", "milk", "mouse", "egg", "dog"].map(function (id) {
          var item = w(id);
    ...
    pages.push(sheet("",
      ...
      '<div class="cut-grid">' +
        ["insect", "milk", "dog", "fan"].map(function (id) {
          var item = w(id);
          return '<div class="cut-card"><img src="' + item.img + '" alt=""><div class="lab">' + item.en + "</div></div>";
        }).join("") +
      "</div>" +
    ...
    pages.push(sheet("theme-candy",
      header("Mascot + letter", "noisy nut 与 Nn 小卡", "剪开") +
      '<div class="cut-grid">' +
        '<div class="cut-card"><img src="' + L.mascot.img + '" alt=""><div class="lab">' + onsetHTML(w("noisy-nut")) + "</div></div>" +
        '<div class="cut-card word-only"><div class="lab" style="font-size:64pt;">Nn</div><div class="ipa" style="font-size:28pt;color:#2e7d32;">/n/</div></div>' +
