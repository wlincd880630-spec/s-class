  ...
  function startG1() {
    ...
    function check() {
      ...
      document.querySelectorAll("#g1-grid .choice").forEach(function (c) {
        ...
        if (w.c && on) c.classList.add("good");
        else if (w.c && !on) { c.classList.add("bad"); miss++; }
        else if (!w.c && on) { c.classList.add("bad"); extra++; }
      });
      if (miss === 0 && extra === 0) {
        addScore(4);
        fb("全对", true);
    ...
    function deal() {
      ...
      $("g3-grid").onclick = function (e) {
        ...
        if (!w.c) {
          ...
          round++;
          later(function () {
            if (round >= total) showResult("完成", "能把 b 开头分开了。", startG3);
            else deal();
          }, 700);
        } else {
          c.classList.add("bad");
  ...
  function startG5() {
    ...
    function ask() {
      ...
      $("play-board").innerHTML =
        '<div class="play-stage">' +
          ...
          '<div class="big-choice" id="g5-choice">' +
            '<button type="button" class="btn btn-leaf" data-cc="1">Cc</button>' +
            '<button type="button" class="btn btn-apple" data-cc="0">✗</button>' +
          "</div>" +
        "</div>";
      $("play-actions").innerHTML = "";
      function play() {
      ...
      $("g5-choice").onclick = function (e) {
        ...
        if (!b) return;
        var chooseCc = b.getAttribute("data-cc") === "1";
        if (chooseCc !== it.writeCc) {
          fb("再听开头音", false);
          play();
          return;
        }
  ...
  function startG6() {
    ...
    function check() {
      ...
      if (ok) {
        addScore(4);
        fb("顺序对了", true);
        showResult("完成", "car, cat, computer, cup", startG6);
      } else {
        fb("再听，按歌曲顺序排", false);
      }
    }
