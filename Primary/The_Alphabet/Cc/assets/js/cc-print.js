...
(function (global) {
  ...
  function onsetHTML(item) {
    if (!item) return "";
    if (item.id === "Cc" || item.kind === "letter") {
      return '<span class="letter-pair"><span class="letter-cap onset">C</span><span class="letter-small rest">c</span></span>';
    }
    if (item.id === "cool-cat") {
      return '<span class="onset">c</span>ool <span class="onset">c</span>at';
    }
    if (!item.c) {
  ...
  function staveHTML(item) {
    ...
    if (item.id === "cool-cat") {
      return (
        ...
        '<span class="letter-small onset">c</span><span class="letter-small rest">ool</span> ' +
        '<span class="letter-small onset">c</span><span class="letter-small rest">at</span>' +
        "</span>"
      );
    }
    return (
  ...
  function bookFoot(page, total) {
    return (
      '<div class="bk-foot">' +
      '<span class="bk-foot-meta">S-Class · The Alphabet · <b>Cc</b></span>' +
  ...
  function foot(page, total, pack) {
    return '<div class="foot"><span>S-Class · Cc · ' + pack + "</span><span>" + page + " / " + total + "</span></div>";
  }
  ...
  function connectHubHTML() {
    var hub = L.connectHub || { pics: [] };
    var cells = (hub.pics || []).map(function (slot) {
      var item = w(slot.id);
      var cls = "bk-hub-pic is-" + (slot.pos || "top") + (item.c ? " is-target" : "");
      return '<div class="' + cls + '"><img src="' + item.img + '" alt=""></div>';
    }).join("");
    return (
      '<div class="bk-connect-hub">' +
        cells +
        '<div class="bk-hub-center"><span class="bk-hub-cap">C</span><span class="bk-hub-sm">c</span></div>' +
      "</div>"
    );
  }
  ...
  function buildBook() {
    ...
    pages.push(bookSheet("book-cover",
      ...
      '<p class="bk-display">Cc</p>' +
      '<p class="bk-display-hand">Cc</p>' +
      '<p class="bk-phrase">cool cat</p>' +
      '<p class="bk-ipa">/k/</p>' +
      '<figure class="bk-hero"><img src="' + L.hero + '" alt="cool cat"></figure>' +
      '<p class="bk-words">cat  ·  cup  ·  computer  ·  car</p>' +
      nameRow() +
      bookFoot(1, total)
    ));
    pages.push(bookSheet("",
      ...
      '<div class="bk-split">' +
        '<figure class="bk-plate">' +
          '<img src="' + L.mascot.img + '" alt="cool cat">' +
          '<figcaption class="bk-caption">cool cat</figcaption>' +
        "</figure>" +
        '<div class="bk-letter">' +
          '<div class="bk-faces">' +
            '<div class="bk-face is-print">' +
              '<span class="face-lab">Print</span>' +
              '<span class="pair">Cc</span>' +
            "</div>" +
        ...
        "</div>" +
      "</div>" +
      stave(staveHTML({ kind: "letter", id: "Cc" }), "trace", "fs-lg", "Trace") +
    ...
    pages.push(bookSheet("",
      ...
      '<div class="letter-models">' +
        ...
        modelCard("c", "small") +
      "</div>" +
      stave('<span class="letter-pair"><span class="letter-cap onset">C</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-cap onset">C</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      stave('<span class="letter-pair"><span class="letter-small onset">c</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-small onset">c</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      bookFoot(4, total)
    ));
    pages.push(bookSheet("",
      bookHead("D", "Colour") +
      bookSection('Colour the parts with <span class="bk-keep-case">c</span>.') +
      '<figure class="bk-car-color"><img src="' + L.carColor + '" alt="Colour the c parts on the car"></figure>' +
      '<p class="bk-car-hint">Look at the letters. Only colour the parts marked <b>c</b>.</p>' +
      bookFoot(5, total)
    ));
    pages.push(bookSheet("",
      bookHead("E", "Connect") +
      bookSection('Draw lines from <span class="bk-keep-case">C</span> to the /k/ pictures.') +
      connectHubHTML() +
      bookFoot(6, total)
    ));
    pages.push(bookSheet("",
      bookHead("F", "Listen and chant") +
      '<div class="vocab-4 bk-vocab">' +
    ...
    pages.push(bookSheet("",
      bookHead("L", "Trace and write") +
      '<div class="bk-copy-page">' +
        wordCopyBlock(w("computer")) +
        wordCopyBlock(w("car")) +
      "</div>" +
      bookFoot(13, total)
    ));
    pages.push(bookSheet("",
      bookHead("M", "Circle and colour") +
      bookSection('Circle the <span class="bk-ipa-inline">/k/</span> pictures') +
      '<div class="bk-picnic">' +
      ...
      "</div>" +
      bookSection('Colour C and <span class="bk-keep-case">c</span>') +
  ...
  function buildGames() {
    ...
    pages.push(sheet("theme-candy",
      header("Paper games · Cc", "六个复习游戏 · 纸上也能玩", "课堂 / 回家") +
      nameRow() +
      '<ul class="howto">' +
        "<li><b>① 开头音小侦探</b> 蜡笔圈出 /k/ 开头的图。</li>" +
        "<li><b>② 听一听点一点</b> 老师读词，学生在图旁写 1–4。</li>" +
        "<li><b>③ 谁是局外人</b> 每一行打叉不是 /k/ 的那张。</li>" +
        "<li><b>④ 图词连线 + 剪卡配对</b> 先连线，再剪开做实物配对。</li>" +
        "<li><b>⑤ 写 Cc 还是打叉</b> 老师读词或让学生点屏幕听英音慢读，学生写。</li>" +
    ...
    pages.push(sheet("theme-leaf",
      ...
      task("1", "用红蜡笔圈出开头音是 /k/ 的图。不是 /k/ 的请不要圈。") +
    ...
    pages.push(sheet("",
      ...
      task("3", "每一行有三张图。把不是 /k/ 开头的那一张打 X。", "sun") +
    ...
    pages.push(sheet("theme-sky",
      header("Game 5 · Cc or X", "听音写 Cc 或打叉", "英音慢读") +
      task("5", "老师读图名，或让学生点屏幕听英音慢读。是 /k/ 就写 Cc，不是就打 X。", "sky") +
      '<div class="six-grid">' +
    ...
    pages.push(sheet("theme-leaf",
      ...
      task("6", "剪下四张图。边唱 chant 边排队：car, cat, computer, cup。", "leaf") +
      '<div class="cut-grid">' +
        L.chantOrder.map(function (id, i) {
          var item = w(id);
          return (
  ...
  function buildCards() {
    ...
    pages.push(sheet("theme-poster-a",
      '<div class="letter-poster"><div class="giant">C</div><div class="sub">the letter C · /k/</div>' +
      '<img src="' + L.mascot.img + '" alt="cool cat"></div>' +
      foot(1, total, "字母卡")
    ));
    pages.push(sheet("theme-poster-s",
      '<div class="letter-poster"><div class="giant">c</div><div class="sub">little c · /k/</div>' +
      '<img src="' + w("cat").img + '" alt="cat"></div>' +
      foot(2, total, "字母卡")
    ));
    pages.push(sheet("theme-poster",
      '<div class="letter-poster"><div class="giant">Cc</div><div class="sub">cool cat · beginning sound /k/</div>' +
      '<img src="' + L.hero + '" alt="Cc"></div>' +
      foot(3, total, "字母卡")
    ));
    pages.push(sheet("",
      header("Fold cards 1–2", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("cat")) + foldCard(w("cup")) + "</div>" +
      foot(4, total, "对折卡")
    ));
    pages.push(sheet("",
      header("Fold cards 3–4", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("computer")) + foldCard(w("car")) + "</div>" +
      foot(5, total, "对折卡")
    ));
    pages.push(sheet("",
    ...
    pages.push(sheet("theme-leaf",
      ...
      '<div class="mini-grid">' +
        ["cat", "cup", "computer", "car", "apple", "ant", "ball", "bear"].map(function (id) {
          var item = w(id);
    ...
    pages.push(sheet("",
      ...
      '<div class="cut-grid">' +
        ["apple", "ant", "dog", "cat"].map(function (id) {
          var item = w(id);
          return '<div class="cut-card"><img src="' + item.img + '" alt=""><div class="lab">' + item.en + "</div></div>";
        }).join("") +
      "</div>" +
    ...
    pages.push(sheet("theme-candy",
      header("Mascot + letter", "cool cat 与 Cc 小卡", "剪开") +
      '<div class="cut-grid">' +
        '<div class="cut-card"><img src="' + L.mascot.img + '" alt=""><div class="lab">' + onsetHTML(w("cool-cat")) + "</div></div>" +
        '<div class="cut-card word-only"><div class="lab" style="font-size:64pt;">Cc</div><div class="ipa" style="font-size:28pt;color:#2e7d32;">/k/</div></div>' +
        '<div class="cut-card word-only"><div class="lab" style="font-size:80pt;color:#c62828;">C</div></div>' +
        '<div class="cut-card word-only"><div class="lab" style="font-size:80pt;color:#2e7d32;">c</div></div>' +
      "</div>" +
      foot(11, total, "字母小卡")
    ));
    return pages.join("");
