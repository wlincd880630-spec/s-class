...
(function (global) {
  ...
  function onsetHTML(item) {
    if (!item) return "";
    if (item.id === "Ii" || item.kind === "letter") {
      return '<span class="letter-pair"><span class="letter-cap onset">I</span><span class="letter-small rest">i</span></span>';
    }
    if (item.id === "interesting-insect") {
      return '<span class="onset">i</span>nteresting <span class="onset">i</span>nsect';
    }
    if (!item.i) {
  ...
  function staveHTML(item) {
    ...
    if (item.id === "interesting-insect") {
      return (
        ...
        '<span class="letter-small onset">i</span><span class="letter-small rest">nteresting</span> ' +
        '<span class="letter-small onset">i</span><span class="letter-small rest">nsect</span>' +
        "</span>"
      );
    }
    return (
  ...
  function bookFoot(page, total) {
    return (
      '<div class="bk-foot">' +
      '<span class="bk-foot-meta">S-Class · The Alphabet · <b>Ii</b></span>' +
  ...
  function foot(page, total, pack) {
    return '<div class="foot"><span>S-Class · Ii · ' + pack + "</span><span>" + page + " / " + total + "</span></div>";
  }
    ...
    return (
      '<div class="bk-igloo-color">' +
        ...
        '<div class="bk-igloo-write">' +
          stave(staveHTML({ kind: "letter", id: "Ii" }), "write", "fs-lg", "Write Ii") +
        "</div>" +
      "</div>"
    );
  }
  ...
  function buildBook() {
    ...
    pages.push(bookSheet("book-cover",
      ...
      '<p class="bk-display">Ii</p>' +
      '<p class="bk-display-hand">Ii</p>' +
      '<p class="bk-phrase">interesting insect</p>' +
      '<p class="bk-ipa">/ɪ/</p>' +
      '<figure class="bk-hero"><img src="' + L.hero + '" alt="interesting insect"></figure>' +
      '<p class="bk-words">insect  ·  ink  ·  igloo  ·  iguana</p>' +
      nameRow() +
      bookFoot(1, total)
    ));
    pages.push(bookSheet("",
      ...
      '<div class="bk-split">' +
        '<figure class="bk-plate">' +
          '<img src="' + L.mascot.img + '" alt="interesting insect">' +
          '<figcaption class="bk-caption">interesting insect</figcaption>' +
        "</figure>" +
        '<div class="bk-letter">' +
          '<div class="bk-faces">' +
            '<div class="bk-face is-print">' +
              '<span class="face-lab">Print</span>' +
              '<span class="pair">Ii</span>' +
            "</div>" +
        ...
        "</div>" +
      "</div>" +
      stave(staveHTML({ kind: "letter", id: "Ii" }), "trace", "fs-lg", "Trace") +
    ...
    pages.push(bookSheet("",
      ...
      '<div class="letter-models">' +
        ...
        modelCard("i", "small") +
      "</div>" +
      stave('<span class="letter-pair"><span class="letter-cap onset">I</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-cap onset">I</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      stave('<span class="letter-pair"><span class="letter-small onset">i</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-small onset">i</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      bookFoot(4, total)
    ));
    pages.push(bookSheet("",
      bookHead("D", "Which ones begin with the i sound? Trace and color.") +
      bookSection("找出以 /ɪ/ 开头的图，描边并涂色：ink、igloo、iguana、insect。") +
      traceColorHTML() +
      bookFoot(5, total)
    ));
    pages.push(bookSheet("",
      ...
      "</div>" +
      '<figure class="bk-chant-girl"><img src="' + (L.chantCharacter || L.mascot.img) + '" alt=""><p class="bk-chant-bubble">I have an <img src="' + w("igloo").img + '" alt="igloo" class="bk-inline-egg"></p></figure>' +
      bookFoot(6, total)
    ));
    pages.push(bookSheet("",
      bookHead("F", "Match") +
    ...
    pages.push(bookSheet("",
      ...
      '<div class="bk-copy-page">' +
        wordCopyBlock(w("igloo")) +
        wordCopyBlock(w("iguana")) +
      "</div>" +
      bookFoot(12, total)
    ));
    pages.push(bookSheet("",
      bookHead("L", "Circle the pictures beginning with the /ɪ/ sound.") +
      '<p class="bk-wb-label">Workbook · Unit 3 Ii</p>' +
      workbookCircleHTML() +
      bookFoot(13, total)
    ));
    pages.push(bookSheet("",
      bookHead("M", "Color I and i. Write Ii.") +
      bookSection('Color the parts with <span class="bk-keep-case">I</span> and <span class="bk-keep-case">i</span>. Then write <b>Ii</b>.') +
      iglooColorHTML() +
      bookFoot(14, total)
    ));
    return pages.join("");
  ...
  function buildGames() {
    ...
    pages.push(sheet("theme-candy",
      header("Paper games · Ii", "六个复习游戏 · 纸上也能玩", "课堂 / 回家") +
      nameRow() +
      '<ul class="howto">' +
        "<li><b>① 开头音小侦探</b> 蜡笔圈出 /ɪ/ 开头的图。</li>" +
        "<li><b>② 听一听点一点</b> 老师读词，学生在图旁写 1–4。</li>" +
        "<li><b>③ 谁是局外人</b> 每一行打叉不是 /ɪ/ 的那张。</li>" +
        "<li><b>④ 图词连线 + 剪卡配对</b> 先连线，再剪开做实物配对。</li>" +
        "<li><b>⑤ 写 Ii 还是打叉</b> 老师读（或放 Track 09），学生写。</li>" +
    ...
    pages.push(sheet("theme-leaf",
      ...
      task("1", "用红蜡笔圈出开头音是 /ɪ/ 的图。不是 /ɪ/ 的请不要圈。") +
    ...
    pages.push(sheet("",
      ...
      task("3", "每一行有三张图。把不是 /ɪ/ 开头的那一张打 X。", "sun") +
    ...
    pages.push(sheet("theme-sky",
      header("Game 5 · Ii or X", "听音写 Ii 或打叉", "Track 09") +
      task("5", "老师放 Track 09 或读图名。是 /ɪ/ 就写 Ii，不是就打 X。", "sky") +
      '<div class="six-grid">' +
    ...
    pages.push(sheet("theme-leaf",
      ...
      task("6", "剪下四张图。边唱 chant 边排队：igloo, insect, iguana, ink。", "leaf") +
      '<div class="cut-grid">' +
        L.chantOrder.map(function (id, i) {
          var item = w(id);
          return (
  ...
  function buildCards() {
    ...
    pages.push(sheet("theme-poster-a",
      '<div class="letter-poster"><div class="giant">I</div><div class="sub">the letter I · /ɪ/</div>' +
      '<img src="' + L.mascot.img + '" alt="interesting insect"></div>' +
      foot(1, total, "字母卡")
    ));
    pages.push(sheet("theme-poster-s",
      '<div class="letter-poster"><div class="giant">i</div><div class="sub">little i · /ɪ/</div>' +
      '<img src="' + w("insect").img + '" alt="insect"></div>' +
      foot(2, total, "字母卡")
    ));
    pages.push(sheet("theme-poster",
      '<div class="letter-poster"><div class="giant">Ii</div><div class="sub">interesting insect · beginning sound /ɪ/</div>' +
      '<img src="' + L.hero + '" alt="Ii"></div>' +
      foot(3, total, "字母卡")
    ));
    pages.push(sheet("",
      header("Fold cards 1–2", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("insect")) + foldCard(w("ink")) + "</div>" +
      foot(4, total, "对折卡")
    ));
    pages.push(sheet("",
      header("Fold cards 3–4", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("igloo")) + foldCard(w("iguana")) + "</div>" +
      foot(5, total, "对折卡")
    ));
    pages.push(sheet("",
    ...
    pages.push(sheet("theme-leaf",
      ...
      '<div class="mini-grid">' +
        ["insect", "ink", "igloo", "iguana", "computer", "horse", "car", "house"].map(function (id) {
          var item = w(id);
    ...
    pages.push(sheet("theme-candy",
      header("Mascot + letter", "interesting insect 与 Ii 小卡", "剪开") +
      '<div class="cut-grid">' +
        '<div class="cut-card"><img src="' + L.mascot.img + '" alt=""><div class="lab">' + onsetHTML(w("interesting-insect")) + "</div></div>" +
        '<div class="cut-card word-only"><div class="lab" style="font-size:64pt;">Ii</div><div class="ipa" style="font-size:28pt;color:#2e7d32;">/ɪ/</div></div>' +
