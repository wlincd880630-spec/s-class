...
(function (global) {
  ...
  function onsetHTML(item) {
    if (!item) return "";
    if (item.id === "Uu" || item.kind === "letter") {
  ...
  function staveHTML(item) {
    ...
    if (item.id === "unhappy-umbrella") {
      return (
        ...
        '<span class="letter-small onset">u</span><span class="letter-small rest">nhappy</span> ' +
        '<span class="letter-small onset">u</span><span class="letter-small rest">onkey</span>' +
        "</span>"
      );
    }
    return (
  ...
  function bookFoot(page, total) {
    return (
      '<div class="bk-foot">' +
      '<span class="bk-foot-meta">S-Class · The Alphabet · <b>Uu</b></span>' +
  ...
  function foot(page, total, pack) {
    return '<div class="foot"><span>S-Class · Uu · ' + pack + "</span><span>" + page + " / " + total + "</span></div>";
  }
  ...
  function listenMatchHTML() {
    return '<div class="bk-wb-six">' + (L.listenMatch||[]).map(function(row){
      var item=w(row.id); return '<div class="bk-wb-six-item"><b>'+row.num+'</b><div class="bk-lw-write">'+stave("","write","fs-sm","")+'</div><img src="'+item.img+'" alt=""></div>';}).join('')+'</div>';
  }
  ...
  function buildBook() {
    ...
    pages.push(bookSheet("book-cover",
      ...
      '<p class="bk-display">Uu</p>' +
      '<p class="bk-display-hand">Uu</p>' +
      '<p class="bk-phrase">unhappy umbrella</p>' +
      '<p class="bk-ipa">/ʌ/</p>' +
      '<figure class="bk-hero"><img src="' + L.hero + '" alt="unhappy umbrella"></figure>' +
      '<p class="bk-words">umbrella  ·  up  ·  uncle  ·  umpire</p>' +
      nameRow() +
      bookFoot(1, total)
    ));
    pages.push(bookSheet("",
      ...
      '<div class="bk-split">' +
        '<figure class="bk-plate">' +
          '<img src="' + L.mascot.img + '" alt="unhappy umbrella">' +
          '<figcaption class="bk-caption">unhappy umbrella</figcaption>' +
        "</figure>" +
        '<div class="bk-letter">' +
          '<div class="bk-faces">' +
            '<div class="bk-face is-print">' +
              '<span class="face-lab">Print</span>' +
              '<span class="pair">Uu</span>' +
            "</div>" +
        ...
        "</div>" +
      "</div>" +
      stave(staveHTML({ kind: "letter", id: "Uu" }), "trace", "fs-lg", "Trace") +
    ...
    pages.push(bookSheet("",
      ...
      '<div class="letter-models">' +
        ...
        modelCard("u", "small") +
      "</div>" +
      stave('<span class="letter-pair"><span class="letter-cap onset">U</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-cap onset">U</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      stave('<span class="letter-pair"><span class="letter-small onset">u</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-small onset">u</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      bookFoot(4, total)
    ));
    ...
    pages.push(bookSheet("",
      ...
      "</div>" +
      '<figure class="bk-chant-girl"><img src="' + (L.chantCharacter || L.mascot.img) + '" alt=""><p class="bk-chant-bubble">This is an <img src="' + w(L.chantBubbleWord || "umpire").img + '" alt="umbrella" class="bk-inline-egg"></p></figure>' +
      bookFoot(6, total)
    ));
    pages.push(bookSheet("",
      bookHead("F", "Match") +
    ...
    pages.push(bookSheet("",
      ...
      '<div class="bk-copy-page">' +
        wordCopyBlock(w("umbrella")) +
        wordCopyBlock(w(L.chantBubbleWord || "umpire")) +
      "</div>" +
      bookFoot(11, total)
    ));
    pages.push(bookSheet("",
      ...
      '<div class="bk-copy-page">' +
        wordCopyBlock(w("up")) +
        wordCopyBlock(w("uncle")) +
      "</div>" +
      bookFoot(12, total)
    ));
    pages.push(bookSheet("",
      bookHead("L", "Connect, say, and write Uu.") +
      '<p class="bk-wb-label">Workbook · Unit 7 Uu</p>' +
      '<figure class="bk-story"><img src="' + L.umbrellaMaze + '" alt="umbrella maze"></figure>' +
      stave(staveHTML({ kind: "letter", id: "Uu" }), "trace", "fs-lg", "Write Uu") +
      bookFoot(13, total)
    ));
    pages.push(bookSheet("",
      bookHead("M", "Circle the pictures beginning with the /ʌ/ sound.") +
      '<p class="bk-wb-label">Workbook · Unit 7 Uu</p>' +
      workbookCircleHTML() +
      bookFoot(14, total)
    ));
    pages.push(bookSheet("",
      bookHead("N", "Say. Then write Uu or cross it out.") +
      bookSection("说一说，然后在四线格写 <b>Uu</b> 或打叉。") +
      workbookCrossHTML() +
      bookFoot(15, total)
    ));
    return pages.join("");
  ...
  function buildGames() {
    ...
    pages.push(sheet("theme-candy",
      header("Paper games · Uu", "六个复习游戏 · 纸上也能玩", "课堂 / 回家") +
      nameRow() +
      '<ul class="howto">' +
        "<li><b>① 开头音小侦探</b> 蜡笔圈出 /ʌ/ 开头的图。</li>" +
        "<li><b>② 听一听点一点</b> 老师读词，学生在图旁写 1–4。</li>" +
        "<li><b>③ 谁是局外人</b> 每一行打叉不是 /ʌ/ 的那张。</li>" +
        "<li><b>④ 图词连线 + 剪卡配对</b> 先连线，再剪开做实物配对。</li>" +
        "<li><b>⑤ 写 Uu 还是打叉</b> 老师读（或放 Track 09），学生写。</li>" +
    ...
    pages.push(sheet("theme-leaf",
      ...
      task("1", "用红蜡笔圈出开头音是 /ʌ/ 的图。不是 /ʌ/ 的请不要圈。") +
    ...
    pages.push(sheet("",
      ...
      task("3", "每一行有三张图。把不是 /ʌ/ 开头的那一张打 X。", "sun") +
    ...
    pages.push(sheet("theme-sky",
      header("Game 5 · Uu or X", "听音写 Uu 或打叉", "Track 09") +
      task("5", "老师放 Track 09 或读图名。是 /ʌ/ 就写 Uu，不是就打 X。", "sky") +
      '<div class="six-grid">' +
  ...
  function buildCards() {
    ...
    pages.push(sheet("theme-poster",
      '<div class="letter-poster"><div class="giant">Uu</div><div class="sub">unhappy umbrella · beginning sound /ʌ/</div>' +
      '<img src="' + L.hero + '" alt="Uu"></div>' +
      foot(3, total, "字母卡")
    ));
    pages.push(sheet("",
      header("Fold cards 1–2", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("umbrella")) + foldCard(w("umbrella")) + "</div>" +
      foot(4, total, "对折卡")
    ));
    pages.push(sheet("",
      header("Fold cards 3–4", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("up")) + foldCard(w("uncle")) + "</div>" +
      foot(5, total, "对折卡")
    ));
    pages.push(sheet("",
    ...
    pages.push(sheet("theme-leaf",
      ...
      '<div class="mini-grid">' +
        ["umbrella", "umbrella", "up", "uncle", "tent", "soap", "egg", "dog"].map(function (id) {
          var item = w(id);
    ...
    pages.push(sheet("",
      ...
      '<div class="cut-grid">' +
        ["under", "bear", "cat", "fan"].map(function (id) {
          var item = w(id);
          return '<div class="cut-card"><img src="' + item.img + '" alt=""><div class="lab">' + item.en + "</div></div>";
        }).join("") +
      "</div>" +
    ...
    pages.push(sheet("theme-candy",
      header("Mascot + letter", "unhappy umbrella 与 Uu 小卡", "剪开") +
      '<div class="cut-grid">' +
        '<div class="cut-card"><img src="' + L.mascot.img + '" alt=""><div class="lab">' + onsetHTML(w("unhappy-umbrella")) + "</div></div>" +
        '<div class="cut-card word-only"><div class="lab" style="font-size:64pt;">Uu</div><div class="ipa" style="font-size:28pt;color:#2e7d32;">/ʌ/</div></div>' +
