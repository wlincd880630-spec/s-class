/**
 * WXYZ 复习 · 教具工坊：教材 PDF（Ww Xx Yy Zz）
 */
(function (global) {
  "use strict";
  ...
  function bookFoot(page, total) {
    return '<div class="bk-foot"><span class="bk-foot-meta">S-Class · Unit 8 Review · <b>Ww Xx Yy Zz</b></span>' +
    ...
    return '<div class="bk-wb-ghi-grid">' +
      L.wbBeginningSound.map(function (row, i) {
        ...
        return '<div class="bk-wb-ghi-cell' + (row.sample ? " is-sample" : "") + '">' +
          ...
          '<div class="bk-wb-ghi-opts"><span>w</span><span>x</span><span>y</span><span>z</span></div></div>';
      }).join("") + "</div>";
  }
  ...
  function wbLetterPickHTML() {
    return '<div class="bk-wb-letter-pick">' +
      (L.wbLetterPick || []).map(function (row, i) {
        var item = w(row.pic);
        return '<div class="bk-wb-letter-cell' + (row.sample ? " is-sample" : "") + '">' +
          '<b>' + (i + 1) + '</b><img src="' + item.img + '" alt="' + item.en + '">' +
          stave(row.sample ? staveHTML(row.letters) : "", row.sample ? "trace" : "write") +
        "</div>";
      }).join("") + "</div>";
  }
  ...
  function buildBook() {
    ...
    pages.push(bookSheet(
      '<p class="bk-brand">The Alphabet</p><p class="bk-unit">Unit 8 · Review</p>' +
      '<p class="bk-display">Ww Xx Yy Zz</p><p class="bk-phrase">watch · fox · yo-yo · zebra</p>' +
    ...
    pages.push(bookSheet(bookHead("A", "Say. Then circle the beginning sound.") +
      '<p class="bk-wb-label">Workbook · Unit 8 Review</p>' + wbBeginningSoundHTML() + bookFoot(6, total)));
    ...
    pages.push(bookSheet(bookHead("C", "Listen and write.") +
      '<p class="bk-wb-label">Workbook · Unit 8 Review</p>' + wbLetterPickHTML() + bookFoot(8, total)));
    return pages.join("");
  }

  function buildGames() {
    var ids = ["wolf", "fox", "yoyo", "zebra", "watch", "box", "yak", "zero", "yacht", "zip", "web", "zoo"];
    return '<div class="sheet-frame"><article class="sheet theme-candy"><div class="rainbow-bar"></div>' +
      '<div class="sh-head"><p class="sh-kicker">Review games</p><h1 class="sh-title">Ww Xx Yy Zz 复习图卡</h1></div>' +
