...
(function (global) {
  ...
  function onsetHTML(item) {
    if (!item) return "";
    if (item.id === "Tt" || item.kind === "letter") {
  ...
  function staveHTML(item) {
    ...
    if (item.id === "tall-turtle") {
      return (
        ...
        '<span class="letter-small onset">t</span><span class="letter-small rest">all </span> ' +
        '<span class="letter-small onset">t</span><span class="letter-small rest">onkey</span>' +
        "</span>"
      );
    }
    return (
  ...
  function bookFoot(page, total) {
    return (
      '<div class="bk-foot">' +
      '<span class="bk-foot-meta">S-Class · The Alphabet · <b>Tt</b></span>' +
  ...
  function foot(page, total, pack) {
    return '<div class="foot"><span>S-Class · Tt · ' + pack + "</span><span>" + page + " / " + total + "</span></div>";
  }
  ...
  function buildBook() {
    ...
    pages.push(bookSheet("book-cover",
      ...
      '<p class="bk-display">Tt</p>' +
      '<p class="bk-display-hand">Tt</p>' +
      '<p class="bk-phrase">tall turtle</p>' +
      '<p class="bk-ipa">/t/</p>' +
      '<figure class="bk-hero"><img src="' + L.hero + '" alt="tall turtle"></figure>' +
      '<p class="bk-words">turtle  ·  tent  ·  tiger  ·  teacher</p>' +
      nameRow() +
      bookFoot(1, total)
    ));
    pages.push(bookSheet("",
      ...
      '<div class="bk-split">' +
        '<figure class="bk-plate">' +
          '<img src="' + L.mascot.img + '" alt="tall turtle">' +
          '<figcaption class="bk-caption">tall turtle</figcaption>' +
        "</figure>" +
        '<div class="bk-letter">' +
          '<div class="bk-faces">' +
            '<div class="bk-face is-print">' +
              '<span class="face-lab">Print</span>' +
              '<span class="pair">Tt</span>' +
            "</div>" +
        ...
        "</div>" +
      "</div>" +
      stave(staveHTML({ kind: "letter", id: "Tt" }), "trace", "fs-lg", "Trace") +
    ...
    pages.push(bookSheet("",
      ...
      '<div class="letter-models">' +
        ...
        modelCard("t", "small") +
      "</div>" +
      stave('<span class="letter-pair"><span class="letter-cap onset">T</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-cap onset">T</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      stave('<span class="letter-pair"><span class="letter-small onset">t</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-small onset">t</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      bookFoot(4, total)
    ));
    pages.push(bookSheet("",
      bookHead("D", "Which ones begin with the t sound? Trace and color.") +
      bookSection("描一描、涂一涂 t 开头的图。") +
      traceColorHTML() +
      bookFoot(5, total)
    ));
    pages.push(bookSheet("",
      ...
      "</div>" +
      '<figure class="bk-chant-girl"><img src="' + (L.chantCharacter || L.mascot.img) + '" alt=""><p class="bk-chant-bubble">That is my <img src="' + w(L.chantBubbleWord || "turtle").img + '" alt="turtle" class="bk-inline-egg"></p></figure>' +
      bookFoot(6, total)
    ));
    pages.push(bookSheet("",
      bookHead("F", "Match") +
    ...
    pages.push(bookSheet("",
      ...
      '<div class="bk-copy-page">' +
        wordCopyBlock(w("turtle")) +
        wordCopyBlock(w(L.chantBubbleWord || "turtle")) +
      "</div>" +
      bookFoot(11, total)
    ));
    pages.push(bookSheet("",
      ...
      '<div class="bk-copy-page">' +
        wordCopyBlock(w("tent")) +
        wordCopyBlock(w("tiger")) +
      "</div>" +
      bookFoot(12, total)
    ));
    pages.push(bookSheet("",
      bookHead("L", "Write Tt. Then match.") +
      '<p class="bk-wb-label">Workbook · Unit 7 Tt</p>' +
      '<div class="bk-wb-six"><div class="bk-wb-six-center">' + stave(staveHTML({ kind: "letter", id: "Tt" }), "write", "fs-lg", "Write Tt") + '</div>' +
      (L.wbMatchItems||[]).map(function(row){var item=w(row.id);return '<div class="bk-wb-six-item"><b>'+row.num+'</b><img src="'+item.img+'" alt=""></div>';}).join('')+'</div>' +
      bookFoot(13, total)
    ));
    pages.push(bookSheet("",
      bookHead("M", "Color T and t. Write Tt.") +
      '<figure class="bk-story"><img src="' + L.turtleColor + '" alt="turtle color"></figure>' +
      stave(staveHTML({ kind: "letter", id: "Tt" }), "trace", "fs-lg", "Write Tt") +
      bookFoot(14, total)
    ));
    pages.push(bookSheet("",
      bookHead("N", "Circle the pictures beginning with the /t/ sound.") +
      '<p class="bk-wb-label">Workbook · Unit 7 Tt</p>' +
      workbookCircleHTML() +
      bookFoot(15, total)
    ));
    pages.push(bookSheet("",
      bookHead("O", "Say. Then write Tt or cross it out.") +
      bookSection("说一说，然后在四线格写 <b>Tt</b> 或打叉。") +
      workbookCrossHTML() +
      bookFoot(16, total)
    ));
    return pages.join("");
  ...
  function buildGames() {
    ...
    pages.push(sheet("theme-candy",
      header("Paper games · Tt", "六个复习游戏 · 纸上也能玩", "课堂 / 回家") +
      nameRow() +
      '<ul class="howto">' +
        "<li><b>① 开头音小侦探</b> 蜡笔圈出 /t/ 开头的图。</li>" +
        "<li><b>② 听一听点一点</b> 老师读词，学生在图旁写 1–4。</li>" +
        "<li><b>③ 谁是局外人</b> 每一行打叉不是 /t/ 的那张。</li>" +
        "<li><b>④ 图词连线 + 剪卡配对</b> 先连线，再剪开做实物配对。</li>" +
        "<li><b>⑤ 写 Tt 还是打叉</b> 老师读（或放 Track 09），学生写。</li>" +
    ...
    pages.push(sheet("theme-leaf",
      ...
      task("1", "用红蜡笔圈出开头音是 /t/ 的图。不是 /t/ 的请不要圈。") +
    ...
    pages.push(sheet("",
      ...
      task("3", "每一行有三张图。把不是 /t/ 开头的那一张打 X。", "sun") +
    ...
    pages.push(sheet("theme-sky",
      header("Game 5 · Tt or X", "听音写 Tt 或打叉", "Track 09") +
      task("5", "老师放 Track 09 或读图名。是 /t/ 就写 Tt，不是就打 X。", "sky") +
      '<div class="six-grid">' +
  ...
  function buildCards() {
    ...
    pages.push(sheet("theme-poster",
      '<div class="letter-poster"><div class="giant">Tt</div><div class="sub">tall turtle · beginning sound /t/</div>' +
      '<img src="' + L.hero + '" alt="Tt"></div>' +
      foot(3, total, "字母卡")
    ));
    pages.push(sheet("",
      header("Fold cards 1–2", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("turtle")) + foldCard(w("turtle")) + "</div>" +
      foot(4, total, "对折卡")
    ));
    pages.push(sheet("",
      header("Fold cards 3–4", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("tent")) + foldCard(w("tiger")) + "</div>" +
      foot(5, total, "对折卡")
    ));
    pages.push(sheet("",
    ...
    pages.push(sheet("",
      ...
      '<div class="cut-grid">' +
        ["boots", "bear", "dog", "fan"].map(function (id) {
          var item = w(id);
          return '<div class="cut-card"><img src="' + item.img + '" alt=""><div class="lab">' + item.en + "</div></div>";
        }).join("") +
      "</div>" +
    ...
    pages.push(sheet("theme-candy",
      header("Mascot + letter", "tall turtle 与 Tt 小卡", "剪开") +
      '<div class="cut-grid">' +
        '<div class="cut-card"><img src="' + L.mascot.img + '" alt=""><div class="lab">' + onsetHTML(w("tall-turtle")) + "</div></div>" +
        '<div class="cut-card word-only"><div class="lab" style="font-size:64pt;">Tt</div><div class="ipa" style="font-size:28pt;color:#2e7d32;">/t/</div></div>' +
