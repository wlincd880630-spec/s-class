(function () {
  ...
  var slides = [{ type: "meet" }]
    ...
    .concat([{ type: "video" }, { type: "trace", letter: "M" }, { type: "trace", letter: "m" }]);
  ...
  function bindTrace(canvas) {
    ...
    canvas.addEventListener("pandadown", start);
    canvas.addEventListener("pandamove", move);
    window.addEventListener("pandaup", end);
  ...
  function render() {
    ...
    if (s.type === "meet") {
      box.innerHTML =
        ...
        '<div class="meet-row">' +
          '<div class="meet-card"><img src="' + L.mascot.img + '" alt="pink peach"></div>' +
          '<div class="meet-card ff-mark"><div class="big">Pp</div></div>' +
        "</div>" +
        "<h2>pink peach</h2>" +
