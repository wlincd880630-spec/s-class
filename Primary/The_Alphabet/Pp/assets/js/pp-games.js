(function () {
  ...
  function startG1() {
    ...
    function check() {
      ...
      document.querySelectorAll("#g1-grid .choice").forEach(function (c) {
        ...
        if (w.k && on) c.classList.add("good");
        else if (w.k && !on) { c.classList.add("bad"); miss++; }
        else if (!w.k && on) { c.classList.add("bad"); extra++; }
      });
      if (miss === 0 && extra === 0) {
        addScore(4);
        fb("全对", true);
  ...
  function startG5() {
    var n = 0;
    var items = L.pandaListenItems || L.track05Items;
    var answered = false;
    function ask() {
      answered = false;
      var it = items[n];
      ...
      $("play-board").innerHTML =
        '<div class="play-stage">' +
          ...
          '<div class="big-choice" id="g5-choice">' +
            '<button type="button" class="btn btn-leaf" data-bb="1">Pp</button>' +
      ...
      $("g5-choice").onclick = function (e) {
        ...
        if (!b) return;
        var choosePp = b.getAttribute("data-bb") === "1";
        if (choosePp !== it.writePp) {
          fb("再听开头音", false);
          play();
          return;
        }
  ...
  function startG6() {
    ...
    function check() {
      ...
      if (ok) {
        addScore(4);
        fb("顺序对了", true);
        showResult("完成", "panda, pineapple, peach, panda", startG6);
      } else {
        fb("再听，按歌曲顺序排", false);
      }
    }
