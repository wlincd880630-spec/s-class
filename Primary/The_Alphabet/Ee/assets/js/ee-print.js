...
(function (global) {
  ...
  function onsetHTML(item) {
    if (!item) return "";
    if (item.id === "Ee" || item.kind === "letter") {
      return '<span class="letter-pair"><span class="letter-cap onset">E</span><span class="letter-small rest">e</span></span>';
    }
    if (item.id === "energetic-egg") {
      return '<span class="onset">e</span>nergetic <span class="onset">e</span>gg';
    }
    if (!item.e) {
  ...
  function staveHTML(item) {
    ...
    if (item.id === "energetic-egg") {
      return (
        ...
        '<span class="letter-small onset">e</span><span class="letter-small rest">nergetic</span> ' +
        '<span class="letter-small onset">e</span><span class="letter-small rest">gg</span>' +
        "</span>"
      );
    }
    return (
  ...
  function bookFoot(page, total) {
    return (
      '<div class="bk-foot">' +
      '<span class="bk-foot-meta">S-Class · The Alphabet · <b>Ee</b></span>' +
  ...
  function foot(page, total, pack) {
    return '<div class="foot"><span>S-Class · Ee · ' + pack + "</span><span>" + page + " / " + total + "</span></div>";
  }
  ...
  function eggCartonHTML() {
    return (
      '<div class="bk-egg-carton">' +
        '<figure class="bk-egg-carton-art"><img src="' + L.eggCarton + '" alt="Egg carton listen and write"></figure>' +
        '<div class="bk-egg-slots">' +
          (L.eggCartonItems || []).map(function (row, i) {
            return (
              '<div class="bk-egg-slot">' +
                '<span class="bk-egg-num">' + (i + 1) + "</span>" +
                '<div class="bk-egg-write"><span>Ee / ✗</span></div>' +
              "</div>"
            );
          }).join("") +
        "</div>" +
  ...
  function workbookMatchHTML() {
    return (
      '<div class="bk-ee-match">' +
        (L.workbookMatch || []).map(function (row, i) {
          var ee = w(row.ee);
          var other = w(row.other);
          return (
            '<div class="bk-ee-match-row">' +
              '<span class="bk-ee-label"><b>' + (i + 1) + '</b> Ee</span>' +
              '<div class="bk-ee-pics">' +
                '<div class="bk-ee-pic"><img src="' + ee.img + '" alt=""><span>' + ee.en + "</span></div>" +
                '<div class="bk-ee-pic"><img src="' + other.img + '" alt=""><span>' + other.en + "</span></div>" +
              "</div>" +
              stave("", "write", "fs-md", "") +
            "</div>"
          );
  ...
  function buildBook() {
    ...
    pages.push(bookSheet("book-cover",
      ...
      '<p class="bk-display">Ee</p>' +
      '<p class="bk-display-hand">Ee</p>' +
      '<p class="bk-phrase">energetic egg</p>' +
      '<p class="bk-ipa">/e/</p>' +
      '<figure class="bk-hero"><img src="' + L.hero + '" alt="energetic egg"></figure>' +
      '<p class="bk-words">egg  ·  elbow  ·  envelope  ·  elephant</p>' +
      nameRow() +
      bookFoot(1, total)
    ));
    pages.push(bookSheet("",
      ...
      '<div class="bk-split">' +
        '<figure class="bk-plate">' +
          '<img src="' + L.mascot.img + '" alt="energetic egg">' +
          '<figcaption class="bk-caption">energetic egg</figcaption>' +
        "</figure>" +
        '<div class="bk-letter">' +
          '<div class="bk-faces">' +
            '<div class="bk-face is-print">' +
              '<span class="face-lab">Print</span>' +
              '<span class="pair">Ee</span>' +
            "</div>" +
        ...
        "</div>" +
      "</div>" +
      stave(staveHTML({ kind: "letter", id: "Ee" }), "trace", "fs-lg", "Trace") +
    ...
    pages.push(bookSheet("",
      ...
      '<div class="letter-models">' +
        ...
        modelCard("e", "small") +
      "</div>" +
      stave('<span class="letter-pair"><span class="letter-cap onset">E</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-cap onset">E</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      stave('<span class="letter-pair"><span class="letter-small onset">e</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-small onset">e</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      bookFoot(4, total)
    ));
    pages.push(bookSheet("",
      bookHead("D", "Listen. Then write Ee or cross it out.") +
      bookSection('Listen. If you hear the <span class="bk-ipa-inline">/e/</span> sound, write <b>Ee</b>. If not, cross it out.') +
      eggCartonHTML() +
      bookFoot(5, total)
    ));
    pages.push(bookSheet("",
      ...
      "</div>" +
      '<figure class="bk-chant-girl"><img src="' + L.mascot.img + '" alt=""><p class="bk-chant-bubble">An <img src="' + w("egg").img + '" alt="egg" class="bk-inline-egg"></p></figure>' +
      bookFoot(6, total)
    ));
    pages.push(bookSheet("",
      bookHead("F", "Match") +
    ...
    pages.push(bookSheet("",
      ...
      '<div class="bk-copy-page">' +
        wordCopyBlock(w("envelope")) +
        wordCopyBlock(w("elephant")) +
      "</div>" +
      bookFoot(12, total)
    ));
    pages.push(bookSheet("",
      bookHead("L", "Match and write") +
      bookSection('Which ones begin with the <span class="bk-ipa-inline">/e/</span> sound? Match and write <b>Ee</b>.') +
      workbookMatchHTML() +
      bookFoot(13, total)
    ));
    pages.push(bookSheet("",
      bookHead("M", "Say. Then write Ee or cross it out.") +
      bookSection("Say each picture. Write <b>Ee</b> or cross it out.") +
      workbookWriteHTML() +
      bookFoot(14, total)
    ));
    return pages.join("");
  ...
  function buildGames() {
    ...
    pages.push(sheet("theme-candy",
      header("Paper games · Ee", "六个复习游戏 · 纸上也能玩", "课堂 / 回家") +
      nameRow() +
      '<ul class="howto">' +
        "<li><b>① 开头音小侦探</b> 蜡笔圈出 /e/ 开头的图。</li>" +
        "<li><b>② 听一听点一点</b> 老师读词，学生在图旁写 1–4。</li>" +
        "<li><b>③ 谁是局外人</b> 每一行打叉不是 /e/ 的那张。</li>" +
        "<li><b>④ 图词连线 + 剪卡配对</b> 先连线，再剪开做实物配对。</li>" +
        "<li><b>⑤ 写 Ee 还是打叉</b> 老师读（或放 Track 09），学生写。</li>" +
    ...
    pages.push(sheet("theme-leaf",
      ...
      task("1", "用红蜡笔圈出开头音是 /e/ 的图。不是 /e/ 的请不要圈。") +
    ...
    pages.push(sheet("",
      ...
      task("3", "每一行有三张图。把不是 /e/ 开头的那一张打 X。", "sun") +
    ...
    pages.push(sheet("theme-sky",
      header("Game 5 · Ee or X", "听音写 Ee 或打叉", "Track 09") +
      task("5", "老师放 Track 09 或读图名。是 /e/ 就写 Ee，不是就打 X。", "sky") +
      '<div class="six-grid">' +
        L.track05Items.map(function (row, i) {
          var item = w(row.id);
          return (
    ...
    pages.push(sheet("theme-leaf",
      ...
      task("6", "剪下四张图。边唱 chant 边排队：egg, envelope, elbow, elephant。", "leaf") +
      '<div class="cut-grid">' +
        L.chantOrder.map(function (id, i) {
          var item = w(id);
          return (
  ...
  function buildCards() {
    ...
    pages.push(sheet("theme-poster-a",
      '<div class="letter-poster"><div class="giant">E</div><div class="sub">the letter E · /e/</div>' +
      '<img src="' + L.mascot.img + '" alt="energetic egg"></div>' +
      foot(1, total, "字母卡")
    ));
    pages.push(sheet("theme-poster-s",
      '<div class="letter-poster"><div class="giant">e</div><div class="sub">little e · /e/</div>' +
      '<img src="' + w("egg").img + '" alt="egg"></div>' +
      foot(2, total, "字母卡")
    ));
    pages.push(sheet("theme-poster",
      '<div class="letter-poster"><div class="giant">Ee</div><div class="sub">energetic egg · beginning sound /e/</div>' +
      '<img src="' + L.hero + '" alt="Ee"></div>' +
      foot(3, total, "字母卡")
    ));
    pages.push(sheet("",
      header("Fold cards 1–2", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("egg")) + foldCard(w("elbow")) + "</div>" +
      foot(4, total, "对折卡")
    ));
    pages.push(sheet("",
      header("Fold cards 3–4", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("envelope")) + foldCard(w("elephant")) + "</div>" +
      foot(5, total, "对折卡")
    ));
    pages.push(sheet("",
    ...
    pages.push(sheet("theme-leaf",
      ...
      '<div class="mini-grid">' +
        ["egg", "elbow", "envelope", "elephant", "apple", "ant", "dog", "cup"].map(function (id) {
          var item = w(id);
    ...
    pages.push(sheet("theme-candy",
      header("Mascot + letter", "energetic egg 与 Ee 小卡", "剪开") +
      '<div class="cut-grid">' +
        '<div class="cut-card"><img src="' + L.mascot.img + '" alt=""><div class="lab">' + onsetHTML(w("energetic-egg")) + "</div></div>" +
        '<div class="cut-card word-only"><div class="lab" style="font-size:64pt;">Ee</div><div class="ipa" style="font-size:28pt;color:#2e7d32;">/e/</div></div>' +
        '<div class="cut-card word-only"><div class="lab" style="font-size:80pt;color:#c62828;">E</div></div>' +
        '<div class="cut-card word-only"><div class="lab" style="font-size:80pt;color:#2e7d32;">e</div></div>' +
      "</div>" +
      foot(11, total, "字母小卡")
    ));
    return pages.join("");
