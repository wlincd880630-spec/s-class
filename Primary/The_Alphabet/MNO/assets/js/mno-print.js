...
(function (global) {
  ...
  function stave(html, mode) {
    return (
      ...
      (mode === "trace"
        ? '<div class="trace-word fs-md">' + html + "</div>" +
  ...
  function bookFoot(page, total) {
    return '<div class="bk-foot"><span class="bk-foot-meta">S-Class · Unit 5 Review · <b>Mm Nn Oo</b></span>' +
    ...
    return '<div class="bk-wb-ghi-grid">' +
      L.wbBeginningSound.map(function (row, i) {
        ...
        return '<div class="bk-wb-ghi-cell' + (row.sample ? " is-sample" : "") + '">' +
          ...
          '<div class="bk-wb-ghi-opts"><span>m</span><span>n</span><span>o</span></div></div>';
      }).join("") + "</div>";
  }
  ...
  function buildBook() {
    ...
    pages.push(bookSheet(
      '<p class="bk-brand">The Alphabet</p><p class="bk-unit">Unit 5 · Review</p>' +
      '<p class="bk-display">Mm Nn Oo</p><p class="bk-phrase">monkey · noisy nut · octopus</p>' +
    ...
    pages.push(bookSheet(bookHead("A", "Say. Then circle the beginning sound.") +
      '<p class="bk-wb-label">Workbook · Unit 5 Review</p>' + wbBeginningSoundHTML() + bookFoot(6, total)));
    pages.push(bookSheet(bookHead("B", "Trace and match.") + wbTraceMatchHTML() + bookFoot(7, total)));
    return pages.join("");
  }

  function buildGames() {
    var ids = ["monkey", "mouse", "milk", "money", "nut", "net", "nest", "nose", "octopus", "olive", "ox", "ostrich"];
    return '<div class="sheet-frame"><article class="sheet theme-candy"><div class="rainbow-bar"></div>' +
      '<div class="sh-head"><p class="sh-kicker">Review games</p><h1 class="sh-title">Mm Nn Oo 复习图卡</h1></div>' +
