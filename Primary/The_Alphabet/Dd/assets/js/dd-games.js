  ...
  function startG1() {
    ...
    function check() {
      ...
      document.querySelectorAll("#g1-grid .choice").forEach(function (c) {
        ...
        if (w.d && on) c.classList.add("good");
        else if (w.d && !on) { c.classList.add("bad"); miss++; }
        else if (!w.d && on) { c.classList.add("bad"); extra++; }
      });
      if (miss === 0 && extra === 0) {
        addScore(4);
        fb("全对", true);
  ...
  function startG5() {
    ...
    function ask() {
      ...
      $("play-board").innerHTML =
        '<div class="play-stage">' +
          ...
          '<div class="big-choice" id="g5-choice">' +
            '<button type="button" class="btn btn-leaf" data-bb="1">Dd</button>' +
      ...
      $("g5-choice").onclick = function (e) {
        ...
        if (!b) return;
        var chooseDd = b.getAttribute("data-bb") === "1";
        if (chooseDd !== it.writeDd) {
          fb("再听开头音", false);
          play();
          return;
        }
  ...
  function startG6() {
    ...
    function check() {
      ...
      if (ok) {
        addScore(4);
        fb("顺序对了", true);
        showResult("完成", "doll, dog, desk, duck", startG6);
      } else {
        fb("再听，按歌曲顺序排", false);
      }
    }
