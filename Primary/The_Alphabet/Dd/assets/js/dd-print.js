...
(function (global) {
  ...
  function onsetHTML(item) {
    if (!item) return "";
    if (item.id === "Dd" || item.kind === "letter") {
      return '<span class="letter-pair"><span class="letter-cap onset">D</span><span class="letter-small rest">d</span></span>';
    }
    if (item.id === "dizzy-dog") {
      return '<span class="onset">d</span>izzy <span class="onset">d</span>og';
    }
    if (!item.d) {
  ...
  function staveHTML(item) {
    ...
    if (item.id === "dizzy-dog") {
      return (
        ...
        '<span class="letter-small onset">d</span><span class="letter-small rest">izzy</span> ' +
        '<span class="letter-small onset">d</span><span class="letter-small rest">og</span>' +
        "</span>"
      );
    }
    return (
  ...
  function bookFoot(page, total) {
    return (
      '<div class="bk-foot">' +
      '<span class="bk-foot-meta">S-Class · The Alphabet · <b>Dd</b></span>' +
  ...
  function foot(page, total, pack) {
    return '<div class="foot"><span>S-Class · Dd · ' + pack + "</span><span>" + page + " / " + total + "</span></div>";
  }
  ...
  function soundGridHTML() {
    return (
      '<div class="bk-sound-grid">' +
        (L.soundHuntGrid || []).map(function (id, i) {
          var item = w(id);
          var sample = item.d;
          return (
  ...
  function buildBook() {
    ...
    pages.push(bookSheet("book-cover",
      ...
      '<p class="bk-display">Dd</p>' +
      '<p class="bk-display-hand">Dd</p>' +
      '<p class="bk-phrase">dizzy dog</p>' +
      '<p class="bk-ipa">/d/</p>' +
      '<figure class="bk-hero"><img src="' + L.hero + '" alt="dizzy dog"></figure>' +
      '<p class="bk-words">dog  ·  desk  ·  doll  ·  duck</p>' +
      nameRow() +
      bookFoot(1, total)
    ));
    pages.push(bookSheet("",
      ...
      '<div class="bk-split">' +
        '<figure class="bk-plate">' +
          '<img src="' + L.mascot.img + '" alt="dizzy dog">' +
          '<figcaption class="bk-caption">dizzy dog</figcaption>' +
        "</figure>" +
        '<div class="bk-letter">' +
          '<div class="bk-faces">' +
            '<div class="bk-face is-print">' +
              '<span class="face-lab">Print</span>' +
              '<span class="pair">Dd</span>' +
            "</div>" +
        ...
        "</div>" +
      "</div>" +
      stave(staveHTML({ kind: "letter", id: "Dd" }), "trace", "fs-lg", "Trace") +
    ...
    pages.push(bookSheet("",
      ...
      '<div class="letter-models">' +
        ...
        modelCard("d", "small") +
      "</div>" +
      stave('<span class="letter-pair"><span class="letter-cap onset">D</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-cap onset">D</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      stave('<span class="letter-pair"><span class="letter-small onset">d</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-small onset">d</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      bookFoot(4, total)
    ));
    pages.push(bookSheet("",
      bookHead("D", "Which one begins with the d sound?") +
      bookSection('Listen and circle the pictures that begin with <span class="bk-ipa-inline">/d/</span>.') +
      soundGridHTML() +
      bookFoot(5, total)
    ));
    pages.push(bookSheet("",
    ...
    pages.push(bookSheet("",
      ...
      '<div class="bk-copy-page">' +
        wordCopyBlock(w("doll")) +
        wordCopyBlock(w("duck")) +
      "</div>" +
      bookFoot(12, total)
    ));
    pages.push(bookSheet("",
      bookHead("L", "Circle the d sound") +
      bookSection('Circle the pictures beginning with <span class="bk-ipa-inline">/d/</span>.') +
      '<div class="bk-picnic">' +
    ...
    pages.push(bookSheet("",
      bookHead("M", "Connect, say, and write Dd") +
      bookSection("Connect the dog to the doghouse. Say each picture on the path.") +
      '<figure class="bk-connect-map"><img src="' + L.connectMap + '" alt="Dog maze connect"></figure>' +
      '<div class="bk-connect-write">' +
        '<p class="bk-connect-label">Write Dd.</p>' +
        stave(staveHTML({ kind: "letter", id: "Dd" }), "write", "fs-lg", "") +
      "</div>" +
      bookFoot(14, total)
    ));
    return pages.join("");
  ...
  function buildGames() {
    ...
    pages.push(sheet("theme-candy",
      header("Paper games · Dd", "六个复习游戏 · 纸上也能玩", "课堂 / 回家") +
      nameRow() +
      '<ul class="howto">' +
        "<li><b>① 开头音小侦探</b> 蜡笔圈出 /d/ 开头的图。</li>" +
        "<li><b>② 听一听点一点</b> 老师读词，学生在图旁写 1–4。</li>" +
        "<li><b>③ 谁是局外人</b> 每一行打叉不是 /d/ 的那张。</li>" +
        "<li><b>④ 图词连线 + 剪卡配对</b> 先连线，再剪开做实物配对。</li>" +
        "<li><b>⑤ 写 Dd 还是打叉</b> 老师读（或放 Track 09），学生写。</li>" +
    ...
    pages.push(sheet("theme-leaf",
      ...
      task("1", "用红蜡笔圈出开头音是 /d/ 的图。不是 /d/ 的请不要圈。") +
    ...
    pages.push(sheet("",
      ...
      task("3", "每一行有三张图。把不是 /d/ 开头的那一张打 X。", "sun") +
    ...
    pages.push(sheet("theme-sky",
      header("Game 5 · Dd or X", "听音写 Dd 或打叉", "Track 09") +
      task("5", "老师放 Track 09 或读图名。是 /d/ 就写 Dd，不是就打 X。", "sky") +
      '<div class="six-grid">' +
    ...
    pages.push(sheet("theme-leaf",
      ...
      task("6", "剪下四张图。边唱 chant 边排队：doll, dog, desk, duck。", "leaf") +
      '<div class="cut-grid">' +
        L.chantOrder.map(function (id, i) {
          var item = w(id);
          return (
  ...
  function buildCards() {
    ...
    pages.push(sheet("theme-poster-a",
      '<div class="letter-poster"><div class="giant">D</div><div class="sub">the letter D · /d/</div>' +
      '<img src="' + L.mascot.img + '" alt="dizzy dog"></div>' +
      foot(1, total, "字母卡")
    ));
    pages.push(sheet("theme-poster-s",
      '<div class="letter-poster"><div class="giant">d</div><div class="sub">little d · /d/</div>' +
      '<img src="' + w("dog").img + '" alt="dog"></div>' +
      foot(2, total, "字母卡")
    ));
    pages.push(sheet("theme-poster",
      '<div class="letter-poster"><div class="giant">Dd</div><div class="sub">dizzy dog · beginning sound /d/</div>' +
      '<img src="' + L.hero + '" alt="Dd"></div>' +
      foot(3, total, "字母卡")
    ));
    pages.push(sheet("",
      header("Fold cards 1–2", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("dog")) + foldCard(w("desk")) + "</div>" +
      foot(4, total, "对折卡")
    ));
    pages.push(sheet("",
      header("Fold cards 3–4", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("doll")) + foldCard(w("duck")) + "</div>" +
      foot(5, total, "对折卡")
    ));
    pages.push(sheet("",
    ...
    pages.push(sheet("theme-leaf",
      ...
      '<div class="mini-grid">' +
        ["dog", "desk", "doll", "duck", "apple", "ant", "car", "cup"].map(function (id) {
          var item = w(id);
    ...
    pages.push(sheet("theme-candy",
      header("Mascot + letter", "dizzy dog 与 Dd 小卡", "剪开") +
      '<div class="cut-grid">' +
        '<div class="cut-card"><img src="' + L.mascot.img + '" alt=""><div class="lab">' + onsetHTML(w("dizzy-dog")) + "</div></div>" +
        '<div class="cut-card word-only"><div class="lab" style="font-size:64pt;">Dd</div><div class="ipa" style="font-size:28pt;color:#2e7d32;">/d/</div></div>' +
        '<div class="cut-card word-only"><div class="lab" style="font-size:80pt;color:#c62828;">D</div></div>' +
        '<div class="cut-card word-only"><div class="lab" style="font-size:80pt;color:#2e7d32;">d</div></div>' +
      "</div>" +
      foot(11, total, "字母小卡")
    ));
    return pages.join("");
