/**
 * Gg 互动练习册：描红、圈 /g/ 图、写 Gg 或打叉。
 */
(function () {
  "use strict";
  var L = window.GG_LESSON;
  var step = 1;
  var titles = ["", "练习册 A · 描红", "练习册 B · 圈图", "练习册 C · 写 Gg"];
  ...
  var ctxA = bindTrace("G", "cap");
  var ctxa = bindTrace("g", "small");
  ...
  function renderCircle() {
    ...
    $("btn-check-b").addEventListener("click", function () {
      ...
      $("fb-b").textContent = ok ? "全对！girl、gift、goat、gorilla 都是 /g/。" : "再看看：只圈 /g/ 开头的图。";
  ...
  function renderWrite() {
    ...
    grid.innerHTML = L.track05Items.map(function (row, i) {
      ...
      return (
        '<div class="wb-write-card" data-i="' + i + '">' +
          '<img src="' + item.img + '" alt="' + item.en + '">' +
          '<div class="btn-row">' +
            '<button type="button" class="btn btn-leaf" data-v="1">Gg</button>' +
            '<button type="button" class="btn btn-apple" data-v="0">✗</button>' +
          "</div>" +
        "</div>"
      );
    }).join("");
    grid.querySelectorAll(".wb-write-card").forEach(function (card) {
      var i = Number(card.getAttribute("data-i"));
      var row = L.track05Items[i];
      var item = L.words[row.id];
      card.querySelectorAll("button").forEach(function (btn) {
        btn.addEventListener("click", function () {
          picked[i] = btn.getAttribute("data-v") === "1";
          card.querySelectorAll("button").forEach(function (b) { b.classList.remove("is-on"); });
          btn.classList.add("is-on");
          if (window.AAAudio) AAAudio.speakWord(item.en, true);
        });
      });
    });
    $("btn-check-c").addEventListener("click", function () {
      var ok = L.track05Items.every(function (row, i) {
        return picked[i] === row.writeGg;
      });
      $("fb-c").textContent = ok ? "全对！" : "再听每个词的开头音。";
