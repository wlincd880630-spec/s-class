...
(function (global) {
  ...
  function onsetHTML(item) {
    if (!item) return "";
    if (item.id === "Gg" || item.kind === "letter") {
      return '<span class="letter-pair"><span class="letter-cap onset">G</span><span class="letter-small rest">g</span></span>';
    }
    if (item.id === "good-gorilla") {
      return '<span class="onset">g</span>ood <span class="onset">g</span>orilla';
    }
    if (!item.g) {
  ...
  function staveHTML(item) {
    ...
    if (item.id === "good-gorilla") {
      return (
        ...
        '<span class="letter-small onset">g</span><span class="letter-small rest">ood</span> ' +
        '<span class="letter-small onset">g</span><span class="letter-small rest">orilla</span>' +
        "</span>"
      );
    }
    return (
  ...
  function bookFoot(page, total) {
    return (
      '<div class="bk-foot">' +
      '<span class="bk-foot-meta">S-Class · The Alphabet · <b>Gg</b></span>' +
  ...
  function foot(page, total, pack) {
    return '<div class="foot"><span>S-Class · Gg · ' + pack + "</span><span>" + page + " / " + total + "</span></div>";
  }
  ...
  function soundHuntRowsHTML() {
    return '<div class="bk-sound-rows">' +
      (L.soundHuntRows || []).map(function (row, ri) {
        return '<div class="bk-sound-row"><b>' + (ri + 1) + ".</b>" +
          row.pics.map(function (id) {
            var item = w(id);
            var sample = id === row.answer;
            return '<div class="bk-opt is-pic' + (sample ? " is-sample" : "") + '">' +
              '<span class="bk-ring' + (sample ? " is-sample" : "") + '"></span>' +
              '<img src="' + item.img + '" alt="' + item.en + '"></div>';
          }).join("") + "</div>";
      }).join("") + "</div>";
  }
  ...
  function workbookWriteHTML() {
    return '<div class="bk-gg-write-row">' +
      (L.track05Items || []).map(function (row, i) {
        var item = w(row.id);
        return '<div class="bk-gg-write-item"><span class="bk-gg-num">' + (i + 1) + "</span>" +
          '<img src="' + item.img + '" alt="' + item.en + '">' +
          '<div class="bk-gg-write-box">' + stave("", "write", "fs-md", "Gg / ✗") + "</div></div>";
      }).join("") + "</div>";
  }
    ...
    return (
      '<div class="bk-gg-hub">' +
        '<div class="bk-gg-hub-center">' +
          stave(staveHTML({ kind: "letter", id: "Gg" }), "write", "fs-lg", "Write Gg") +
        "</div>" +
  ...
  function buildBook() {
    ...
    pages.push(bookSheet("book-cover",
      ...
      '<p class="bk-display">Gg</p>' +
      '<p class="bk-display-hand">Gg</p>' +
      '<p class="bk-phrase">good gorilla</p>' +
      '<p class="bk-ipa">/g/</p>' +
      '<figure class="bk-hero"><img src="' + L.hero + '" alt="good gorilla"></figure>' +
      '<p class="bk-words">gorilla  ·  goat  ·  gift  ·  girl</p>' +
      nameRow() +
      bookFoot(1, total)
    ));
    pages.push(bookSheet("",
      ...
      '<div class="bk-split">' +
        '<figure class="bk-plate">' +
          '<img src="' + L.mascot.img + '" alt="good gorilla">' +
          '<figcaption class="bk-caption">good gorilla</figcaption>' +
        "</figure>" +
        '<div class="bk-letter">' +
          '<div class="bk-faces">' +
            '<div class="bk-face is-print">' +
              '<span class="face-lab">Print</span>' +
              '<span class="pair">Gg</span>' +
            "</div>" +
        ...
        "</div>" +
      "</div>" +
      stave(staveHTML({ kind: "letter", id: "Gg" }), "trace", "fs-lg", "Trace") +
    ...
    pages.push(bookSheet("",
      ...
      '<div class="letter-models">' +
        ...
        modelCard("g", "small") +
      "</div>" +
      stave('<span class="letter-pair"><span class="letter-cap onset">G</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-cap onset">G</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      stave('<span class="letter-pair"><span class="letter-small onset">g</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-small onset">g</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      bookFoot(4, total)
    ));
    pages.push(bookSheet("",
      bookHead("D", "Which one begins with the g sound? Listen and circle.") +
      bookSection("每一行只有一个图以 /g/ 开头。听音后圈出来。") +
      soundHuntRowsHTML() +
      bookFoot(5, total)
    ));
    pages.push(bookSheet("",
      ...
      "</div>" +
      '<figure class="bk-chant-girl"><img src="' + (L.chantCharacter || L.mascot.img) + '" alt=""><p class="bk-chant-bubble">I see a <img src="' + w("gorilla").img + '" alt="gorilla" class="bk-inline-egg"></p></figure>' +
      bookFoot(6, total)
    ));
    pages.push(bookSheet("",
      bookHead("F", "Match") +
    ...
    pages.push(bookSheet("",
      ...
      '<div class="bk-copy-page">' +
        wordCopyBlock(w("gorilla")) +
        wordCopyBlock(w("goat")) +
      "</div>" +
      bookFoot(11, total)
    ));
    pages.push(bookSheet("",
      ...
      '<div class="bk-copy-page">' +
        wordCopyBlock(w("gift")) +
        wordCopyBlock(w("girl")) +
      "</div>" +
      bookFoot(12, total)
    ));
    pages.push(bookSheet("",
      bookHead("L", "Circle the pictures beginning with the /g/ sound.") +
      '<p class="bk-wb-label">Workbook · Unit 3 Gg</p>' +
      workbookCircleHTML() +
      bookFoot(13, total)
    ));
    pages.push(bookSheet("",
      bookHead("M", "Say. Then write Gg or cross it out.") +
      workbookWriteHTML() +
      bookFoot(14, total)
    ));
    return pages.join("");
  ...
  function buildGames() {
    ...
    pages.push(sheet("theme-candy",
      header("Paper games · Gg", "六个复习游戏 · 纸上也能玩", "课堂 / 回家") +
      nameRow() +
      '<ul class="howto">' +
        "<li><b>① 开头音小侦探</b> 蜡笔圈出 /g/ 开头的图。</li>" +
        "<li><b>② 听一听点一点</b> 老师读词，学生在图旁写 1–4。</li>" +
        "<li><b>③ 谁是局外人</b> 每一行打叉不是 /g/ 的那张。</li>" +
        "<li><b>④ 图词连线 + 剪卡配对</b> 先连线，再剪开做实物配对。</li>" +
        "<li><b>⑤ 写 Gg 还是打叉</b> 老师读（或放 Track 09），学生写。</li>" +
    ...
    pages.push(sheet("theme-leaf",
      ...
      task("1", "用红蜡笔圈出开头音是 /g/ 的图。不是 /g/ 的请不要圈。") +
    ...
    pages.push(sheet("",
      ...
      task("3", "每一行有三张图。把不是 /g/ 开头的那一张打 X。", "sun") +
    ...
    pages.push(sheet("theme-sky",
      header("Game 5 · Gg or X", "听音写 Gg 或打叉", "Track 09") +
      task("5", "老师放 Track 09 或读图名。是 /g/ 就写 Gg，不是就打 X。", "sky") +
      '<div class="six-grid">' +
        L.track05Items.map(function (row, i) {
          var item = w(row.id);
          return (
    ...
    pages.push(sheet("theme-leaf",
      ...
      task("6", "剪下四张图。边唱 chant 边排队：gorilla, girl, gift, goat。", "leaf") +
      '<div class="cut-grid">' +
        L.chantOrder.map(function (id, i) {
          var item = w(id);
          return (
  ...
  function buildCards() {
    ...
    pages.push(sheet("theme-poster-a",
      '<div class="letter-poster"><div class="giant">F</div><div class="sub">the letter F · /g/</div>' +
      '<img src="' + L.mascot.img + '" alt="good gorilla"></div>' +
      foot(1, total, "字母卡")
    ));
    pages.push(sheet("theme-poster-s",
      '<div class="letter-poster"><div class="giant">f</div><div class="sub">little f · /g/</div>' +
      '<img src="' + w("gorilla").img + '" alt="gorilla"></div>' +
      foot(2, total, "字母卡")
    ));
    pages.push(sheet("theme-poster",
      '<div class="letter-poster"><div class="giant">Gg</div><div class="sub">good gorilla · beginning sound /g/</div>' +
      '<img src="' + L.hero + '" alt="Gg"></div>' +
      foot(3, total, "字母卡")
    ));
    pages.push(sheet("",
      header("Fold cards 1–2", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("gorilla")) + foldCard(w("goat")) + "</div>" +
      foot(4, total, "对折卡")
    ));
    pages.push(sheet("",
      header("Fold cards 3–4", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("gift")) + foldCard(w("girl")) + "</div>" +
      foot(5, total, "对折卡")
    ));
    pages.push(sheet("",
    ...
    pages.push(sheet("theme-leaf",
      ...
      '<div class="mini-grid">' +
        ["gorilla", "goat", "gift", "girl", "fan", "bird", "dog", "cup"].map(function (id) {
          var item = w(id);
    ...
    pages.push(sheet("theme-candy",
      header("Mascot + letter", "good gorilla 与 Gg 小卡", "剪开") +
      '<div class="cut-grid">' +
        '<div class="cut-card"><img src="' + L.mascot.img + '" alt=""><div class="lab">' + onsetHTML(w("good-gorilla")) + "</div></div>" +
        '<div class="cut-card word-only"><div class="lab" style="font-size:64pt;">Gg</div><div class="ipa" style="font-size:28pt;color:#2e7d32;">/g/</div></div>' +
