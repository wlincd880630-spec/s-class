/**
 * GHI 复习 · 教具工坊：教材 PDF（Student's Book + Workbook）
 */
(function (global) {
  "use strict";

  var L = window.GHI_REVIEW;
  ...
  function bookFoot(page, total) {
    return '<div class="bk-foot"><span class="bk-foot-meta">S-Class · Unit 3 Review · <b>Gg Hh Ii</b></span>' +
    ...
    return '<div class="bk-wb-ghi-grid">' +
      L.wbBeginningSound.map(function (row, i) {
        ...
        return '<div class="bk-wb-ghi-cell' + (row.sample ? " is-sample" : "") + '">' +
          ...
          '<div class="bk-wb-ghi-opts"><span>g</span><span>h</span><span>i</span></div></div>';
      }).join("") + "</div>";
  }
  ...
  function buildBook() {
    ...
    pages.push(bookSheet(
      '<p class="bk-brand">The Alphabet</p><p class="bk-unit">Unit 3 · Review</p>' +
      '<p class="bk-display">Gg Hh Ii</p><p class="bk-phrase">gorilla · horse · insect</p>' +
      '<figure class="bk-hero"><img src="' + L.hero + '" alt=""></figure>' +
      '<p class="bk-words">Student\'s Book p.30–31 · Workbook p.15</p>' + nameRow() + bookFoot(1, total)
    ));
    ...
    pages.push(bookSheet(bookHead("A", "Say. Then circle the beginning sound.") +
      '<p class="bk-wb-label">Workbook · Unit 3 Review</p>' + wbBeginningSoundHTML() + bookFoot(6, total)));
    pages.push(bookSheet(bookHead("B", "Trace and match.") + wbTraceMatchHTML() + bookFoot(7, total)));
    return pages.join("");
  }

  function buildGames() {
    var ids = ["gorilla", "horse", "insect", "hat", "ink", "igloo", "gift", "girl", "hot-dog", "iguana", "goat", "bear"];
    return '<div class="sheet-frame"><article class="sheet theme-candy"><div class="rainbow-bar"></div>' +
      '<div class="sh-head"><p class="sh-kicker">Review games</p><h1 class="sh-title">Gg Hh Ii 复习图卡</h1></div>' +
