/**
 * Ww 互动练习册：描红、配对、写 Ww 或打叉。
 */
(function () {
  "use strict";
  var L = window.WW_LESSON;
  var step = 1;
  var titles = ["", "练习册 A · 描红", "练习册 B · 配对", "练习册 C · Ww / ✗"];
  ...
  var ctxW = bindTrace("W", "cap");
  var ctxw = bindTrace("w", "small");
  ...
  function renderMatch() {
    ...
    $("btn-check-b").addEventListener("click", function () {
      ...
      $("fb-b").textContent = ok ? "全对！wolf、web、water、watch 都是 /w/ 开头。" : "再看看：每行只选 w 开头的图。";
  ...
  function renderCross() {
    ...
    grid.innerHTML = (L.track05Items || []).map(function (row, i) {
      ...
      return (
        '<div class="wb-cross-item" data-i="' + i + '">' +
          ...
          '<div class="en">' + item.en + "</div>" +
          '<div class="btn-row">' +
            '<button type="button" class="btn btn-leaf" data-pick="ww" data-i="' + i + '">Ww</button>' +
    ...
    $("btn-check-c").addEventListener("click", function () {
      ...
      $("fb-c").textContent = ok ? "全对！wolf、web、watch 写 Ww；jet 打叉。" : "再听一遍：/w/ 开头写 Ww，否则打叉。";
  ...
  $("btn-clear-trace").addEventListener("click", function () {
    if (ctxW) ctxW.clearRect(0, 0, ctxW.canvas.width, ctxW.canvas.height);
    if (ctxw) ctxw.clearRect(0, 0, ctxw.canvas.width, ctxw.canvas.height);
  });
