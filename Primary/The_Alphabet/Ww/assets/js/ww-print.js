...
(function (global) {
  ...
  function onsetHTML(item) {
    if (!item) return "";
    if (item.id === "Ww" || item.kind === "letter") {
      return '<span class="letter-pair"><span class="letter-cap onset">W</span><span class="letter-small rest">w</span></span>';
    }
    if (item.id === "wise-wolf") {
      return '<span class="onset">w</span>ise <span class="onset">w</span>olf';
    }
    if (!item.w) {
  ...
  function staveHTML(item) {
    ...
    if (item.id === "wise-wolf") {
      return (
        ...
        '<span class="letter-small onset">w</span><span class="letter-small rest">ise</span> ' +
        '<span class="letter-small onset">w</span><span class="letter-small rest">olf</span>' +
        "</span>"
      );
    }
    return (
  ...
  function bookFoot(page, total) {
    return (
      '<div class="bk-foot">' +
      '<span class="bk-foot-meta">S-Class · The Alphabet · <b>Ww</b></span>' +
  ...
  function foot(page, total, pack) {
    return '<div class="foot"><span>S-Class · Ww · ' + pack + "</span><span>" + page + " / " + total + "</span></div>";
  }
  ...
  function watchWriteHTML() {
    ...
    return '<div class="bk-watch-grid">' +
      (L.watchWriteItems || []).map(function (row, i) {
        return (
  ...
  function matchEightHTML() {
    return '<div class="bk-wb-six"><div class="bk-wb-six-center">' + stave(staveHTML({ kind: "letter", id: "Ww" }), "write", "fs-lg", "Write Ww") + '</div>' +
        ...
        (L.wbMatchRows || []).map(function (row, i) {
          ...
          return (
            '<div class="bk-ee-match-row">' +
              '<span class="bk-ee-label"><b>' + (i + 1) + '</b> Ww</span>' +
        ...
        (L.track05Items || []).map(function (row, i) {
          ...
          return (
            '<div class="bk-ee-write-item">' +
              ...
              '<img src="' + item.img + '" alt="' + item.en + '">' +
              '<div class="bk-egg-write"><span>Ww / ✗</span></div>' +
            "</div>"
          );
        }).join("") +
      "</div>"
  ...
  function buildBook() {
    ...
    pages.push(bookSheet("book-cover",
      ...
      '<p class="bk-display">Ww</p>' +
      '<p class="bk-display-hand">Ww</p>' +
      '<p class="bk-phrase">wise wolf</p>' +
      '<p class="bk-ipa">/w/</p>' +
      '<figure class="bk-hero"><img src="' + L.hero + '" alt="wise wolf"></figure>' +
      '<p class="bk-words">wolf  ·  web  ·  water  ·  watch</p>' +
      nameRow() +
      bookFoot(1, total)
    ));
    pages.push(bookSheet("",
      ...
      '<div class="bk-split">' +
        '<figure class="bk-plate">' +
          '<img src="' + L.mascot.img + '" alt="wise wolf">' +
          '<figcaption class="bk-caption">wise wolf</figcaption>' +
        "</figure>" +
        '<div class="bk-letter">' +
          '<div class="bk-faces">' +
            '<div class="bk-face is-print">' +
              '<span class="face-lab">Print</span>' +
              '<span class="pair">Ww</span>' +
            "</div>" +
        ...
        "</div>" +
      "</div>" +
      stave(staveHTML({ kind: "letter", id: "Ww" }), "trace", "fs-lg", "Trace") +
    ...
    pages.push(bookSheet("",
      ...
      '<div class="letter-models">' +
        ...
        modelCard("w", "small") +
      "</div>" +
      stave('<span class="letter-pair"><span class="letter-cap onset">W</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-cap onset">W</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      stave('<span class="letter-pair"><span class="letter-small onset">w</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-small onset">w</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      bookFoot(4, total)
    ));
    pages.push(bookSheet("",
      bookHead("D", "Listen. Then write Ww or cross it out.") +
      bookSection("听音后，在四线格写 <b>Ww</b> 或打叉。（Disc 2 · Track 48）") +
      watchWriteHTML() +
      bookFoot(5, total)
    ));
    pages.push(bookSheet("",
    ...
    pages.push(bookSheet("",
      ...
      '<div class="bk-copy-page">' +
        wordCopyBlock(w("water")) +
        wordCopyBlock(w("watch")) +
      "</div>" +
      bookFoot(12, total)
    ));
    pages.push(bookSheet("",
      bookHead("L", "Trace, write, and say") +
      '<p class="bk-wb-label">Workbook · Unit 8 Ww</p>' +
      '<figure class="bk-story"><img src="' + w("wolf").img + '" alt="wolf"></figure>' +
      stave(staveHTML({ kind: "letter", id: "Ww" }), "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      bookFoot(13, total)
    ));
    pages.push(bookSheet("",
      bookHead("M", "Which ones begin with w? Match and write.") +
      bookSection('Which ones begin with the <span class="bk-ipa-inline">/w/</span> sound? Match and write <b>Ww</b>.') +
      workbookMatchHTML() +
      bookFoot(14, total)
    ));
    pages.push(bookSheet("",
      bookHead("N", "Say. Then write Ww or cross it out.") +
      bookSection("说一说，然后在四线格写 <b>Ww</b> 或打叉。") +
      workbookWriteHTML() +
      bookFoot(15, total)
    ));
    return pages.join("");
  ...
  function buildGames() {
    ...
    pages.push(sheet("theme-candy",
      header("Paper games · Ww", "六个复习游戏 · 纸上也能玩", "课堂 / 回家") +
      nameRow() +
      '<ul class="howto">' +
        "<li><b>① 开头音小侦探</b> 蜡笔圈出 /w/ 开头的图。</li>" +
        "<li><b>② 听一听点一点</b> 老师读词，学生在图旁写 1–4。</li>" +
        "<li><b>③ 谁是局外人</b> 每一行打叉不是 /w/ 的那张。</li>" +
        "<li><b>④ 图词连线 + 剪卡配对</b> 先连线，再剪开做实物配对。</li>" +
        "<li><b>⑤ 写 Ww 还是打叉</b> 老师读（或放 Track 09），学生写。</li>" +
    ...
    pages.push(sheet("theme-leaf",
      ...
      task("1", "用红蜡笔圈出开头音是 /w/ 的图。不是 /w/ 的请不要圈。") +
    ...
    pages.push(sheet("",
      ...
      task("3", "每一行有三张图。把不是 /w/ 开头的那一张打 X。", "sun") +
    ...
    pages.push(sheet("theme-sky",
      header("Game 5 · Ww or X", "听音写 Ww 或打叉", "Track 09") +
      task("5", "老师放 Track 09 或读图名。是 /w/ 就写 Ww，不是就打 X。", "sky") +
      '<div class="six-grid">' +
        L.track05Items.map(function (row, i) {
          var item = w(row.id);
          return (
    ...
    pages.push(sheet("theme-leaf",
      ...
      task("6", "剪下四张图。边唱 chant 边排队：web, watch, water, wolf。", "leaf") +
      '<div class="cut-grid">' +
        L.chantOrder.map(function (id, i) {
          var item = w(id);
          return (
  ...
  function buildCards() {
    ...
    pages.push(sheet("theme-poster-a",
      '<div class="letter-poster"><div class="giant">W</div><div class="sub">the letter W · /w/</div>' +
      '<img src="' + L.mascot.img + '" alt="wise wolf"></div>' +
      foot(1, total, "字母卡")
    ));
    pages.push(sheet("theme-poster-s",
      '<div class="letter-poster"><div class="giant">w</div><div class="sub">little w · /w/</div>' +
      '<img src="' + w("wolf").img + '" alt="wolf"></div>' +
      foot(2, total, "字母卡")
    ));
    pages.push(sheet("theme-poster",
      '<div class="letter-poster"><div class="giant">Ww</div><div class="sub">wise wolf · beginning sound /w/</div>' +
      '<img src="' + L.hero + '" alt="Ww"></div>' +
      foot(3, total, "字母卡")
    ));
    pages.push(sheet("",
      header("Fold cards 1–2", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("wolf")) + foldCard(w("wolf")) + "</div>" +
      foot(4, total, "对折卡")
    ));
    pages.push(sheet("",
      header("Fold cards 3–4", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("web")) + foldCard(w("water")) + "</div>" +
      foot(5, total, "对折卡")
    ));
    pages.push(sheet("",
    ...
    pages.push(sheet("theme-leaf",
      ...
      '<div class="mini-grid">' +
        ["wolf", "web", "water", "watch", "jet", "lemon", "pen", "bed"].map(function (id) {
          var item = w(id);
    ...
    pages.push(sheet("theme-candy",
      header("Mascot + letter", "wise wolf 与 Ww 小卡", "剪开") +
      '<div class="cut-grid">' +
        '<div class="cut-card"><img src="' + L.mascot.img + '" alt=""><div class="lab">' + onsetHTML(w("wise-wolf")) + "</div></div>" +
        '<div class="cut-card word-only"><div class="lab" style="font-size:64pt;">Ww</div><div class="ipa" style="font-size:28pt;color:#2e7d32;">/w/</div></div>' +
