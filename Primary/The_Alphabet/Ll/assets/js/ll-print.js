...
(function (global) {
  ...
  function onsetHTML(item) {
    if (!item) return "";
    if (item.id === "Ll" || item.kind === "letter") {
      return '<span class="letter-pair"><span class="letter-cap onset">L</span><span class="letter-small rest">l</span></span>';
    }
    if (item.id === "lazy-lion") {
      return '<span class="onset">l</span>azy <span class="onset">l</span>ion';
    }
    if (!item.l) {
  ...
  function staveHTML(item) {
    ...
    if (item.id === "lazy-lion") {
      return (
        ...
        '<span class="letter-small onset">l</span><span class="letter-small rest">azy</span> ' +
        '<span class="letter-small onset">l</span><span class="letter-small rest">ion</span>' +
        "</span>"
      );
    }
    return (
  ...
  function bookFoot(page, total) {
    return (
      '<div class="bk-foot">' +
      '<span class="bk-foot-meta">S-Class · The Alphabet · <b>Ll</b></span>' +
  ...
  function foot(page, total, pack) {
    return '<div class="foot"><span>S-Class · Ll · ' + pack + "</span><span>" + page + " / " + total + "</span></div>";
  }
  ...
  function lionStairsHTML() {
    return (
      '<div class="bk-connect">' +
        '<div class="bk-connect-write">' +
          '<p class="bk-connect-label">Write Ll.</p>' +
          stave(staveHTML({ kind: "letter", id: "Ll" }), "write", "fs-lg", "") +
        "</div>" +
        '<figure class="bk-connect-map"><img src="' + L.lionStairs + '" alt="Listen and write Ll on lion stairs"></figure>' +
      "</div>"
    );
  }

  function lionDotsHTML() {
    return (
      '<div class="bk-j-maze">' +
        '<figure class="bk-j-maze-art"><img src="' + L.lionDots + '" alt="Connect the dots lion"></figure>' +
        '<div class="bk-j-maze-write">' +
          stave(staveHTML({ kind: "letter", id: "Ll" }), "write", "fs-lg", "Write Ll") +
        "</div>" +
      "</div>"
    );
  }
  ...
  function buildBook() {
    ...
    pages.push(bookSheet("book-cover",
      ...
      '<p class="bk-display">Ll</p>' +
      '<p class="bk-display-hand">Ll</p>' +
      '<p class="bk-phrase">lazy lion</p>' +
      '<p class="bk-ipa">/l/</p>' +
      '<figure class="bk-hero"><img src="' + L.hero + '" alt="lazy lion"></figure>' +
      '<p class="bk-words">lion  ·  leaf  ·  leg  ·  lamp</p>' +
      nameRow() +
      bookFoot(1, total)
    ));
    pages.push(bookSheet("",
      ...
      '<div class="bk-split">' +
        '<figure class="bk-plate">' +
          '<img src="' + L.mascot.img + '" alt="lazy lion">' +
          '<figcaption class="bk-caption">lazy lion</figcaption>' +
        "</figure>" +
        '<div class="bk-letter">' +
          '<div class="bk-faces">' +
            '<div class="bk-face is-print">' +
              '<span class="face-lab">Print</span>' +
              '<span class="pair">Ll</span>' +
            "</div>" +
        ...
        "</div>" +
      "</div>" +
      stave(staveHTML({ kind: "letter", id: "Ll" }), "trace", "fs-lg", "Trace") +
    ...
    pages.push(bookSheet("",
      ...
      '<div class="letter-models">' +
        ...
        modelCard("l", "small") +
      "</div>" +
      stave('<span class="letter-pair"><span class="letter-cap onset">L</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-cap onset">L</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      stave('<span class="letter-pair"><span class="letter-small onset">l</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-small onset">l</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      bookFoot(4, total)
    ));
    pages.push(bookSheet("",
      bookHead("D", "Listen. Then write Ll or cross it out.") +
      bookSection("听音后在狮子旁写 Ll 或打叉，然后写 Ll。") +
      lionStairsHTML() +
      bookFoot(5, total)
    ));
    pages.push(bookSheet("",
      ...
      "</div>" +
      '<figure class="bk-chant-girl"><img src="' + (L.chantCharacter || L.mascot.img) + '" alt=""><p class="bk-chant-bubble">These are <img src="' + w("leg").img + '" alt="leg" class="bk-inline-egg"></p></figure>' +
      bookFoot(6, total)
    ));
    pages.push(bookSheet("",
      bookHead("F", "Match") +
    ...
    pages.push(bookSheet("",
      ...
      '<div class="bk-copy-page">' +
        wordCopyBlock(w("lion")) +
        wordCopyBlock(w("leaf")) +
      "</div>" +
      bookFoot(11, total)
    ));
    pages.push(bookSheet("",
      ...
      '<div class="bk-copy-page">' +
        wordCopyBlock(w("leg")) +
        wordCopyBlock(w("lamp")) +
      "</div>" +
      bookFoot(12, total)
    ));
    pages.push(bookSheet("",
      bookHead("L", "Circle the pictures beginning with the /l/ sound.") +
      '<p class="bk-wb-label">Workbook · Unit 4 Ll</p>' +
      workbookCircleHTML() +
      bookFoot(13, total)
    ));
    pages.push(bookSheet("",
      bookHead("M", "Connect, say, and write Ll.") +
      bookSection("沿着狮子连线图连点，说一说 leaf、leg、lamp、lion，然后写 <b>Ll</b>。") +
      lionDotsHTML() +
      bookFoot(14, total)
    ));
    return pages.join("");
  ...
  function buildGames() {
    ...
    pages.push(sheet("theme-candy",
      header("Paper games · Ll", "六个复习游戏 · 纸上也能玩", "课堂 / 回家") +
      nameRow() +
      '<ul class="howto">' +
        "<li><b>① 开头音小侦探</b> 蜡笔圈出 /l/ 开头的图。</li>" +
        "<li><b>② 听一听点一点</b> 老师读词，学生在图旁写 1–4。</li>" +
        "<li><b>③ 谁是局外人</b> 每一行打叉不是 /l/ 的那张。</li>" +
        "<li><b>④ 图词连线 + 剪卡配对</b> 先连线，再剪开做实物配对。</li>" +
        "<li><b>⑤ 写 Ll 还是打叉</b> 老师读（或放 Track 09），学生写。</li>" +
    ...
    pages.push(sheet("theme-leaf",
      ...
      task("1", "用红蜡笔圈出开头音是 /l/ 的图。不是 /l/ 的请不要圈。") +
    ...
    pages.push(sheet("",
      ...
      task("3", "每一行有三张图。把不是 /l/ 开头的那一张打 X。", "sun") +
    ...
    pages.push(sheet("theme-sky",
      header("Game 5 · Ll or X", "听音写 Ll 或打叉", "Track 09") +
      task("5", "老师放 Track 09 或读图名。是 /l/ 就写 Ll，不是就打 X。", "sky") +
      '<div class="six-grid">' +
    ...
    pages.push(sheet("theme-leaf",
      ...
      task("6", "剪下四张图。边唱 chant 边排队：leaf, leg, lamp, lion。", "leaf") +
      '<div class="cut-grid">' +
        L.chantOrder.map(function (id, i) {
          var item = w(id);
          return (
  ...
  function buildCards() {
    ...
    pages.push(sheet("theme-poster-a",
      '<div class="letter-poster"><div class="giant">L</div><div class="sub">the letter L · /l/</div>' +
      '<img src="' + L.mascot.img + '" alt="lazy lion"></div>' +
      foot(1, total, "字母卡")
    ));
    pages.push(sheet("theme-poster-s",
      '<div class="letter-poster"><div class="giant">l</div><div class="sub">little l · /l/</div>' +
      '<img src="' + w("lamp").img + '" alt="lamp"></div>' +
      foot(2, total, "字母卡")
    ));
    pages.push(sheet("theme-poster",
      '<div class="letter-poster"><div class="giant">Ll</div><div class="sub">lazy lion · beginning sound /l/</div>' +
      '<img src="' + L.hero + '" alt="Ll"></div>' +
      foot(3, total, "字母卡")
    ));
    pages.push(sheet("",
      header("Fold cards 1–2", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("lion")) + foldCard(w("leaf")) + "</div>" +
      foot(4, total, "对折卡")
    ));
    pages.push(sheet("",
      header("Fold cards 3–4", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("leg")) + foldCard(w("lamp")) + "</div>" +
      foot(5, total, "对折卡")
    ));
    pages.push(sheet("",
    ...
    pages.push(sheet("theme-leaf",
      ...
      '<div class="mini-grid">' +
        ["lion", "leaf", "leg", "lamp", "gorilla", "horse", "egg", "dog"].map(function (id) {
          var item = w(id);
    ...
    pages.push(sheet("theme-candy",
      header("Mascot + letter", "lazy lion 与 Ll 小卡", "剪开") +
      '<div class="cut-grid">' +
        '<div class="cut-card"><img src="' + L.mascot.img + '" alt=""><div class="lab">' + onsetHTML(w("lazy-lion")) + "</div></div>" +
        '<div class="cut-card word-only"><div class="lab" style="font-size:64pt;">Ll</div><div class="ipa" style="font-size:28pt;color:#2e7d32;">/l/</div></div>' +
