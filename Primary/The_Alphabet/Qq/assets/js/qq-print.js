...
(function (global) {
  ...
  function onsetHTML(item) {
    if (!item) return "";
    if (item.id === "Qq" || item.kind === "letter") {
      return '<span class="letter-pair"><span class="letter-cap onset">Q</span><span class="letter-small rest">q</span></span>';
    }
    if (item.id === "quiet-queen") {
  ...
  function staveHTML(item) {
    ...
    if (item.id === "quiet-queen") {
      return (
        ...
        '<span class="letter-small onset">q</span><span class="letter-small rest">oisy</span> ' +
        '<span class="letter-small onset">q</span><span class="letter-small rest">ut</span>' +
        "</span>"
      );
    }
    return (
  ...
  function bookFoot(page, total) {
    return (
      '<div class="bk-foot">' +
      '<span class="bk-foot-meta">S-Class · The Alphabet · <b>Qq</b></span>' +
  ...
  function foot(page, total, pack) {
    return '<div class="foot"><span>S-Class · Qq · ' + pack + "</span><span>" + page + " / " + total + "</span></div>";
  }
    ...
    return '<div class="bk-wb-match">' +
      (L.wbMatchRows || []).map(function (row) {
        return '<div class="bk-wb-match-row">' +
          '<span class="bk-wb-match-label">Qq</span>' +
  ...
  function quiltColorHTML() {
    return (
      '<div class="bk-j-maze">' +
        '<figure class="bk-j-maze-art"><img src="' + L.quiltColor + '" alt="Quilt color connect"></figure>' +
        '<div class="bk-j-maze-write">' +
          stave(staveHTML({ kind: "letter", id: "Qq" }), "write", "fs-lg", "Write Qq") +
        "</div>" +
      "</div>"
    );
  }
  ...
  function buildBook() {
    ...
    pages.push(bookSheet("book-cover",
      ...
      '<p class="bk-display">Qq</p>' +
      '<p class="bk-display-hand">Qq</p>' +
      '<p class="bk-phrase">quiet queen</p>' +
      '<p class="bk-ipa">/kw/</p>' +
      '<figure class="bk-hero"><img src="' + L.hero + '" alt="quiet queen"></figure>' +
      '<p class="bk-words">queen  ·  quiz  ·  quilt  ·  question</p>' +
      nameRow() +
      bookFoot(1, total)
    ));
    pages.push(bookSheet("",
      ...
      '<div class="bk-split">' +
        '<figure class="bk-plate">' +
          '<img src="' + L.mascot.img + '" alt="quiet queen">' +
          '<figcaption class="bk-caption">quiet queen</figcaption>' +
        "</figure>" +
        '<div class="bk-letter">' +
          '<div class="bk-faces">' +
            '<div class="bk-face is-print">' +
              '<span class="face-lab">Print</span>' +
              '<span class="pair">Qq</span>' +
            "</div>" +
            ...
            "</div>" +
          "</div>" +
          '<span class="ipa">/kw/</span>' +
        "</div>" +
      "</div>" +
      stave(staveHTML({ kind: "letter", id: "Qq" }), "trace", "fs-lg", "Trace") +
    ...
    pages.push(bookSheet("",
      ...
      '<div class="letter-models">' +
        ...
        modelCard("n", "small") +
      "</div>" +
      stave('<span class="letter-pair"><span class="letter-cap onset">Q</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-cap onset">Q</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      stave('<span class="letter-pair"><span class="letter-small onset">q</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-small onset">q</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      bookFoot(4, total)
    ));
    pages.push(bookSheet("",
      ...
      bookSection("把上面的图和下面的书写框连线配对，然后写单词。") +
      matchWriteHTML() +
      bookFoot(5, total)
    ));
    pages.push(bookSheet("",
      ...
      "</div>" +
      '<figure class="bk-chant-girl"><img src="' + (L.chantCharacter || L.mascot.img) + '" alt=""><p class="bk-chant-bubble">' + (L.chantBubblePrefix || "Is this a") + ' <img src="' + w(L.chantBubbleWord || "quilt").img + '" alt="quilt" class="bk-inline-egg"></p></figure>' +
      bookFoot(6, total)
    ));
    pages.push(bookSheet("",
      bookHead("F", "Match") +
    ...
    pages.push(bookSheet("",
      ...
      '<div class="bk-copy-page">' +
        wordCopyBlock(w("queen")) +
        wordCopyBlock(w("quiz")) +
      "</div>" +
      bookFoot(11, total)
    ));
    pages.push(bookSheet("",
      ...
      '<div class="bk-copy-page">' +
        wordCopyBlock(w("quilt")) +
        wordCopyBlock(w("question")) +
      "</div>" +
      bookFoot(12, total)
    ));
    pages.push(bookSheet("",
      bookHead("L", "Circle the pictures beginning with the q sound.") +
      '<p class="bk-wb-label">Workbook · Unit 6 Qq</p>' +
      workbookCircleHTML() +
      bookFoot(13, total)
    ));
    pages.push(bookSheet("",
      bookHead("M", "Color Q and q. Write Qq.") +
      bookSection("给被子上的 <b>Q</b> 和 <b>q</b> 涂色，说一说，然后写 <b>Qq</b>。") +
      quiltColorHTML() +
      bookFoot(14, total)
    ));
    return pages.join("");
  ...
  function buildGames() {
    ...
    pages.push(sheet("theme-candy",
      header("Paper games · Qq", "六个复习游戏 · 纸上也能玩", "课堂 / 回家") +
      nameRow() +
      '<ul class="howto">' +
        "<li><b>① 开头音小侦探</b> 蜡笔圈出 /kw/ 开头的图。</li>" +
        "<li><b>② 听一听点一点</b> 老师读词，学生在图旁写 1–4。</li>" +
        "<li><b>③ 谁是局外人</b> 每一行打叉不是 /kw/ 的那张。</li>" +
        "<li><b>④ 图词连线 + 剪卡配对</b> 先连线，再剪开做实物配对。</li>" +
        "<li><b>⑤ 写 Qq 还是打叉</b> 老师读（或放 Track 09），学生写。</li>" +
    ...
    pages.push(sheet("theme-leaf",
      ...
      task("1", "用红蜡笔圈出开头音是 /kw/ 的图。不是 /kw/ 的请不要圈。") +
    ...
    pages.push(sheet("",
      ...
      task("3", "每一行有三张图。把不是 /kw/ 开头的那一张打 X。", "sun") +
    ...
    pages.push(sheet("theme-sky",
      header("Game 5 · Qq or X", "听音写 Qq 或打叉", "Track 09") +
      task("5", "老师放 Track 09 或读图名。是 /kw/ 就写 Qq，不是就打 X。", "sky") +
      '<div class="six-grid">' +
    ...
    pages.push(sheet("theme-leaf",
      ...
      task("6", "剪下四张图。边唱 chant 边排队：quilt, question, queen, quiz。", "leaf") +
      '<div class="cut-grid">' +
        L.chantOrder.map(function (id, i) {
          var item = w(id);
          return (
  ...
  function buildCards() {
    ...
    pages.push(sheet("theme-poster-a",
      '<div class="letter-poster"><div class="giant">N</div><div class="sub">the letter N · /kw/</div>' +
      '<img src="' + L.mascot.img + '" alt="quiet queen"></div>' +
      foot(1, total, "字母卡")
    ));
    pages.push(sheet("theme-poster-s",
      '<div class="letter-poster"><div class="giant">n</div><div class="sub">little n · /kw/</div>' +
      '<img src="' + w("quilt").img + '" alt="quilt"></div>' +
      foot(2, total, "字母卡")
    ));
    pages.push(sheet("theme-poster",
      '<div class="letter-poster"><div class="giant">Qq</div><div class="sub">quiet queen · beginning sound /kw/</div>' +
      '<img src="' + L.hero + '" alt="Qq"></div>' +
      foot(3, total, "字母卡")
    ));
    pages.push(sheet("",
      header("Fold cards 1–2", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("queen")) + foldCard(w("quiz")) + "</div>" +
      foot(4, total, "对折卡")
    ));
    pages.push(sheet("",
      header("Fold cards 3–4", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("quilt")) + foldCard(w("question")) + "</div>" +
      foot(5, total, "对折卡")
    ));
    pages.push(sheet("",
    ...
    pages.push(sheet("theme-leaf",
      ...
      '<div class="mini-grid">' +
        ["queen", "quiz", "quilt", "question", "milk", "peach", "egg", "dog"].map(function (id) {
          var item = w(id);
    ...
    pages.push(sheet("",
      ...
      '<div class="cut-grid">' +
        ["cup", "king", "dog", "fan"].map(function (id) {
          var item = w(id);
          return '<div class="cut-card"><img src="' + item.img + '" alt=""><div class="lab">' + item.en + "</div></div>";
        }).join("") +
      "</div>" +
    ...
    pages.push(sheet("theme-candy",
      header("Mascot + letter", "quiet queen 与 Qq 小卡", "剪开") +
      '<div class="cut-grid">' +
        '<div class="cut-card"><img src="' + L.mascot.img + '" alt=""><div class="lab">' + onsetHTML(w("quiet-queen")) + "</div></div>" +
        '<div class="cut-card word-only"><div class="lab" style="font-size:64pt;">Qq</div><div class="ipa" style="font-size:28pt;color:#2e7d32;">/kw/</div></div>' +
