/**
 * Yy 互动练习册：描红、配对、写 Yy 或打叉。
 */
(function () {
  "use strict";
  var L = window.YY_LESSON;
  var step = 1;
  var titles = ["", "练习册 A · 描红", "练习册 B · 配对", "练习册 C · Yy / ✗"];
  ...
  var ctxY = bindTrace("Y", "cap");
  var ctxy = bindTrace("y", "small");
  ...
  function renderMatch() {
    ...
    $("btn-check-b").addEventListener("click", function () {
      ...
      $("fb-b").textContent = ok ? "全对！yo-yo、yak、yogurt、yacht 都是 /j/ 开头。" : "再看看：每行只选 y 开头的图。";
  ...
  function renderCross() {
    ...
    grid.innerHTML = (L.track05Items || []).map(function (row, i) {
      ...
      return (
        '<div class="wb-cross-item" data-i="' + i + '">' +
          ...
          '<div class="en">' + item.en + "</div>" +
          '<div class="btn-row">' +
            '<button type="button" class="btn btn-leaf" data-pick="yy" data-i="' + i + '">Yy</button>' +
    ...
    $("btn-check-c").addEventListener("click", function () {
      ...
      $("fb-c").textContent = ok ? "全对！yacht、yak、yogurt 写 Yy；wolf 打叉。" : "再听一遍：/j/ 开头写 Yy，否则打叉。";
