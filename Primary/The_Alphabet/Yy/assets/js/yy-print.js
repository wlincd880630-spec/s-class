...
(function (global) {
  ...
  function onsetHTML(item) {
    if (!item) return "";
    if (item.id === "Yy" || item.kind === "letter") {
      return '<span class="letter-pair"><span class="letter-cap onset">Y</span><span class="letter-small rest">y</span></span>';
    }
    if (item.id === "yellow-yoyo") {
      return '<span class="onset">y</span>ellow <span class="onset">y</span>o-yo';
    }
    if (!item.y) {
  ...
  function staveHTML(item) {
    ...
    if (item.id === "yellow-yoyo") {
      return (
        ...
        '<span class="letter-small onset">y</span><span class="letter-small rest">ellow</span> ' +
        '<span class="letter-small onset">y</span><span class="letter-small rest">o-yo</span>' +
        "</span>"
      );
    }
    if (!item.y) {
  ...
  function bookFoot(page, total) {
    return (
      '<div class="bk-foot">' +
      '<span class="bk-foot-meta">S-Class · The Alphabet · <b>Yy</b></span>' +
  ...
  function foot(page, total, pack) {
    return '<div class="foot"><span>S-Class · Yy · ' + pack + "</span><span>" + page + " / " + total + "</span></div>";
  }
  ...
  function listenWriteMatchHTML() {
    var items = L.listenWriteMatch || [];
    return (
      '<div class="bk-yy-listen">' +
        '<div class="bk-yy-write-row">' +
          items.map(function (row, i) {
            return '<div class="bk-yy-write-cell"><b>' + (i + 1) + "</b>" + stave("", "write", "fs-sm", "") + "</div>";
          }).join("") +
        "</div>" +
        '<div class="bk-yy-pic-row">' +
          items.map(function (row) {
            var item = w(row.id);
            return '<div class="bk-yy-pic-cell"><img src="' + item.img + '" alt="' + item.en + '"><span class="bk-ring"></span></div>';
          }).join("") +
        "</div>" +
      "</div>"
    );
  ...
  function watchWriteHTML() {
    var colors = ["purple", "blue", "orange", "pink", "lime", "red"];
    return '<div class="bk-watch-grid">' +
      (L.listenWriteMatch || []).map(function (row, i) {
        return (
          '<div class="bk-watch-cell watch-' + colors[i % colors.length] + '">' +
            '<span class="bk-watch-num">' + (i + 1) + "</span>" +
            '<div class="bk-watch-face">' +
              stave("", "write", "fs-sm", "") +
            "</div>" +
          "</div>"
        );
  ...
  function matchEightHTML() {
    return '<div class="bk-wb-six"><div class="bk-wb-six-center">' + stave(staveHTML({ kind: "letter", id: "Yy" }), "write", "fs-lg", "Write Yy") + '</div>' +
        ...
        (L.wbMatchRows || []).map(function (row, i) {
          ...
          return (
            '<div class="bk-ee-match-row">' +
              '<span class="bk-ee-label"><b>' + (i + 1) + '</b> Yy</span>' +
        ...
        (L.track05Items || []).map(function (row, i) {
          ...
          return (
            '<div class="bk-ee-write-item">' +
              ...
              '<img src="' + item.img + '" alt="' + item.en + '">' +
              '<div class="bk-egg-write"><span>Yy / ✗</span></div>' +
            "</div>"
          );
        }).join("") +
      "</div>"
  ...
  function buildBook() {
    ...
    pages.push(bookSheet("book-cover",
      ...
      '<p class="bk-display">Yy</p>' +
      '<p class="bk-display-hand">Yy</p>' +
      '<p class="bk-phrase">yellow yo-yo</p>' +
      '<p class="bk-ipa">/j/</p>' +
      '<figure class="bk-hero"><img src="' + L.hero + '" alt="yellow yo-yo"></figure>' +
      '<p class="bk-words">yo-yo  ·  yak  ·  yogurt  ·  yacht</p>' +
      nameRow() +
      bookFoot(1, total)
    ));
    pages.push(bookSheet("",
      ...
      '<div class="bk-split">' +
        '<figure class="bk-plate">' +
          '<img src="' + L.mascot.img + '" alt="yellow yo-yo">' +
          '<figcaption class="bk-caption">yellow yo-yo</figcaption>' +
        "</figure>" +
        '<div class="bk-letter">' +
          '<div class="bk-faces">' +
            '<div class="bk-face is-print">' +
              '<span class="face-lab">Print</span>' +
              '<span class="pair">Yy</span>' +
            "</div>" +
        ...
        "</div>" +
      "</div>" +
      stave(staveHTML({ kind: "letter", id: "Yy" }), "trace", "fs-lg", "Trace") +
    ...
    pages.push(bookSheet("",
      ...
      '<div class="letter-models">' +
        ...
        modelCard("y", "small") +
      "</div>" +
      stave('<span class="letter-pair"><span class="letter-cap onset">Y</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-cap onset">Y</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      stave('<span class="letter-pair"><span class="letter-small onset">y</span></span>', "trace", "fs-lg", "Trace") +
      stave('<span class="letter-pair"><span class="letter-small onset">y</span></span>', "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      bookFoot(4, total)
    ));
    pages.push(bookSheet("",
      ...
      bookSection("听音写在格子里，再连线配对图片。（Disc 2 · Track 55）") +
    ...
    pages.push(bookSheet("",
      ...
      "</div>" +
      '<figure class="bk-chant-girl"><img src="' + (L.chantCharacter || L.mascot.img) + '" alt=""><p class="bk-chant-bubble">' + (L.chantBubblePrefix || "I don't have a") + ' <img src="' + w(L.chantBubbleWord || "yoyo").img + '" alt="yo-yo" class="bk-inline-egg">.</p></figure>' +
      bookFoot(6, total)
    ));
    pages.push(bookSheet("",
      bookHead("F", "Match") +
    ...
    pages.push(bookSheet("",
      ...
      '<div class="bk-copy-page">' +
        wordCopyBlock(w("yoyo")) +
        wordCopyBlock(w(L.chantBubbleWord || "wolf")) +
      "</div>" +
      bookFoot(11, total)
    ));
    pages.push(bookSheet("",
      ...
      '<div class="bk-copy-page">' +
        wordCopyBlock(w("yogurt")) +
        wordCopyBlock(w("yacht")) +
      "</div>" +
      bookFoot(12, total)
    ));
    pages.push(bookSheet("",
      bookHead("L", "Trace, write, and say") +
      '<p class="bk-wb-label">Workbook · Unit 8 Yy</p>' +
      '<figure class="bk-story"><img src="' + w("yoyo").img + '" alt="yo-yo"></figure>' +
      stave(staveHTML({ kind: "letter", id: "Yy" }), "trace", "fs-lg", "Trace") +
      stave("", "write", "fs-lg", "Write") +
      bookFoot(13, total)
    ));
    pages.push(bookSheet("",
      bookHead("M", "Which ones begin with y? Match and write.") +
      bookSection('Which ones begin with the <span class="bk-ipa-inline">/j/</span> sound? Match and write <b>Yy</b>.') +
      workbookMatchHTML() +
      bookFoot(14, total)
    ));
    pages.push(bookSheet("",
      bookHead("N", "Say. Then write Yy or cross it out.") +
      bookSection("说一说，然后在四线格写 <b>Yy</b> 或打叉。") +
      workbookWriteHTML() +
      bookFoot(15, total)
    ));
    return pages.join("");
  ...
  function buildGames() {
    ...
    pages.push(sheet("theme-candy",
      header("Paper games · Yy", "六个复习游戏 · 纸上也能玩", "课堂 / 回家") +
      nameRow() +
      '<ul class="howto">' +
        "<li><b>① 开头音小侦探</b> 蜡笔圈出 /j/ 开头的图。</li>" +
        "<li><b>② 听一听点一点</b> 老师读词，学生在图旁写 1–4。</li>" +
        "<li><b>③ 谁是局外人</b> 每一行打叉不是 /j/ 的那张。</li>" +
        "<li><b>④ 图词连线 + 剪卡配对</b> 先连线，再剪开做实物配对。</li>" +
        "<li><b>⑤ 写 Yy 还是打叉</b> 老师读（或放 Track 09），学生写。</li>" +
    ...
    pages.push(sheet("theme-leaf",
      ...
      task("1", "用红蜡笔圈出开头音是 /j/ 的图。不是 /j/ 的请不要圈。") +
    ...
    pages.push(sheet("",
      ...
      task("3", "每一行有三张图。把不是 /j/ 开头的那一张打 X。", "sun") +
    ...
    pages.push(sheet("theme-sky",
      header("Game 5 · Yy or X", "听音写 Yy 或打叉", "Track 09") +
      task("5", "老师放 Track 09 或读图名。是 /j/ 就写 Yy，不是就打 X。", "sky") +
      '<div class="six-grid">' +
        L.track05Items.map(function (row, i) {
          var item = w(row.id);
          return (
    ...
    pages.push(sheet("theme-leaf",
      ...
      task("6", "剪下四张图。边唱 chant 边排队：yo-yo, yak, yogurt, yacht。", "leaf") +
      '<div class="cut-grid">' +
        L.chantOrder.map(function (id, i) {
          var item = w(id);
          return (
  ...
  function buildCards() {
    ...
    pages.push(sheet("theme-poster-a",
      '<div class="letter-poster"><div class="giant">Y</div><div class="sub">the letter Y · /j/</div>' +
      '<img src="' + L.mascot.img + '" alt="yellow yo-yo"></div>' +
      foot(1, total, "字母卡")
    ));
    pages.push(sheet("theme-poster-s",
      '<div class="letter-poster"><div class="giant">y</div><div class="sub">little y · /j/</div>' +
      '<img src="' + w("yoyo").img + '" alt="yo-yo"></div>' +
      foot(2, total, "字母卡")
    ));
    pages.push(sheet("theme-poster",
      '<div class="letter-poster"><div class="giant">Yy</div><div class="sub">yellow yo-yo · beginning sound /j/</div>' +
      '<img src="' + L.hero + '" alt="Yy"></div>' +
      foot(3, total, "字母卡")
    ));
    pages.push(sheet("",
      header("Fold cards 1–2", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("yoyo")) + foldCard(w("yak")) + "</div>" +
      foot(4, total, "对折卡")
    ));
    pages.push(sheet("",
      header("Fold cards 3–4", "对折闪卡 · 图背对词", "剪开对折") +
      '<div class="fold-page" style="height:236mm">' + foldCard(w("yak")) + foldCard(w("yogurt")) + "</div>" +
      foot(5, total, "对折卡")
    ));
    pages.push(sheet("",
    ...
    pages.push(sheet("theme-leaf",
      ...
      '<div class="mini-grid">' +
        ["wet", "kite", "box", "jacket", "hat", "jet", "wolf", "web"].map(function (id) {
          var item = w(id);
    ...
    pages.push(sheet("",
      ...
      '<div class="cut-grid">' +
        ["apple", "bear", "cat", "fox"].map(function (id) {
          var item = w(id);
          return '<div class="cut-card"><img src="' + item.img + '" alt=""><div class="lab">' + item.en + "</div></div>";
        }).join("") +
      "</div>" +
    ...
    pages.push(sheet("theme-candy",
      header("Mascot + letter", "yellow yo-yo 与 Yy 小卡", "剪开") +
      '<div class="cut-grid">' +
        '<div class="cut-card"><img src="' + L.mascot.img + '" alt=""><div class="lab">' + onsetHTML(w("yellow-yoyo")) + "</div></div>" +
        '<div class="cut-card word-only"><div class="lab" style="font-size:64pt;">Yy</div><div class="ipa" style="font-size:28pt;color:#2e7d32;">/j/</div></div>' +
