/**
 * TL11 閰嶅浘鐐瑰嚮鏀惧ぇ锛坰cene-frame / comic-frame锛?
 * 浜嬩欢濮旀墭锛屾敮鎸佸姩鎬佹崲鍥撅紙pair-scene-img銆乨isc-scene-img 绛夛級
 */
(function () {
  const LB_ID = "lesson-img-lightbox";
  const ZOOM_IMG =
    "#l14-book .scene-frame img, #l11-book .scene-frame img, #app .scene-frame img, " +
    "#l14-book .comic-frame img, #l11-book .comic-frame img, #app .comic-frame img";
  ...
  function ensureLightbox() {
    ...
    lb.setAttribute("aria-label", "閰嶅浘鏀惧ぇ");

    const closeBtn = document.createElement("button");
    closeBtn.type = "button";
    closeBtn.className = "lesson-img-lightbox-close";
    closeBtn.setAttribute("aria-label", "鍏抽棴");
  ...
  function decorateZoomImages(root) {
    root.querySelectorAll(ZOOM_IMG).forEach((img) => {
      ...
      if (!img.title) img.title = "鐐瑰嚮鏀惧ぇ";
    });
  }

  function initLessonImageLightbox() {
    const root =
      document.getElementById("l14-book") ||
      document.getElementById("l11-book") ||
      document.querySelector("#app");
    if (!root || root.dataset.lessonLightbox === "1") return;
    root.dataset.lessonLightbox = "1";
