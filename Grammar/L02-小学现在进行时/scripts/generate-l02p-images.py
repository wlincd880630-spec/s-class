#!/usr/bin/env python3
"""
L03 小学现在进行时 · 批量生成配图（Pollinations flux）并上传腾讯云 COS。

用法:
  python3 Grammar/L02-小学现在进行时/scripts/generate-l02p-images.py audit
  python3 Grammar/L02-小学现在进行时/scripts/generate-l02p-images.py generate
  python3 Grammar/L02-小学现在进行时/scripts/generate-l02p-images.py upload
  python3 Grammar/L02-小学现在进行时/scripts/generate-l02p-images.py all
...
COS_PREFIX = "s-class/Grammar/L02-小学现在进行时/assets/img/"
...
def collect_refs() -> set[str]:
    refs: set[str] = set()
    scenes = (ASSETS / "l02p-scenes.js").read_text(encoding="utf-8")
    refs.update(re.findall(r"l02p-[a-z0-9-]+\.jpg", scenes))
    for name in ("l02p-data.js", "l02p-corpus.js"):
        refs.update(re.findall(r"l02p-[a-z0-9-]+\.jpg", (ASSETS / name).read_text(encoding="utf-8")))
    return refs


def build_manifest() -> list[dict]:
    todo_path = ASSETS / "images-todo.json"
    if todo_path.exists():
        tasks = []
        for item in json.loads(todo_path.read_text(encoding="utf-8")):
            fn = item["filename"]
            tasks.append(
                {
                    "filename": fn,
                    "sentence": item.get("sentence", ""),
                    "prompt": item.get("prompt") or STYLE + fn,
                    "seed": abs(hash(fn)) % 10_000_000,
                }
            )
        local = {p.name for p in IMG_DIR.iterdir()} if IMG_DIR.exists() else set()
        return [t for t in tasks if t["filename"] not in local]
    ...
    for i, s in enumerate(corpus.get("sceneSentences", [])):
        fn = img_name("l02p-scene-", s["en"], i + 1)
    ...
    for kind, key in [
        ...
        for i, v in enumerate(corpus.get(key, [])):
            ...
            fn = img_name(f"l02p-{kind}-", ex, i + 1)
...
def download_one(task: dict) -> tuple[str, bool, str]:
    ...
    for attempt in range(1, RETRIES + 1):
        try:
            ...
            req = urllib.request.Request(url, headers={"User-Agent": "s-class-l02p/1.0"})
...
def cmd_upload() -> None:
    ...
    files = sorted(IMG_DIR.glob("l02p-*.jpg"))
