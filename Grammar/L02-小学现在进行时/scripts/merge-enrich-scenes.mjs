#!/usr/bin/env node
/**
 * 把 enrich 语料合并进主语料，并修正 was/were + V-ing（过去进行时）为现在进行时
 */
import fs from "fs";
      ...
      messages: [
        {
          role: "system",
          content:
            "你是小学英语编辑。把过去进行时句子改写成现在进行时，保留画面细节与人物。只输出 JSON：{\"items\":[{\"id\":\"...\",\"en\":\"...\",\"zh\":\"...\",\"focus\":\"was|were|regular|irregular|...\",\"imageHint\":\"...\"}]}。不要 markdown。was/were 仅用于表语状态（happy/at home），不要 was doing。",
        },
        {
          role: "user",
...
function normalizeSentence(s, i) {
  ...
  return {
    ...
    image: "l02p-scene-" + slugify(en) + ".jpg",
  };
}
