...
(function (global) {
  ...
  function stemText(q) {
    if (global.L13RCUI && typeof global.L13RCUI.quizStemText === "function") {
      return global.L13RCUI.quizStemText(q) || "????????";
    }
    if (!q) return "????????";
  ...
  function stemExtraHtml(q) {
    if (global.L13RCUI && typeof global.L13RCUI.quizStemExtraHtml === "function") {
      return global.L13RCUI.quizStemExtraHtml(q) || "";
    }
    if (q && q.audio && !q.stem) {
      return '<p class="zh-hint quiz-listen-hint">??????????????</p>';
    }
    return "";
  }

  function normOpts(q) {
    if (global.L13RCUI && typeof global.L13RCUI.quizOpts === "function") {
      return global.L13RCUI.quizOpts(q);
    }
    var raw = q && q.opts;
  ...
  function correctAnswerText(q, opts) {
    if (!q || q.ans == null) return "";
    if (typeof q.ans === "number") {
      return opts[q.ans] != null ? opts[q.ans] : "";
    }
    var s = String(q.ans);
    return opts.indexOf(s) >= 0 ? s : s;
  }

  function isCorrectPick(q, opts, pick) {
    var ans = correctAnswerText(q, opts);
    return pick != null && ans !== "" && pick === ans;
  }
  ...
  function createQuizEngine(options) {
    ...
    const slotId = options.slotId || "quiz-" + key;
    ...
    function render(i) {
      ...
      const correct = correctAnswerText(q, opts);
      opts.forEach(function (opt) {
        ...
        if (saved.answered) {
          btn.disabled = true;
          if (opt === correct) btn.classList.add("ok");
          else if (opt === saved.pick) btn.classList.add("bad");
        }
        btn.addEventListener("click", function () {
          resolve(isCorrectPick(q, opts, opt), opt);
        });
        row.appendChild(btn);
      });
