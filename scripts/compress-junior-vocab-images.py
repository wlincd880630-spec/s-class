#!/usr/bin/env python3
"""Compress any new *_1.png under junior_vocab books to jpg and patch JSON img1."""
import json, re, sys
from pathlib import Path
from PIL import Image

ROOT = Path('/workspace/junior_vocab')
BOOKS = sys.argv[1:] or ['G7_B1', 'G7_B2', 'G8_B2', 'G9', 'G8_B1']
...
for book in BOOKS:
    ...
    for unit_dir in sorted(bdir.glob('Unit[0-9]*')):
        ...
        if not img_dir.exists():
            continue
        for png in list(img_dir.glob('*_1.png')):
...
for book in BOOKS:
    ...
    for unit_dir in sorted(bdir.glob('Unit[0-9]*')):
        ...
        for w in json.load(open(jf)).get('words', []):
            ...
            s = slug(w['word'])
            img_dir = unit_dir / 'images'
            if img_dir.exists() and any(p.stem.lower() == f'{s}_1'.lower() for p in img_dir.glob('*_1.jpg')):
                have += 1
    print(f'{book}: {have}/{total}')
