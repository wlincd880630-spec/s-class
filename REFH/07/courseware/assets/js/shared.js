...
请返回JSON: {"score":0-100,"issues":"问题指出(中文)","standard":"标准翻译(中文)"}`;
  ...
  function renderNav(active) {
    ...
    return `<nav class="top-nav">
      <a class="brand" href="index.html">🏄 Meet the Dogs · Surfing Champs</a>
  ...
  function getArticlePdfCss(accent) {
    ...
    return `
      ...
      .pdf-img-wrap{
        ...
        background:linear-gradient(180deg,#e0f7fa,#f0f9ff);border:1px solid #bae6fd;
        box-sizing:border-box;overflow:visible;
      }
      .pdf-section-img{
        width:100%;height:auto;max-height:none;object-fit:contain;object-position:center;
        border-radius:8px;margin:0;border:0;display:block;background:#e0f7fa;
      }
  ...
  function getQuizPdfCss() {
    return `
      ...
      .pdf-export-block{overflow:visible;page-break-inside:avoid;break-inside:avoid;}
      .qpdf-cover{
        border-radius:16px;overflow:hidden;border:2px solid #0e7490;
        background:linear-gradient(135deg,#0e7490 0%,#0369a1 42%,#7c3aed 78%,#db2777 100%);
    ...
    return `<div class="pdf-export-block qpdf-cover">
      <div class="qpdf-cover-top">
        <div class="qpdf-cover-copy">
          <div class="qpdf-kicker">S-Class · REFH 07 · Vocabulary Quiz Packet</div>
          <h1>${escapeHtml(data.title || meta.title || 'Meet the Dogs')}</h1>
