...
(function (global) {
  ...
  const CONFIG_KEY = 'cappadocia_balloons_courseware_config';
  const IMAGE_BASE = 'https://s-class-1403296481.cos.ap-chengdu.myqcloud.com/s-class/REFH/10/courseware/assets/images/';
  const COURSEWARE_WEB_BASE = 'https://s-class-1403296481.cos.ap-chengdu.myqcloud.com/s-class/REFH/10/courseware/';
  const ARTICLE_PAGE_URL = COURSEWARE_WEB_BASE + 'part2-reading.html';
  const QUIZ_PAGE_URL = 'https://www.s-class.top/REFH/10/courseware/part4-quiz.html';
  const QUIZ_KICKER = 'S-Class · REFH 10 · Vocabulary Quiz Packet';
  const QUIZ_ACCENT = '#b45309';
...
请返回JSON: {"score":0-100,"issues":"问题指出(中文)","standard":"标准翻译(中文)"}`;
  ...
  function renderNav(active) {
    ...
    return `<nav class="top-nav">
      <a class="brand" href="index.html">🎈 Cappadocia · Female Pilots</a>
  ...
  function getArticlePdfCss(accent) {
    ...
    return `
      ...
      .pdf-img-wrap{
        width:100%;margin-bottom:10px;padding:6px;border-radius:10px;
        background:linear-gradient(180deg,#eef1f7,#f7f4ef);border:1px solid #e4ddd4;
        box-sizing:border-box;overflow:visible;
      }
      .pdf-section-img{
        width:100%;height:auto;max-height:none;object-fit:contain;object-position:center;
        border-radius:8px;margin:0;border:0;display:block;background:#eef1f7;
      }
    ...
    try {
      ...
      for (const block of root.querySelectorAll('.pdf-export-block')) {
        ...
        appendCanvasAsWholeBlock(pdf, canvas, st, {
          fitOnePage: block.classList.contains('pdf-img-wrap')
        });
